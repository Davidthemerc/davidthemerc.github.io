...
const loadHero = () => {
  const saveJSON = localStorage.getItem('SS-heroData');
...
const enemyGeneration = () => {
  const saveJSON = localStorage.getItem('SS-enemyData');
...
const heroAttack = (weaponName, weapon) => {
  ...
  damageAmount > 0
    ? playAudio(weapon.weaponHitSound)
    : playAudio(weapon.weaponMissSound);
  ...
  if (enemy.hitpoints <= 0) {
    ...
    lootbutton.textContent = 'Loot Dead Enemy';
    ...
    localStorage.removeItem('SS-enemyData');
    return;
  }

  saveJSON(enemy, 'SS-enemyData');
  damageAmount > 0
    ? displayMessage(
...
const lootEnemy = (enemy) => {
  ...
  let coinAmount = ranBetween(1, 99);
  coinChange(coinAmount, '+');
  saveJSON(hero, 'SS-heroData');
  // Play coin sound
  playAudio(2);
  ...
  displayMessage(
    `<b>${hero.name}</b> collected ${coinAmount} cents.`,
    statusEl,
    3
  );
  setTimeout(() => {
