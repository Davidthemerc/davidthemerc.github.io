...
DSH.Seasons=(()=>{
 ...
 function effects(state,level=state?.level||1){
   const out={...(info(level).effects||{})},tier=Math.max(0,Math.min(3,Math.floor(Number(state?.farmYearMasteryTier)||0)));
   if(tier){const mult=1+tier*(C().farmYear?.masteryFarmCoinPerTier||.02);out.cropCoin=(Number(out.cropCoin)||1)*mult;out.orderCoin=(Number(out.orderCoin)||1)*mult}
   return out;
 }
 function farmYearBasketStatus(state,year){
   const defs=C().definitions,claimed={};
   defs.forEach(d=>{const key=`${year}-${d.key}`;claimed[d.key]=state?.seasonState?.key===key?state.seasonState.claimed===true:state?.seasonHistory?.[key]?.claimed===true});
   return{claimed,count:Object.values(claimed).filter(Boolean).length,complete:defs.every(d=>claimed[d.key])};
 }
 function trimFarmYearHistory(state){
   const keys=Object.keys(state.farmYearHistory||{}).map(Number).filter(Number.isFinite).sort((a,b)=>b-a);keys.slice(24).forEach(k=>delete state.farmYearHistory[k]);
 }
 function farmYearRewardFor(state,year,streak){
   const fy=C().farmYear||{},scaleYears=Math.max(0,Math.min(fy.coinScaleCapYears||12,Math.max(1,year)-1)),streakScale=Math.max(0,Math.min(fy.streakCap||5,streak||0));
   const coins=(fy.baseCoins||1800)+scaleYears*(fy.coinPerYear||150)+streakScale*(fy.streakCoinBonus||250);
   const gems=(fy.baseGems||5)+Math.floor(scaleYears/4)+Math.floor(streakScale/3),bundle=year%4;
   return{coins,gems,windmills:bundle===1?1:0,gates:bundle===2?1:0,rescues:bundle===3?1:0,treats:bundle===0?2:0};
 }
 function finalizeFarmYear(state,year,allowMiss=true){
   year=Math.max(1,Math.floor(Number(year)||1));state.farmYearHistory=state.farmYearHistory&&typeof state.farmYearHistory==='object'?state.farmYearHistory:{};
   if(state.farmYearHistory[year])return state.farmYearHistory[year];
   const status=farmYearBasketStatus(state,year);if(!status.complete&&!allowMiss)return null;
   if(!status.complete){state.farmYearStreak=0;const miss={year,complete:false,baskets:status.count,completedAt:Date.now(),streak:0,reward:null,masteryTier:state.farmYearMasteryTier||0};state.farmYearHistory[year]=miss;trimFarmYearHistory(state);return miss}
   state.farmYearStreak=(state.farmYearStreak||0)+1;state.bestFarmYearStreak=Math.max(state.bestFarmYearStreak||0,state.farmYearStreak);state.farmYearsCompleted=(state.farmYearsCompleted||0)+1;
   const reward=farmYearRewardFor(state,year,state.farmYearStreak);
   state.coins=(state.coins||0)+reward.coins;state.gems=(state.gems||0)+reward.gems;state.windmills=(state.windmills||0)+reward.windmills;state.magicGates=(state.magicGates||0)+reward.gates;state.rosieRescues=(state.rosieRescues||0)+reward.rescues;state.rosieTreats=(state.rosieTreats||0)+reward.treats;
   state.stats=state.stats||{};state.stats.farmYearsCompleted=(state.stats.farmYearsCompleted||0)+1;state.stats.farmYearFinaleCoins=(state.stats.farmYearFinaleCoins||0)+reward.coins;state.stats.farmYearFinaleGems=(state.stats.farmYearFinaleGems||0)+reward.gems;state.stats.bestFarmYearStreak=Math.max(state.stats.bestFarmYearStreak||0,state.farmYearStreak);
   if(reward.windmills)state.stats.windmillsFound=(state.stats.windmillsFound||0)+reward.windmills;if(reward.gates)state.stats.magicGatesFound=(state.stats.magicGatesFound||0)+reward.gates;if(reward.rescues)state.stats.rosieRescuesFound=(state.stats.rosieRescuesFound||0)+reward.rescues;if(reward.treats)state.stats.treatsFound=(state.stats.treatsFound||0)+reward.treats;
   const thresholds=C().farmYear?.masteryThresholds||[3,6,10],newTier=thresholds.filter(n=>state.farmYearsCompleted>=n).length,oldTier=state.farmYearMasteryTier||0;state.farmYearMasteryTier=Math.max(oldTier,Math.min(3,newTier));const masteryUp=state.farmYearMasteryTier>oldTier;
   if(masteryUp)state.stats.farmYearMasteryUpgrades=(state.stats.farmYearMasteryUpgrades||0)+(state.farmYearMasteryTier-oldTier);
   const result={year,complete:true,baskets:4,completedAt:Date.now(),streak:state.farmYearStreak,reward,masteryTier:state.farmYearMasteryTier,masteryUp};state.farmYearHistory[year]=result;trimFarmYearHistory(state);return result;
 }
 function farmYearStatus(state,year=info(state?.level||1).year){
   const baskets=farmYearBasketStatus(state,year),record=state?.farmYearHistory?.[year]||null;
   return{year,...baskets,record,streak:state?.farmYearStreak||0,bestStreak:state?.bestFarmYearStreak||0,completed:state?.farmYearsCompleted||0,masteryTier:state?.farmYearMasteryTier||0};
 }
 ...
 function ensure(state,level=state?.level||1){
   ...
   if(!cur||cur.key!==inf.key){
     if(cur?.key&&cur.key!==inf.key){archiveCurrent(state,cur);if(cur.season==='winter'&&inf.year>cur.year)finalizeFarmYear(state,cur.year,true)}
   ...
   state.farmYearHistory=state.farmYearHistory&&typeof state.farmYearHistory==='object'?state.farmYearHistory:{};
   state.farmYearStreak=Math.max(0,Math.floor(Number(state.farmYearStreak)||0));state.bestFarmYearStreak=Math.max(state.farmYearStreak,Math.floor(Number(state.bestFarmYearStreak)||0));state.farmYearsCompleted=Math.max(0,Math.floor(Number(state.farmYearsCompleted)||0));state.farmYearMasteryTier=Math.max(0,Math.min(3,Math.floor(Number(state.farmYearMasteryTier)||0)));
   return inf;
 }
 ...
 function award(state,inf){
   ...
   const yearResult=inf.key.endsWith('-winter')?finalizeFarmYear(state,inf.year,false):null;
   return{reward:r,firstKeepsake,yearResult};
 }
 function addProgress(state,source,units=1,level=state?.level||1){
   ...
   let completed=false,reward=null,firstKeepsake=false,yearResult=null;if(ss.points>=inf.goal&&!ss.claimed){completed=true;const a=award(state,inf);reward=a.reward;firstKeepsake=a.firstKeepsake;yearResult=a.yearResult||null}
   return{info:inf,added,completed,reward,firstKeepsake,yearResult,points:ss.points,goal:inf.goal};
 }
 return{info,byKey,effects,ensure,addProgress,rewardFor,rewardText,effectText,variantText,orderFavors,luckyFavor,challengeFlavor,combineEffects,farmYearStatus,farmYearRewardFor,finalizeFarmYear};
})();
