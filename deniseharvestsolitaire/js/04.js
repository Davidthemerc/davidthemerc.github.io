...
DSH.Levels=(()=>{
 ...
 const formations=[
  ...
  {name:'Blossom Bridge',min:12,season:'spring',build:blossomBridge},
  {name:'Petal Path',min:20,season:'spring',build:petalPath},
  {name:'Sunbeam Terrace',min:12,season:'summer',build:sunbeamTerrace},
  {name:'Harvest Ridge',min:20,season:'summer',build:harvestRidge},
  {name:'Leaf Spiral',min:12,season:'autumn',build:leafSpiral},
  {name:'Orchard Steps',min:20,season:'autumn',build:orchardSteps},
  {name:'Frosted Peaks',min:12,season:'winter',build:frostedPeaks},
  {name:'Snowfall Lane',min:20,season:'winter',build:snowfallLane}
 ];
 ...
 function seasonalRows(count,widths,wiggle=0){
   let a=[];for(let r=0;r<widths.length;r++){const n=widths[r];for(let i=0;i<n;i++){
     const x=.5+(i-(n-1)/2)*(.72/Math.max(n-1,1))+Math.sin((i+1)*(r+1))*wiggle;
     a.push(node(Math.max(.06,Math.min(.94,x)),.045+r*(.70/Math.max(1,widths.length-1)),r))
   }}return autoBlock(trim(a,count),.095,2)
 }
 function blossomBridge(count){return seasonalRows(count,[4,7,9,7,5],.012)}
 function petalPath(count){let a=[];for(let r=0;r<5;r++){const n=5+r;for(let i=0;i<n;i++){const bend=Math.sin((r*2+i)*.9)*.055;a.push(node(.5+(i-(n-1)/2)*.095+bend,.05+r*.16,r))}}return autoBlock(trim(a,count),.09,2)}
 function sunbeamTerrace(count){return seasonalRows(count,[3,5,7,9,7],.006)}
 function harvestRidge(count){let a=[];for(let r=0;r<5;r++){const n=6+(r%2)*2;for(let i=0;i<n;i++){const rise=Math.abs(i-(n-1)/2)*.012;a.push(node((i+.5)/n,.055+r*.15+rise,r))}}return autoBlock(trim(a,count),.085,2)}
 function leafSpiral(count){let a=[];const total=Math.max(30,count);for(let i=0;i<total;i++){const t=i/(total-1),ang=t*Math.PI*3.1,rad=.32*(1-t*.58);a.push(node(.5+Math.cos(ang)*rad,.38+Math.sin(ang)*rad*.72,Math.floor(t*6))) }return autoBlock(trim(a,count),.082,2)}
 function orchardSteps(count){let a=[];for(let r=0;r<5;r++){const n=5+r;for(let i=0;i<n;i++)a.push(node(.16+r*.045+i*.075,.05+r*.155,r))}return autoBlock(trim(a,count),.085,2)}
 function frostedPeaks(count){return seasonalRows(count,[3,6,9,6,9],.016)}
 function snowfallLane(count){let a=[];for(let r=0;r<5;r++){const n=6;for(let i=0;i<n;i++){const drift=(r%2?.045:-.025)+Math.sin(i*1.4+r)*.018;a.push(node(.1+i*.16+drift,.05+r*.16,r))}}return autoBlock(trim(a,count),.09,1)}
 ...
 function build(level,rng=Math.random){
   ...
   try{
     const milestone=level%10===0,farmYearMilestone=level%40===0;
     const seasonKeys=['spring','summer','autumn','winter'],seasonKey=seasonKeys[Math.floor((level-1)/10)%4];
     const available=formations.filter(f=>level>=f.min&&(!f.season||f.season===seasonKey));
     const seasonal=available.filter(f=>f.season===seasonKey);
     // Seasonal formations are common enough to establish identity, while classic
     // formations remain in circulation. Farm Year finales always use a seasonal field.
     let f=(farmYearMilestone&&seasonal.length)?seasonal[Math.floor(R()*seasonal.length)]:
       (seasonal.length&&R()<.38)?seasonal[Math.floor(R()*seasonal.length)]:available[Math.floor(R()*available.length)];
     const count=targetCount(level),cards=farmYearMilestone?f.build(count):(milestone?milestoneFormation(level):f.build(count));let specials=decorate(cards,level);
     ...
     return{
       name:farmYearMilestone?`${f.name} • Farm Year Milestone`:milestone?`${milestoneFormationName(level)} ${level}`:f.name,cards,count:cards.length,milestone,farmYearMilestone,seasonKey,seasonalFormation:farmYearMilestone||(!milestone&&!!f.season),landmark:!!landmark,
       stockTarget:Math.max(DSH.Config?.difficulty?.stockFloor||24,Math.min(DSH.Config?.difficulty?.weatherCeiling||48,baseStock+specials.stockBonus+milestoneBonus+(farmYearMilestone?2:0))),
