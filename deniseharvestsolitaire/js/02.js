...
DSH.Save=(()=>{
 ...
 function defaults(){return{
   ...
   farmYearHistory:{},farmYearStreak:0,bestFarmYearStreak:0,farmYearsCompleted:0,farmYearMasteryTier:0,
   ...
   returnRewards:{activationDate:'',lastSeenDate:'',lastVisitDate:'',lastDailyClaimDate:'',dailyStreak:0,bestDailyStreak:0,daily:{},weekly:{},monthly:{},clockHold:false},_makeupProgressBackup:null,
   collection:{specials:{},crops:{},regions:{0:true},powers:{},rosie:{}},
   mastery:{crops:{},specials:{},powers:{},rosie:{}},masteryBackfillVersion:0,masteryRewardClaims:{},masterySetClaims:{},
   ...
   dailyBestWinStreak:0,dailyCoinsEarned:0,dailyGemsEarned:0,
   returnDaysVisited:0,returnDaysClaimed:0,returnBestStreak:0,returnCoinsEarned:0,returnGemsEarned:0,returnTreatsEarned:0,weeklySlotsClaimed:0,monthlySlotsClaimed:0,makeupBoardsStarted:0,makeupBoardsCompleted:0,weeklyCompletionRewards:0,monthlyCompletionRewards:0,
   masteryRewardsClaimed:0,masterySetsCompleted:0,masteryRewardCoins:0,masteryRewardGems:0,
   tier2UpgradesOwned:0,
   ...
   farmYearsCompleted:0,farmYearFinaleCoins:0,farmYearFinaleGems:0,bestFarmYearStreak:0,farmYearMasteryUpgrades:0,
 ...
 function sanitize(input){
   ...
   ['upgrades','settings','stats','levelStars','achievements','collection','regionRewardsClaimed','regionDecorations','milestonesCleared','chapterBest',
    'rosieAdventureFinds','rosieAdventureHistory','rosieSeasonalFinds','rosieSeasonStats','seasonState','seasonKeepsakes','seasonHistory','farmYearHistory','farmhouseTrophies','farmhouseTierClaims','farmhouseTrophyRewardsClaimed','farmhouseTierRewardsClaimed','rosieToys','chapterMemorabilia','lifetime','seenMechanics','saveHealth',
    'eventProgress','eventClaimed','mastery','returnRewards','masteryRewardClaims','masterySetClaims'].forEach(k=>{if(!state[k]||typeof state[k]!=='object'||Array.isArray(state[k])){issues.push(`Missing or invalid ${k}`);state[k]=JSON.parse(JSON.stringify(d[k]||{}))}});
   ...
   state.seasonState.points=Math.max(0,Math.min(DSH.Config.seasons?.pointGoal||18,Math.floor(finiteNumber(state.seasonState.points,0))));state.seasonState.claimed=asBool(state.seasonState.claimed);if(state.seasonState.claimed)state.seasonState.points=DSH.Config.seasons?.pointGoal||18;state.seasonState.completedAt=Math.max(0,Math.floor(finiteNumber(state.seasonState.completedAt,0)));
   ...
   Object.keys(state.seasonHistory).forEach(k=>{const x=state.seasonHistory[k];if(!x||typeof x!=='object'){delete state.seasonHistory[k];return}x.season=typeof x.season==='string'?x.season:'';x.year=Math.max(1,Math.floor(finiteNumber(x.year,1)));x.startLevel=Math.max(1,Math.floor(finiteNumber(x.startLevel,1)));x.endLevel=Math.max(x.startLevel,Math.floor(finiteNumber(x.endLevel,x.startLevel+9)));x.points=Math.max(0,Math.min(DSH.Config.seasons?.pointGoal||18,Math.floor(finiteNumber(x.points,0))));x.claimed=asBool(x.claimed);if(x.claimed)x.points=DSH.Config.seasons?.pointGoal||18;x.completedAt=Math.max(0,Math.floor(finiteNumber(x.completedAt,0)))});
   // Bound seasonal history so malformed imports cannot grow the save forever.
   Object.keys(state.seasonHistory).sort((a,b)=>(state.seasonHistory[b]?.endLevel||0)-(state.seasonHistory[a]?.endLevel||0)).slice(24).forEach(k=>delete state.seasonHistory[k]);
   // Farm Year metadata is progression-bearing, so never trust malformed/future records.
   state.farmYearHistory=state.farmYearHistory&&typeof state.farmYearHistory==='object'&&!Array.isArray(state.farmYearHistory)?state.farmYearHistory:{};
   const currentFarmYear=Math.max(1,Math.floor((state.level-1)/40)+1),fyCfg=DSH.Config.seasons?.farmYear||{},fyThresholds=fyCfg.masteryThresholds||[3,6,10];
   Object.keys(state.farmYearHistory).forEach(k=>{
     const x=state.farmYearHistory[k],year=Math.floor(finiteNumber(k,0));
     if(!x||typeof x!=='object'||year<1||year>currentFarmYear){delete state.farmYearHistory[k];issues.push('Invalid Farm Year history entry');return}
     x.year=year;x.complete=asBool(x.complete);x.baskets=Math.max(0,Math.min(4,Math.floor(finiteNumber(x.baskets,x.complete?4:0))));if(x.complete)x.baskets=4;
     x.completedAt=Math.max(0,Math.floor(finiteNumber(x.completedAt,0)));x.streak=x.complete?Math.max(1,Math.floor(finiteNumber(x.streak,1))):0;x.masteryTier=Math.max(0,Math.min(3,Math.floor(finiteNumber(x.masteryTier,0))));
     if(x.complete){
       const r=x.reward&&typeof x.reward==='object'&&!Array.isArray(x.reward)?x.reward:{};x.reward={coins:Math.max(0,Math.floor(finiteNumber(r.coins,0))),gems:Math.max(0,Math.floor(finiteNumber(r.gems,0))),windmills:Math.max(0,Math.floor(finiteNumber(r.windmills,0))),gates:Math.max(0,Math.floor(finiteNumber(r.gates,0))),rescues:Math.max(0,Math.floor(finiteNumber(r.rescues,0))),treats:Math.max(0,Math.floor(finiteNumber(r.treats,0)))};
     }else x.reward=null;
   });
   Object.keys(state.farmYearHistory).map(Number).sort((a,b)=>b-a).slice(24).forEach(k=>delete state.farmYearHistory[k]);
   const recordedComplete=Object.values(state.farmYearHistory).filter(x=>x?.complete).length;
   state.farmYearsCompleted=Math.max(recordedComplete,Math.min(currentFarmYear,Math.max(0,Math.floor(finiteNumber(state.farmYearsCompleted,0)))));
   state.farmYearStreak=Math.min(state.farmYearsCompleted,Math.max(0,Math.floor(finiteNumber(state.farmYearStreak,0))));
   state.bestFarmYearStreak=Math.min(state.farmYearsCompleted,Math.max(state.farmYearStreak,Math.floor(finiteNumber(state.bestFarmYearStreak,0))));
   state.farmYearMasteryTier=Math.min(3,fyThresholds.filter(n=>state.farmYearsCompleted>=n).length);
   state.stats.farmYearsCompleted=Math.max(state.stats.farmYearsCompleted||0,state.farmYearsCompleted);state.stats.bestFarmYearStreak=Math.max(state.stats.bestFarmYearStreak||0,state.bestFarmYearStreak);
   state.mastery=state.mastery&&typeof state.mastery==='object'&&!Array.isArray(state.mastery)?state.mastery:{crops:{},specials:{},powers:{},rosie:{}};
   ['crops','specials','powers','rosie'].forEach(group=>{state.mastery[group]=state.mastery[group]&&typeof state.mastery[group]==='object'&&!Array.isArray(state.mastery[group])?state.mastery[group]:{};Object.keys(state.mastery[group]).forEach(k=>state.mastery[group][k]=Math.max(0,Math.floor(finiteNumber(state.mastery[group][k],0))))});
   state.masteryBackfillVersion=Math.max(0,Math.min(1,Math.floor(finiteNumber(state.masteryBackfillVersion,0))));
   state.masteryRewardClaims=state.masteryRewardClaims&&typeof state.masteryRewardClaims==='object'&&!Array.isArray(state.masteryRewardClaims)?state.masteryRewardClaims:{};normalizeBoolMap(state.masteryRewardClaims);
   state.masterySetClaims=state.masterySetClaims&&typeof state.masterySetClaims==='object'&&!Array.isArray(state.masterySetClaims)?state.masterySetClaims:{};normalizeBoolMap(state.masterySetClaims);
   {
     const rr=state.returnRewards=state.returnRewards&&typeof state.returnRewards==='object'&&!Array.isArray(state.returnRewards)?state.returnRewards:{};
     rr.activationDate=typeof rr.activationDate==='string'?rr.activationDate:'';rr.lastSeenDate=typeof rr.lastSeenDate==='string'?rr.lastSeenDate:'';rr.lastVisitDate=typeof rr.lastVisitDate==='string'?rr.lastVisitDate:'';rr.lastDailyClaimDate=typeof rr.lastDailyClaimDate==='string'?rr.lastDailyClaimDate:'';
     rr.dailyStreak=Math.max(0,Math.floor(finiteNumber(rr.dailyStreak,0)));rr.bestDailyStreak=Math.max(rr.dailyStreak,Math.floor(finiteNumber(rr.bestDailyStreak,0)));rr.clockHold=asBool(rr.clockHold);
     rr.daily=rr.daily&&typeof rr.daily==='object'&&!Array.isArray(rr.daily)?rr.daily:{};rr.weekly=rr.weekly&&typeof rr.weekly==='object'&&!Array.isArray(rr.weekly)?rr.weekly:{};rr.monthly=rr.monthly&&typeof rr.monthly==='object'&&!Array.isArray(rr.monthly)?rr.monthly:{};
     const validDayKey=k=>{if(!/^\d{4}-\d{2}-\d{2}$/.test(String(k||'')))return false;const [y,m,d]=k.split('-').map(Number),t=new Date(Date.UTC(y,m-1,d));return t.getUTCFullYear()===y&&t.getUTCMonth()===m-1&&t.getUTCDate()===d};
     const validMonthKey=k=>{if(!/^\d{4}-\d{2}$/.test(String(k||'')))return false;const[y,m]=k.split('-').map(Number);return y>=2000&&y<=9999&&m>=1&&m<=12};
     const serialKey=k=>{const[y,m,d]=k.split('-').map(Number);return Math.floor(Date.UTC(y,m-1,d)/86400000)};
     for(const key of ['activationDate','lastSeenDate','lastVisitDate','lastDailyClaimDate'])if(rr[key]&&!validDayKey(rr[key])){issues.push(`Invalid Return Reward date: ${key}`);rr[key]=''}
     // v43.1 used lastDailyClaimDate as both visit and automatic claim. Preserve that
     // day as an already-collected legacy Daily reward instead of paying it again.
     if(!rr.lastVisitDate&&rr.lastDailyClaimDate)rr.lastVisitDate=rr.lastDailyClaimDate;
     const fixRewardBundle=r=>{if(!r||typeof r!=='object'||Array.isArray(r))return null;const out={};for(const key of ['coins','gems','treats','windmills','gates','rescues']){const n=Math.max(0,Math.floor(finiteNumber(r[key],0)));if(n)out[key]=n}return out};
     Object.keys(rr.daily).forEach(day=>{
       const x=rr.daily[day];if(!validDayKey(day)||!x||typeof x!=='object'||Array.isArray(x)){delete rr.daily[day];issues.push('Invalid Daily Return Reward entry');return}
       x.streak=Math.max(1,Math.floor(finiteNumber(x.streak,1)));x.claimed=asBool(x.claimed);x.source=typeof x.source==='string'?x.source:'';x.claimedAt=Math.max(0,Math.floor(finiteNumber(x.claimedAt,0)));x.reward=fixRewardBundle(x.reward);
     });
     if(rr.lastDailyClaimDate&&!rr.daily[rr.lastDailyClaimDate])rr.daily[rr.lastDailyClaimDate]={streak:Math.max(1,rr.dailyStreak||1),claimed:true,source:'legacy',claimedAt:0};
     const fixSets=(sets,type)=>Object.keys(sets).forEach(k=>{
       const x=sets[k],periodValid=type==='weekly'?validDayKey(k):validMonthKey(k);
       if(!periodValid||!x||typeof x!=='object'||Array.isArray(x)){issues.push(`Invalid ${type} Return Reward set`);delete sets[k];return}
       x.claimed=x.claimed&&typeof x.claimed==='object'&&!Array.isArray(x.claimed)?x.claimed:{};x.visited=x.visited&&typeof x.visited==='object'&&!Array.isArray(x.visited)?x.visited:{};x.sources=x.sources&&typeof x.sources==='object'&&!Array.isArray(x.sources)?x.sources:{};x.claimedAt=x.claimedAt&&typeof x.claimedAt==='object'&&!Array.isArray(x.claimedAt)?x.claimedAt:{};x.rewards=x.rewards&&typeof x.rewards==='object'&&!Array.isArray(x.rewards)?x.rewards:{};
       for(const mapName of ['claimed','visited','sources','claimedAt','rewards'])Object.keys(x[mapName]).forEach(day=>{
         const inPeriod=validDayKey(day)&&(type==='monthly'?day.startsWith(k+'-'):(serialKey(day)-serialKey(k)>=0&&serialKey(day)-serialKey(k)<=6));
         if(!inPeriod){delete x[mapName][day];issues.push(`Invalid ${type} Return Reward ${mapName} entry`)}
       });
       normalizeBoolMap(x.claimed);normalizeBoolMap(x.visited);
       Object.keys(x.claimed).forEach(day=>{if(x.claimed[day]){x.visited[day]=true;if(!x.sources[day])x.sources[day]='legacy'}});
       Object.keys(x.sources).forEach(day=>x.sources[day]=typeof x.sources[day]==='string'?x.sources[day]:'');
       Object.keys(x.claimedAt).forEach(day=>x.claimedAt[day]=Math.max(0,Math.floor(finiteNumber(x.claimedAt[day],0))));
       Object.keys(x.rewards).forEach(day=>{const r=fixRewardBundle(x.rewards[day]);if(r)x.rewards[day]=r;else delete x.rewards[day]});
       x.completionReward=fixRewardBundle(x.completionReward);x.completionClaimed=asBool(x.completionClaimed);
     });
     fixSets(rr.weekly,'weekly');fixSets(rr.monthly,'monthly');
   }
   if(state._makeupProgressBackup&&typeof state._makeupProgressBackup==='object'&&!Array.isArray(state._makeupProgressBackup)){
     const b=state._makeupProgressBackup,preserveKeys=['returnDaysVisited','returnDaysClaimed','returnBestStreak','returnCoinsEarned','returnGemsEarned','returnTreatsEarned','weeklySlotsClaimed','monthlySlotsClaimed','makeupBoardsStarted','makeupBoardsCompleted','weeklyCompletionRewards','monthlyCompletionRewards','treatsFound','windmillsFound','magicGatesFound','rosieRescuesFound'],preserve={};
     preserveKeys.forEach(k=>preserve[k]=state.stats?.[k]);
     if(Number.isFinite(Number(b.bestStreak))&&Number(b.bestStreak)>=0)state.bestStreak=Math.floor(Number(b.bestStreak));
     if(b.stats&&typeof b.stats==='object'&&!Array.isArray(b.stats)){state.stats={...d.stats,...b.stats};preserveKeys.forEach(k=>{if(preserve[k]!==undefined)state.stats[k]=preserve[k]})}
     state._makeupProgressBackup=null;issues.push('Recovered interrupted Makeup Board progression');
   }else state._makeupProgressBackup=null;
   ...
   state.stats.returnDaysVisited=Math.max(state.stats.returnDaysVisited||0,state.stats.returnDaysClaimed||0);
