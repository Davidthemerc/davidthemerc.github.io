...
DSH.Farm=(()=>{
 ...
 function announceSeasonResult(result){
   ...
   if(result.yearResult?.complete){
     const y=result.yearResult,m=y.masteryUp?` • 🏵️ Stewardship Tier ${y.masteryTier} unlocked (+${y.masteryTier*2}% crop & Order coins)`:''; 
     onReward?.(`🌎 Farm Year ${y.year} Complete!`,`${result.info.icon} ${result.info.name} Basket: ${seasonRewardText(result.reward)}${extra} • 🌎 Finale: ${seasonRewardText(y.reward)} • 🔥 Streak ${y.streak}${m}`,'🌎');
   }else onReward?.(`${result.info.icon} ${result.info.name} Basket Complete!`,`${seasonRewardText(result.reward)}${extra}`,result.info.icon);
 }
 ...
 function petRosie(){
   ...
   state.lastPetAt=now;state.stats.petRosieCount=(state.stats.petRosieCount||0)+1;state.mastery=state.mastery||{crops:{},specials:{},powers:{},rosie:{}};state.mastery.rosie=state.mastery.rosie||{};state.mastery.rosie.pet=(state.mastery.rosie.pet||0)+1;state.collection=state.collection||{specials:{},crops:{},regions:{0:true},powers:{},rosie:{}};state.collection.rosie.petted=true;
 ...
 function feedTreat(){
   ...
   state.rosieTreats--;state.stats.treatsFed=(state.stats.treatsFed||0)+1;state.mastery=state.mastery||{crops:{},specials:{},powers:{},rosie:{}};state.mastery.rosie=state.mastery.rosie||{};state.mastery.rosie.treat=(state.mastery.rosie.treat||0)+1;
 ...
 function showFarmViewportFeedback(icon,title,detail=''){
   const old=document.querySelector('.farmViewportFeedback');if(old)old.remove();
   const box=document.createElement('div');box.className='farmViewportFeedback';
   box.innerHTML=`<span>${icon}</span><div><b>${title}</b><small>${detail}</small></div>`;
   document.body.appendChild(box);requestAnimationFrame(()=>box.classList.add('show'));
   setTimeout(()=>{box.classList.remove('show');setTimeout(()=>box.remove(),220)},2100);
 }
 ...
 function plant(type){
   ...
   showFarmViewportFeedback(d.icon,`${d.name} planted!`,spaceText);
 ...
 function harvest(i,opts={}){
   ...
   state.mastery=state.mastery||{crops:{},specials:{},powers:{},rosie:{}};state.mastery.crops=state.mastery.crops||{};state.mastery.crops[p.type]=(state.mastery.crops[p.type]||0)+1;
 ...
 function claimAdventure(forceRare=false){
   ...
   state.stats.rosieAdventuresCompleted++;state.mastery=state.mastery||{crops:{},specials:{},powers:{},rosie:{}};state.mastery.rosie=state.mastery.rosie||{};state.mastery.rosie.adventure=(state.mastery.rosie.adventure||0)+1;state.stats.rosieAdventureCoins+=coins;state.stats.rosieAdventureGems+=gems;if(power)state.stats.rosieAdventurePowers++;
 ...
 function render(){
   ...
   if(seasonInfo){const fy=DSH.Seasons.farmYearStatus(state,seasonInfo.year);setText('farmSeasonName',`${seasonInfo.icon} ${seasonInfo.name} • Year ${seasonInfo.year}`);setText('farmSeasonEffect',`${DSH.Seasons.variantText(seasonInfo)} • ${DSH.Seasons.effectText(seasonInfo)} • 🌎 Year ${fy.count}/4${fy.masteryTier?` • 🏵️ Tier ${fy.masteryTier}`:''}`);setText('farmSeasonPoints',ss.claimed?'Basket complete ✓':`${ss.points||0}/${seasonInfo.goal}`);const sb=document.getElementById('farmSeasonBar');if(sb)sb.style.width=`${Math.min(100,(ss.points||0)/seasonInfo.goal*100)}%`;const sc=document.getElementById('farmSeasonCard');if(sc){sc.dataset.season=seasonInfo.key.split('-').slice(1).join('-')}}
