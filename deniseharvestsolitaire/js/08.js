...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 function chapterForLevel(level){
   const chapters=DSH.Config.chapters,idx=Math.floor((Math.max(1,level)-1)/10),horizon=DSH.Config.milestones.campaignHorizon||1000;
   if(idx<chapters.length)return{...chapters[idx],index:idx};
   if(level>horizon){
     const endlessIdx=Math.floor((level-horizon-1)/10),start=horizon+1+endlessIdx*10;
     return{start,end:start+9,name:`Endless Harvest ${endlessIdx+1}`,icon:'∞',tone:'#7b8190',index:idx,endless:true};
   }
   const start=idx*10+1,century=Math.floor((start-1)/100),part=((idx%10)+1);
   const eras=[
     {name:'Second Spring',icon:'🌱',tone:'#79a95f'},
     {name:'Orchard Trails',icon:'🍎',tone:'#b88443'},
     {name:'Lantern Harvest',icon:'🏮',tone:'#b57655'},
     {name:'Golden Seasons',icon:'🌾',tone:'#b99a45'},
     {name:'Far Meadow',icon:'🌿',tone:'#6e9a68'},
     {name:'Moon Valley',icon:'🌙',tone:'#7270a1'},
     {name:'Frost & Bloom',icon:'❄️',tone:'#75a4b4'},
     {name:"Rosie's Road",icon:'🐾',tone:'#a77c72'},
     {name:'Ultra Harvest',icon:'🏆',tone:'#b78a35'}
   ];
   const era=eras[Math.min(eras.length-1,Math.max(0,century-1))]||eras[eras.length-1];
   return{start,end:start+9,name:`${era.name} ${part}`,icon:era.icon,tone:era.tone,index:idx};
 }
 ...
 function milestoneName(level){
   const ch=chapterForLevel(level),names={
     10:'Meadow Gate',20:'Sunflower Festival',30:'Rosie Creek Crossing',40:'Golden Orchard Fair',50:'Harvest Ridge Summit',
     60:'Autumn Moon',70:'Moonlit Barn',80:'Winter Garden Finale',90:'Spring Jubilee',100:'Centennial Harvest',
     200:'Second Spring Jubilee',250:'Quarter Harvest',300:'Orchard Trails Homecoming',400:'Lantern Harvest Gala',500:'Golden Seasons Summit',
     600:'Far Meadow Fair',700:'Moon Valley Gathering',750:"Rosie's Long Road",800:'Frost & Bloom Festival',900:"Rosie's Road Homecoming",1000:'Ultra Harvest Finale'
   };
 ...
 function renderCampaignHorizon(){
   const horizon=DSH.Config.milestones.campaignHorizon||1000,level=Math.max(1,state.level||1),completed=Math.max(0,Math.min(horizon,level-1));
   const text=document.getElementById('campaignHorizonText'),bar=document.getElementById('campaignHorizonBar'),badge=document.getElementById('campaignHorizonBadge');
   if(level<=horizon){
     if(text)text.textContent=`Authored campaign • ${completed}/${horizon} levels complete`;
     if(badge)badge.textContent=level===horizon?'🏆 ULTRA HARVEST FINALE':`${Math.min(99,Math.floor(completed/horizon*100))}%`;
     if(bar)bar.style.width=`${completed/horizon*100}%`;
   }else{
     if(text)text.textContent=`Ultra Harvest complete • Endless Harvest Level ${level}`;
     if(badge)badge.textContent='∞ ENDLESS';
     if(bar)bar.style.width='100%';
   }
 }
 function renderSeasonUI(){
   const inf=DSH.Seasons?.ensure(state,state.level);if(!inf)return;const ss=state.seasonState||{},reward=DSH.Seasons.rewardFor(inf),pct=Math.min(100,(ss.points||0)/inf.goal*100),key=inf.key.split('-').slice(1).join('-');
   setTextIfPresent('seasonIcon',inf.icon);setTextIfPresent('seasonName',`${inf.name} • Year ${inf.year}`);setTextIfPresent('seasonLevels',`Levels ${inf.start}–${inf.end} • ${DSH.Seasons.effectText(inf)}`);setTextIfPresent('seasonVariant',DSH.Seasons.variantText(inf));setTextIfPresent('seasonPoints',ss.claimed?'COMPLETE ✓':`${ss.points||0}/${inf.goal}`);setTextIfPresent('seasonRewardPreview',`Solitaire + Farm + Rosie fill the Basket • Reward: ${DSH.Seasons.rewardText(reward)}`);
   const fy=DSH.Seasons.farmYearStatus(state,inf.year),mastery=fy.masteryTier?` • 🏵️ Tier ${fy.masteryTier} (+${fy.masteryTier*2}% Farm coins)`:'';
   setTextIfPresent('farmYearProgressLine',fy.record?.complete?`🌎 Farm Year ${inf.year} complete • Streak ${fy.record.streak}${mastery}`:`🌎 Farm Year ${inf.year}: ${fy.count}/4 Baskets • Streak ${fy.streak}${mastery}`);
   const bar=document.getElementById('seasonProgressBar');if(bar)bar.style.width=pct+'%';const card=document.getElementById('seasonCard');if(card)card.dataset.season=key;
 }
 ...
 function dateKeyFromSerial(serial){const d=new Date(serial*86400000);return`${d.getUTCFullYear()}-${String(d.getUTCMonth()+1).padStart(2,'0')}-${String(d.getUTCDate()).padStart(2,'0')}`}
 function addDaysKey(key,days){return dateKeyFromSerial(dateSerial(key)+days)}
 function weekStartKey(key){const serial=dateSerial(key),dow=new Date(serial*86400000).getUTCDay(),offset=(dow+6)%7;return dateKeyFromSerial(serial-offset)}
 function monthKeyForDate(key){return key.slice(0,7)}
 function monthStartKey(key){return `${monthKeyForDate(key)}-01`}
 function monthDays(key){const[y,m]=monthKeyForDate(key).split('-').map(Number);return new Date(Date.UTC(y,m,0)).getUTCDate()}
 function returnRewardText(r={}){
   const parts=[];if(r.coins)parts.push(`${r.coins} coins`);if(r.gems)parts.push(`${r.gems} 💎`);if(r.treats)parts.push(`${r.treats} 🦴`);
   if(r.windmills)parts.push(`${r.windmills} Windmill${r.windmills===1?'':'s'} 🌬️`);if(r.gates)parts.push(`${r.gates} Magic Gate${r.gates===1?'':'s'} 🚪`);if(r.rescues)parts.push(`${r.rescues} Rosie Rescue${r.rescues===1?'':'s'} 🐾`);
   return parts.join(' • ')||'Reward claimed';
 }
 function grantRewardBundle(bundle={},kind=''){
   const r={coins:Math.max(0,Math.floor(Number(bundle.coins)||0)),gems:Math.max(0,Math.floor(Number(bundle.gems)||0)),treats:Math.max(0,Math.floor(Number(bundle.treats)||0)),windmills:Math.max(0,Math.floor(Number(bundle.windmills)||0)),gates:Math.max(0,Math.floor(Number(bundle.gates)||0)),rescues:Math.max(0,Math.floor(Number(bundle.rescues)||0))};
   state.coins+=r.coins;state.gems+=r.gems;state.rosieTreats=(state.rosieTreats||0)+r.treats;state.windmills=(state.windmills||0)+r.windmills;state.magicGates=(state.magicGates||0)+r.gates;state.rosieRescues=(state.rosieRescues||0)+r.rescues;
   if(r.treats)state.stats.treatsFound=(state.stats.treatsFound||0)+r.treats;if(r.windmills)state.stats.windmillsFound=(state.stats.windmillsFound||0)+r.windmills;if(r.gates)state.stats.magicGatesFound=(state.stats.magicGatesFound||0)+r.gates;if(r.rescues)state.stats.rosieRescuesFound=(state.stats.rosieRescuesFound||0)+r.rescues;
   if(kind==='return'){state.stats.returnCoinsEarned=(state.stats.returnCoinsEarned||0)+r.coins;state.stats.returnGemsEarned=(state.stats.returnGemsEarned||0)+r.gems;state.stats.returnTreatsEarned=(state.stats.returnTreatsEarned||0)+r.treats}
   if(kind==='mastery'){state.stats.masteryRewardCoins=(state.stats.masteryRewardCoins||0)+r.coins;state.stats.masteryRewardGems=(state.stats.masteryRewardGems||0)+r.gems}
   return r;
 }
 function resolvePowerReward(bundle,key){
   const out={...bundle};if(out.power){const p=['windmills','gates','rescues'][hashString(`ReturnPower:${key}`)%3];out[p]=(out[p]||0)+out.power;delete out.power}return out;
 }
 function loyaltyInfo(visits=state.stats.returnDaysVisited||0){
   const C=DSH.Config.returnRewards.loyalty||{},v=Math.max(1,Math.floor(Number(visits)||1)),days=Math.max(1,C.daysPerRank||7),rank=1+Math.floor((v-1)/days),monthBlocks=Math.floor(v/30),mult=1+(rank-1)*(C.rankBonus||.10)+monthBlocks*(C.monthBonus||.25);
   return{visits:v,rank,mult:Math.round(mult*100)/100,nextRankAt:rank*days+1};
 }
 function scaledReturnCoins(base,visits){const S=DSH.Config.returnRewards.economyScale||45,L=loyaltyInfo(visits).mult;return Math.max(0,Math.round(base*S*L/50)*50)}
 function scaledReturnGems(base,visits){return Math.max(0,Math.round(base*loyaltyInfo(visits).mult))}
 function addReturnPowers(bundle,count,key){
   const n=Math.max(0,Math.floor(count||0)),types=['windmills','gates','rescues'],offset=hashString(`ReturnMix:${key}`)%types.length;
   for(let i=0;i<n;i++){const k=types[(offset+i)%types.length];bundle[k]=(bundle[k]||0)+1}return bundle;
 }
 function dailyReturnReward(streak,key,visits=state.stats.returnDaysVisited||1){
   const C=DSH.Config.returnRewards.daily,step=Math.min(C.coinStepCap||3650,Math.max(0,streak-1)),r={coins:scaledReturnCoins((C.baseCoins||80)+step*(C.coinStep||20),visits),gems:scaledReturnGems(C.baseGems||5,visits)};
   if(streak%30===0){r.gems+=scaledReturnGems(225,visits);r.treats=(r.treats||0)+8;addReturnPowers(r,12,`daily30:${key}`)}
   else if(streak%14===0){r.gems+=scaledReturnGems(135,visits);r.treats=(r.treats||0)+5;addReturnPowers(r,6,`daily14:${key}`)}
   else if(streak%7===0){r.gems+=scaledReturnGems(90,visits);r.treats=(r.treats||0)+3;addReturnPowers(r,3,`daily7:${key}`)}
   else if(streak%3===0)r.gems+=scaledReturnGems(45,visits);
   if(streak%2===0)r.treats=(r.treats||0)+1;if(streak%5===0)r.treats=(r.treats||0)+2;if(streak%3===0)addReturnPowers(r,1,`daily3:${key}`);
   return r;
 }
 function weeklySlotReward(dateKey,visits=state.stats.returnDaysVisited||1){
   const idx=dateSerial(dateKey)-dateSerial(weekStartKey(dateKey)),baseCoins=[60,75,90,105,120,135,150][idx]||60,gemBase=[5,8,12,18,25,35,50][idx]||5,treats=[0,1,0,2,1,3,4][idx]||0,powers=[0,0,1,1,1,2,3][idx]||0,r={coins:scaledReturnCoins(baseCoins,visits),gems:scaledReturnGems(gemBase,visits)};
   if(treats)r.treats=Math.max(1,Math.round(treats*loyaltyInfo(visits).mult));addReturnPowers(r,powers,`weekly:${dateKey}`);return r;
 }
 function monthlySlotReward(dateKey,visits=state.stats.returnDaysVisited||1){
   const day=Number(dateKey.slice(8,10)),L=loyaltyInfo(visits).mult,r={coins:scaledReturnCoins(30+day*3,visits),gems:scaledReturnGems(5+Math.floor(day/4)*3,visits)};
   if(day%3===0)r.treats=(r.treats||0)+Math.max(1,Math.round(L));if(day%10===0)r.treats=(r.treats||0)+Math.max(2,Math.round(3*L));
   if(day%5===0)addReturnPowers(r,1,`monthly5:${dateKey}`);if(day%7===0){r.gems+=scaledReturnGems(20,visits);addReturnPowers(r,2,`monthly7:${dateKey}`)}
   if(day===28||day===29||day===30||day===31){r.gems+=scaledReturnGems(25,visits);addReturnPowers(r,1,`monthend:${dateKey}`)}
   return r;
 }
 function legacyDailyReturnReward(streak,key){
   const step=Math.min(14,Math.max(0,streak-1)),r={coins:80+step*20};if(streak%30===0)r.gems=5;else if(streak%14===0)r.gems=3;else if(streak%7===0)r.gems=2;else if(streak%3===0)r.gems=1;if(streak%5===0)r.treats=1;if(streak%7===0)r.power=1;return resolvePowerReward(r,`daily:${key}:${streak}`);
 }
 function legacyWeeklySlotReward(dateKey){const idx=dateSerial(dateKey)-dateSerial(weekStartKey(dateKey)),coins=[60,75,90,105,120,135,150][idx]||60,r={coins};if(idx===3)r.treats=1;if(idx===4)r.gems=1;return r}
 function legacyMonthlySlotReward(dateKey){const day=Number(dateKey.slice(8,10)),r={coins:30+day*3};if(day%7===0)r.gems=1;if(day%10===0)r.treats=1;return r}
 function cleanRewardBundle(r){const out={};for(const k of ['coins','gems','treats','windmills','gates','rescues']){const n=Math.max(0,Math.floor(Number(r?.[k])||0));if(n)out[k]=n}return out}
 let returnSetsCacheDate='',returnSetsCacheRef=null;
 function validReturnDateKey(key){if(!/^\d{4}-\d{2}-\d{2}$/.test(String(key||'')))return false;const serial=dateSerial(key);return Number.isFinite(serial)&&dateKeyFromSerial(serial)===key}
 function returnState(){
   const rr=state.returnRewards=state.returnRewards&&typeof state.returnRewards==='object'?state.returnRewards:{};
   rr.activationDate=validReturnDateKey(rr.activationDate)?rr.activationDate:'';rr.lastSeenDate=validReturnDateKey(rr.lastSeenDate)?rr.lastSeenDate:'';
   rr.lastVisitDate=validReturnDateKey(rr.lastVisitDate)?rr.lastVisitDate:(validReturnDateKey(rr.lastDailyClaimDate)?rr.lastDailyClaimDate:'');
   rr.lastDailyClaimDate=validReturnDateKey(rr.lastDailyClaimDate)?rr.lastDailyClaimDate:'';
   rr.dailyStreak=Math.max(0,Number(rr.dailyStreak)||0);rr.bestDailyStreak=Math.max(rr.dailyStreak,Number(rr.bestDailyStreak)||0);
   rr.daily=rr.daily&&typeof rr.daily==='object'?rr.daily:{};rr.weekly=rr.weekly&&typeof rr.weekly==='object'?rr.weekly:{};rr.monthly=rr.monthly&&typeof rr.monthly==='object'?rr.monthly:{};rr.clockHold=!!rr.clockHold;return rr;
 }
 function returnPeriodKey(type,dateKey){return type==='weekly'?weekStartKey(dateKey):monthKeyForDate(dateKey)}
 function ensureReturnSet(type,key){
   const rr=returnState(),sets=rr[type],x=sets[key]=sets[key]&&typeof sets[key]==='object'?sets[key]:{key,claimed:{},visited:{},sources:{},claimedAt:{},rewards:{},completionClaimed:false,completionReward:null};
   x.key=key;x.claimed=x.claimed&&typeof x.claimed==='object'?x.claimed:{};x.visited=x.visited&&typeof x.visited==='object'?x.visited:{};x.sources=x.sources&&typeof x.sources==='object'?x.sources:{};x.claimedAt=x.claimedAt&&typeof x.claimedAt==='object'?x.claimedAt:{};x.rewards=x.rewards&&typeof x.rewards==='object'?x.rewards:{};x.completionClaimed=!!x.completionClaimed;
   Object.keys(x.claimed).forEach(day=>{if(x.claimed[day])x.visited[day]=true});return x;
 }
 function returnPeriodDates(type,key){
   if(type==='weekly')return Array.from({length:7},(_,i)=>addDaysKey(key,i));
   const n=monthDays(`${key}-01`);return Array.from({length:n},(_,i)=>`${key}-${String(i+1).padStart(2,'0')}`);
 }
 function returnEligibleDates(type,key){
   const rr=returnState(),activation=rr.activationDate||localDateKey();return returnPeriodDates(type,key).filter(k=>dateSerial(k)>=dateSerial(activation));
 }
 function trimReturnSets(){
   const rr=returnState(),C=DSH.Config.returnRewards;
   Object.keys(rr.daily).sort((a,b)=>dateSerial(b)-dateSerial(a)).slice(C.dailyHistory||35).forEach(k=>delete rr.daily[k]);
   Object.keys(rr.weekly).sort((a,b)=>dateSerial(b)-dateSerial(a)).slice(C.weeklyHistory||8).forEach(k=>delete rr.weekly[k]);
   Object.keys(rr.monthly).sort().reverse().slice(C.monthlyHistory||4).forEach(k=>delete rr.monthly[k]);
 }
 function ensureReturnSets(today=localDateKey()){
   const rr=returnState();if(!rr.activationDate)rr.activationDate=today;
   if(returnSetsCacheDate===today&&returnSetsCacheRef===rr)return rr;
   const last=Math.max(dateSerial(rr.activationDate),dateSerial(today)-124);
   for(let serial=last;serial<=dateSerial(today);serial++){const k=dateKeyFromSerial(serial);ensureReturnSet('weekly',returnPeriodKey('weekly',k));ensureReturnSet('monthly',returnPeriodKey('monthly',k))}
   trimReturnSets();returnSetsCacheDate=today;returnSetsCacheRef=rr;return rr;
 }
 function dailyRewardFor(dateKey){
   const rr=returnState(),entry=rr.daily?.[dateKey];if(!entry)return null;if(entry.reward)return cleanRewardBundle(entry.reward);if(entry.claimed)return legacyDailyReturnReward(entry.streak||1,dateKey);
   entry.reward=cleanRewardBundle(dailyReturnReward(entry.streak||1,dateKey,state.stats.returnDaysVisited||1));return entry.reward;
 }
 function slotRewardFor(type,dateKey){
   const set=ensureReturnSet(type,returnPeriodKey(type,dateKey));if(set.rewards?.[dateKey])return cleanRewardBundle(set.rewards[dateKey]);
   if(set.claimed?.[dateKey])return type==='weekly'?legacyWeeklySlotReward(dateKey):legacyMonthlySlotReward(dateKey);
   if(set.visited?.[dateKey]){const reward=type==='weekly'?weeklySlotReward(dateKey):monthlySlotReward(dateKey);set.rewards[dateKey]=cleanRewardBundle(reward);return set.rewards[dateKey]}
   return type==='weekly'?weeklySlotReward(dateKey):monthlySlotReward(dateKey);
 }
 function returnSetPartial(type,key){return returnEligibleDates(type,key).length<returnPeriodDates(type,key).length}
 function returnSetComplete(type,key){
   const set=ensureReturnSet(type,key),eligible=returnEligibleDates(type,key),all=returnPeriodDates(type,key);return eligible.length===all.length&&all.length>0&&eligible.every(k=>!!set.claimed[k]);
 }
 function markReturnVisit(type,dateKey,visits=state.stats.returnDaysVisited||1){
   const set=ensureReturnSet(type,returnPeriodKey(type,dateKey));set.visited[dateKey]=true;if(!set.rewards[dateKey])set.rewards[dateKey]=cleanRewardBundle(type==='weekly'?weeklySlotReward(dateKey,visits):monthlySlotReward(dateKey,visits));return set;
 }
 function returnSlotMissing(type,dateKey,today=localDateKey()){
   const rr=returnState();if(!rr.activationDate||dateSerial(dateKey)<dateSerial(rr.activationDate)||dateSerial(dateKey)>=dateSerial(today))return false;
   const set=rr[type]?.[returnPeriodKey(type,dateKey)];return !!set&&!set.claimed?.[dateKey]&&!set.visited?.[dateKey];
 }
 function returnSlotStatus(type,dateKey,today=localDateKey()){
   const rr=returnState(),eligible=!!rr.activationDate&&dateSerial(dateKey)>=dateSerial(rr.activationDate),set=rr[type]?.[returnPeriodKey(type,dateKey)],claimed=!!set?.claimed?.[dateKey],visited=!!set?.visited?.[dateKey],past=dateSerial(dateKey)<dateSerial(today),future=dateSerial(dateKey)>dateSerial(today);
   return{eligible,set,claimed,visited,past,future,missing:eligible&&past&&!visited&&!claimed,ready:eligible&&visited&&!claimed,source:set?.sources?.[dateKey]||'',claimedAt:set?.claimedAt?.[dateKey]||0};
 }
 function claimDailyReturn(dateKey=localDateKey()){
   const rr=returnState(),entry=rr.daily?.[dateKey];if(!entry||entry.claimed||rr.clockHold)return false;
   const reward=dailyRewardFor(dateKey);entry.claimed=true;entry.source='manual';entry.claimedAt=Date.now();grantRewardBundle(reward,'return');
   if(!rr.lastDailyClaimDate||dateSerial(dateKey)>dateSerial(rr.lastDailyClaimDate))rr.lastDailyClaimDate=dateKey;
   state.stats.returnDaysClaimed=(state.stats.returnDaysClaimed||0)+1;DSH.Save.save(state);renderAllBalances();renderReturnRewardsUI();
   showRewardToast(`🎁 Daily Reward • Day ${entry.streak}`,returnRewardText(reward),'🎁');return true;
 }
 function claimReturnPeriodSlot(type,dateKey,source='manual'){
   const rr=returnState();if(rr.clockHold||dateSerial(dateKey)<dateSerial(rr.activationDate||dateKey))return null;
   const key=returnPeriodKey(type,dateKey),set=ensureReturnSet(type,key);if(set.claimed[dateKey])return null;
   if(source!=='makeup'&&!set.visited[dateKey])return null;
   if(source==='makeup'&&!set.rewards[dateKey])set.rewards[dateKey]=cleanRewardBundle(type==='weekly'?weeklySlotReward(dateKey):monthlySlotReward(dateKey));
   const bundle=slotRewardFor(type,dateKey);set.visited[dateKey]=true;set.claimed[dateKey]=true;set.sources[dateKey]=source;set.claimedAt[dateKey]=Date.now();grantRewardBundle(bundle,'return');
   if(type==='weekly')state.stats.weeklySlotsClaimed=(state.stats.weeklySlotsClaimed||0)+1;else state.stats.monthlySlotsClaimed=(state.stats.monthlySlotsClaimed||0)+1;
   if(returnSetComplete(type,key)&&!set.completionReward)set.completionReward=cleanRewardBundle(completionRewardBase(type,state.stats.returnDaysVisited||1,key));
   if(source==='manual'){DSH.Save.save(state);renderAllBalances();renderReturnRewardsUI();showRewardToast(type==='weekly'?'📆 Weekly Reward Collected':'🗓️ Monthly Reward Collected',`${formatReturnDate(dateKey,{weekday:true})} • ${returnRewardText(bundle)}`,type==='weekly'?'📆':'🗓️')}
   return{type,key,dateKey,bundle,source};
 }
 function returnMissingDates(today=localDateKey()){
   const rr=ensureReturnSets(today),out=new Map();
   for(const type of ['weekly','monthly'])for(const [key,set] of Object.entries(rr[type]))for(const dateKey of returnEligibleDates(type,key)){
     if(dateSerial(dateKey)>=dateSerial(today)||set.claimed?.[dateKey]||set.visited?.[dateKey])continue;
     if(!out.has(dateKey))out.set(dateKey,[]);out.get(dateKey).push(type);
   }
   return[...out.entries()].map(([dateKey,types])=>({dateKey,types})).sort((a,b)=>dateSerial(b.dateKey)-dateSerial(a.dateKey));
 }
 function returnPendingClaims(today=localDateKey()){
   const rr=ensureReturnSets(today),daily=Object.entries(rr.daily).filter(([k,x])=>dateSerial(k)<=dateSerial(today)&&x&&!x.claimed).map(([dateKey])=>({type:'daily',dateKey}));
   const slots=[];for(const type of ['weekly','monthly'])for(const [key,set] of Object.entries(rr[type]))for(const dateKey of returnEligibleDates(type,key))if(set.visited?.[dateKey]&&!set.claimed?.[dateKey]&&dateSerial(dateKey)<=dateSerial(today))slots.push({type,dateKey});
   return[...daily,...slots].sort((a,b)=>dateSerial(b.dateKey)-dateSerial(a.dateKey));
 }
 function processReturnDay(today=localDateKey(),announce=true){
   const rr=ensureReturnSets(today),ts=dateSerial(today);
   if(rr.lastSeenDate&&ts<dateSerial(rr.lastSeenDate)){rr.clockHold=true;DSH.Save.save(state,{skipBackup:true});return{visited:false,clockHold:true}}
   rr.clockHold=false;if(!rr.lastSeenDate||ts>dateSerial(rr.lastSeenDate))rr.lastSeenDate=today;
   if(rr.lastVisitDate===today)return{visited:false,clockHold:false,streak:rr.dailyStreak};
   const gap=rr.lastVisitDate?ts-dateSerial(rr.lastVisitDate):999;rr.dailyStreak=gap===1?rr.dailyStreak+1:1;rr.bestDailyStreak=Math.max(rr.bestDailyStreak,rr.dailyStreak);rr.lastVisitDate=today;
   state.stats.returnDaysVisited=(state.stats.returnDaysVisited||0)+1;const visits=state.stats.returnDaysVisited;
   rr.daily[today]=rr.daily[today]&&typeof rr.daily[today]==='object'?rr.daily[today]:{streak:rr.dailyStreak,claimed:false,source:'',claimedAt:0,reward:null};rr.daily[today].streak=rr.dailyStreak;if(!rr.daily[today].reward)rr.daily[today].reward=cleanRewardBundle(dailyReturnReward(rr.dailyStreak,today,visits));
   markReturnVisit('weekly',today,visits);markReturnVisit('monthly',today,visits);
   state.stats.returnBestStreak=Math.max(state.stats.returnBestStreak||0,rr.bestDailyStreak);DSH.Progress.addHappiness(state,1);trimReturnSets();DSH.Save.save(state);
   if(announce){const L=loyaltyInfo(visits);setTimeout(()=>showRewardToast(`🎁 Return Day ${rr.dailyStreak}`,`Three rewards are ready • Loyalty Rank ${L.rank} • ${Math.round((DSH.Config.returnRewards.economyScale||45)*L.mult)}× coin value`,'🎁'),120)}
   return{visited:true,streak:rr.dailyStreak,loyalty:loyaltyInfo(visits)};
 }

 function completionRewardBase(type,visits=state.stats.returnDaysVisited||1,key=''){
   const base=type==='weekly'?DSH.Config.returnRewards.weeklyCompletion:DSH.Config.returnRewards.monthlyCompletion,L=loyaltyInfo(visits).mult,r={coins:Math.round((base.coins||0)*L/100)*100,gems:Math.round((base.gems||0)*L),treats:Math.max(0,Math.round((base.treats||0)*Math.sqrt(L))),windmills:Math.max(0,Math.round((base.windmills||0)*Math.sqrt(L))),gates:Math.max(0,Math.round((base.gates||0)*Math.sqrt(L))),rescues:Math.max(0,Math.round((base.rescues||0)*Math.sqrt(L)))};return cleanRewardBundle(r);
 }
 function completionReward(type,key){
   const set=returnState()[type]?.[key];if(set?.completionReward)return cleanRewardBundle(set.completionReward);
   return completionRewardBase(type,state.stats.returnDaysVisited||1,key);
 }
 function claimReturnCompletion(type,key){
   const rr=ensureReturnSets(),set=rr[type]?.[key];if(rr.clockHold||!set||set.completionClaimed||!returnSetComplete(type,key))return false;
   const reward=completionReward(type,key);set.completionClaimed=true;grantRewardBundle(reward,'return');
   if(type==='weekly')state.stats.weeklyCompletionRewards=(state.stats.weeklyCompletionRewards||0)+1;else state.stats.monthlyCompletionRewards=(state.stats.monthlyCompletionRewards||0)+1;
   DSH.Save.save(state);renderAllBalances();renderReturnRewardsUI();showRewardToast(type==='weekly'?'📆 Weekly Set Complete!':'🗓️ Monthly Set Complete!',returnRewardText(reward),type==='weekly'?'📆':'🗓️');return true;
 }
 function returnCompletionActions(){
   const rr=returnState(),out=[];for(const type of ['weekly','monthly'])for(const key of Object.keys(rr[type])){const set=rr[type][key];if(!set.completionClaimed&&returnSetComplete(type,key))out.push({type,key,reward:completionReward(type,key)})}return out;
 }
 function formatReturnDate(key,opts={}){const[y,m,d]=key.split('-').map(Number);return new Date(y,m-1,d).toLocaleDateString(undefined,{month:'short',day:'numeric',...(opts.weekday?{weekday:'short'}:{})})}
 function makeupDescriptor(dateKey){
   const C=DSH.Config.returnRewards.makeup,seed=hashString(`DeniseHarvestMakeup:${dateKey}`),level=C.minLevel+(seed%C.levelRange);return{dateKey,seed,level,label:formatReturnDate(dateKey,{weekday:true})};
 }
 function makeupNeeded(dateKey,today=localDateKey()){return returnMissingDates(today).some(x=>x.dateKey===dateKey)}
 ...
 function snapshot(){
   history.push(JSON.stringify({game,state:{
     ...
     magicGates:state.magicGates,rosieRescues:state.rosieRescues,rosieTreats:state.rosieTreats,rosieHappiness:state.rosieHappiness,
     lifetime:state.lifetime,_walletSnapshot:state._walletSnapshot,mastery:state.mastery,masteryRewardClaims:state.masteryRewardClaims,masterySetClaims:state.masterySetClaims,collection:state.collection,
 ...
 function handleHarvestChain(c){
   ...
   const n=game.harvestChainCount,S=DSH.Config.specialCards,isolated=game.dailyMode||game.makeupMode,seasonMult=isolated?1:(game.handSeasonFx?.harvestChainCoin||1),c2=Math.round(S.harvestChain2Coins*seasonMult),c3=Math.round(S.harvestChain3Coins*seasonMult);
   if(n===2){if(isolated){game.score+=c2*10;showRewardToast('Harvest Chain!',`2 linked cards • +${c2*10} challenge score`,'🌾')}else{state.coins+=c2;state.stats.harvestChainCoins=(state.stats.harvestChainCoins||0)+c2;showRewardToast('Harvest Chain!',`2 linked cards • +${c2} coins`,'🌾')}}
   if(n>=3){if(isolated){game.score+=c3*10;showRewardToast('Full Harvest Chain!',`3 linked cards • +${c3*10} challenge score • +1 streak`,'🌾')}else{state.coins+=c3;state.stats.harvestChainCoins=(state.stats.harvestChainCoins||0)+c3;showRewardToast('Full Harvest Chain!',`3 linked cards • +${c3} coins • +1 streak`,'🌾')}state.stats.harvestChainsCompleted=(state.stats.harvestChainsCompleted||0)+1;game.streak++;if(game.streak>=DSH.Config.specialCards.chainRequiredStreak)game.chainUnlocked=true}
 }
 ...
 function showLevelPreview(built){
   ...
   setTextIfPresent('levelPreviewTitle',game.makeupMode?`Makeup Board • ${formatReturnDate(game.makeupDate,{weekday:true})}`:game.dailyMode?`Daily Challenge • ${built.name}`:`Level ${state.level} • ${built.name}`);
   setTextIfPresent('levelPreviewTags',built.specialLabels?.length?built.specialLabels.join(' • '):'Classic field');
   const isolated=game.dailyMode||game.makeupMode,seasonStock=isolated?0:(game.handSeasonFx?.extraStock||0),stockNote=(built.obstacleStockBonus||0)+(game.dailyMode?(DSH.Config.dailyChallenge.extraStock||0):0)+(game.makeupMode?(DSH.Config.returnRewards.makeup.extraStock||0):0)+seasonStock;
   const stockLabel=game.makeupMode?'Makeup cushion + obstacle compensation':game.dailyMode?'Daily cushion + obstacle compensation':seasonStock?'Season + obstacle compensation':'Obstacle compensation';
   setTextIfPresent('levelPreviewStock',stockNote?`${stockLabel}: +${stockNote} stock draw${stockNote===1?'':'s'}`:'');
 ...
 function applySpecialClear(c,grantRewards=true){
   ...
   if(!game.makeupMode){state.mastery=state.mastery||{crops:{},specials:{},powers:{},rosie:{}};state.mastery.specials=state.mastery.specials||{};state.mastery.specials[c.special]=(state.mastery.specials[c.special]||0)+1}
   ...
   if(c.special==='flower'){
     ...
     if(grantRewards){
       if(game.dailyMode||game.makeupMode){game.score+=250;showRewardToast(game.makeupMode?'Makeup Golden Card!':'Daily Golden Card!','+250 challenge score','💰')}
       else{
     ...
     if(grantRewards){
       ...
       if(game.dailyMode||game.makeupMode){game.score+=300;showRewardToast(game.makeupMode?'Makeup Rainbow Card!':'Daily Rainbow Card!',`+300 score • +${S.rainbowBonusDraws} draw`,'🌈')}
 ...
 const MISSION_DEFS=[
   ...
   {id:'streak6',icon:'⚡',name:'Hot Hand',minLevel:40,desc:t=>`Reach a ${t}-card streak at least once.`,target:l=>Math.min(10,6+Math.floor(l/180)),progress:m=>Math.min(game.levelBestStreak||0,m.target),done:m=>(game.levelBestStreak||0)>=m.target},
   {id:'stock2',icon:'🌽',name:'Packed Pantry',minLevel:75,desc:t=>`Finish with at least ${t} draw cards still in stock.`,target:l=>Math.min(8,4+Math.floor(l/160)),progress:m=>Math.min(game.stock.length,m.target),done:m=>game.stock.length>=m.target},
   {id:'specialMaster',icon:'🧩',name:'Obstacle Course',minLevel:100,requires:b=>(b.obstacleCount||0)>=2,desc:t=>`Clear ${t} special cards through ordinary play.`,target:(l,b)=>Math.min(4,Math.max(2,b.obstacleCount||2)),progress:m=>Math.min(game.missionMetrics?.naturalSpecials||0,m.target),done:m=>(game.missionMetrics?.naturalSpecials||0)>=m.target}
 ];
 ...
 function missionRequiresWin(id){return ['stock','stock2','noPower','noBuy','natural'].includes(id)}
 ...
 function challengeVariantForLevel(level=state.level){
   const variants=[
     {key:'rugged',name:'Rugged Field',icon:'🪨',desc:'2 fewer stock cards • 1 Heavy + 1 Sleeping card',stockCut:2,min:40},
     {key:'doubleHeavy',name:'Heavy Harvest',icon:'🪨',desc:'1 fewer stock card • up to 2 Heavy cards',stockCut:1,min:40},
     {key:'drowsy',name:'Sleeping Meadow',icon:'🌙',desc:'2 fewer stock cards • up to 2 Sleeping cards',stockCut:2,min:40},
     {key:'lean',name:'Lean Stock',icon:'🃏',desc:'4 fewer stock cards • 1 Heavy card',stockCut:4,min:40},
     {key:'stoneNap',name:'Stone & Slumber',icon:'🌘',desc:'3 fewer stock cards • Heavy + up to 2 Sleeping cards',stockCut:3,min:40},
     {key:'ironRow',name:'Iron Row',icon:'⛓️',desc:'2 fewer stock cards • up to 3 Heavy cards',stockCut:2,min:40},
     {key:'longHaul',name:'Long-Haul Harvest',icon:'🌾',desc:'3 fewer stock cards • 2 Heavy + 1 Sleeping card',stockCut:3,min:600},
     {key:'ultraNight',name:'Ultra Night Run',icon:'🏆',desc:'4 fewer stock cards • 2 Heavy + up to 2 Sleeping cards',stockCut:4,min:800}
   ],available=variants.filter(v=>level>=v.min);
   return available[Math.floor(Math.max(40,level)/7)%available.length];
 }
 function applyChallengeMutators(){
   game.challengeHand=true;
   const variant=challengeVariantForLevel(state.level),flavor=game.dailyMode?null:DSH.Seasons?.challengeFlavor(state.level);game.challengeVariant=variant.key;game.challengeVariantName=variant.name;game.seasonChallengeBonus=flavor?.bonusCoins||0;game.seasonChallengeLabel=flavor?.label||'';
   game.stock.splice(0,Math.min(variant.stockCut,Math.max(0,game.stock.length-18)));
   const normal=game.layout.filter(c=>!c.special),added=[];
   const addHeavy=qty=>{
     normal.filter(c=>!c.special).slice(0,qty).forEach(c=>{c.special='heavy';added.push(c)});
   };
   const addSleeping=qty=>{
     const need=DSH.Config.specialCards.sleepingClears,requiredIds=new Set(game.layout.flatMap(c=>c.specialRequires||[]));
     normal.filter(c=>!c.special&&!requiredIds.has(c.id)&&blockerAncestorCountForCard(c)>=need).slice(0,qty).forEach(c=>{c.special='sleeping';added.push(c)});
   };
   if(variant.key==='rugged'){addHeavy(1);addSleeping(1)}
   else if(variant.key==='doubleHeavy')addHeavy(2);
   else if(variant.key==='drowsy')addSleeping(2);
   else if(variant.key==='stoneNap'){addHeavy(1);addSleeping(2)}
   else if(variant.key==='ironRow')addHeavy(3);
   else if(variant.key==='longHaul'){addHeavy(2);addSleeping(1)}
   else if(variant.key==='ultraNight'){addHeavy(2);addSleeping(2)}
   else addHeavy(1);
   added.forEach(c=>state.collection.specials[c.special]=true);
   game.obstacleLevel=game.obstacleLevel||added.length>0;
 }
 function renderChallengeOffer(){
   const v=challengeVariantForLevel(state.level),flavor=game.handSeasonInfo?DSH.Seasons?.challengeFlavor(state.level):null;
   setTextIfPresent('challengeVariantTitle',`${v.icon} ${v.name}${flavor?` • ${flavor.label}`:''}`);
   setTextIfPresent('challengeVariantDesc',`${v.desc}${game.handSeasonInfo?.variant?` • Seasonal focus: ${game.handSeasonInfo.variant.icon} ${game.handSeasonInfo.variant.name}`:''}`);
   setTextIfPresent('challengeVariantBonus',`Win bonus: +${DSH.Config.specialCards.challengeCoins+(flavor?.bonusCoins||0)} 🪙 +${DSH.Config.specialCards.challengeGems} 💎${flavor?.bonusCoins?` (${flavor.bonusCoins} seasonal)`:''}`);
 }
 function maybeOfferChallenge(){
   ...
   renderChallengeOffer();game.challengeOfferOpen=true;setTimeout(()=>{if(handStarted&&!handComplete&&game.challengeOfferOpen&&!mechanicNoticeActive)document.getElementById('challengeOfferOverlay').classList.add('show')},90);
 }
 ...
 function startLevelUnsafe(restarting=false){
   ...
   const previousFeather=sameRetry?!!game.featherActive:false,previousPawprint=sameRetry?!!game.pawprintActive:false,previousWeatherFx=sameRetry&&game.handWeatherFx?game.handWeatherFx:null,previousSeasonInfo=sameRetry&&game.handSeasonInfo?game.handSeasonInfo:null,previousSeasonFx=sameRetry&&game.handSeasonFx?game.handSeasonFx:null;
   const d=deck(rng),built=DSH.Levels.buildSafe?DSH.Levels.buildSafe(state.level,rng):DSH.Levels.build(state.level,rng);
   const featherActive=sameRetry?previousFeather:(state.rosieAdventureFinds?.feather||0)>0,pawprintActive=sameRetry?previousPawprint:(state.rosieAdventureFinds?.pawprint||0)>0;
   ...
   const weatherFx=previousWeatherFx||DSH.Weather.effects(state),seasonInfo=previousSeasonInfo||DSH.Seasons?.info(state.level)||null,seasonFx=previousSeasonFx||seasonInfo?.effects||{};built.stockTarget=Math.min(DSH.Config.difficulty.weatherCeiling,built.stockTarget+(weatherFx.extraStock||0)+(seasonFx.extraStock||0));
   ...
   const luckyHand=rng()<DSH.Config.specialCards.luckyHandChance,luckyVariants=['bountiful','wildflower','golden','deepStock','treasurePatch'],luckyVariant=luckyHand?luckyVariants[Math.floor(rng()*luckyVariants.length)]:'';
   game={layout:built.cards,formation:built.name,stock,waste,streak:0,levelBestStreak:0,chainUnlocked:false,score:0,moves:0,awardedMilestones:[],
     ...
     powersUsed:0,drawPacksBought:0,obstacleLevel:built.obstacleCount>0,obstacleStockBonus:built.obstacleStockBonus||0,milestone:!!built.milestone,farmYearMilestone:!!built.farmYearMilestone,seasonalFormation:!!built.seasonalFormation,
     reserve:null,reserveUsed:false,reserveEverUsed:false,cardsClearedThisHand:0,challengeDecisionMade:previousChallengeDecision,challengeOfferOpen:false,luckyHand,luckyVariant,challengeHand:false,previewChoiceUsed:false,
     previewChoiceOpen:false,handWeatherFx:weatherFx,handSeasonInfo:seasonInfo,handSeasonFx:seasonFx,seasonChallengeBonus:0,seasonChallengeLabel:'',seasonLuckyFavor:'',dailyMode:false,mission:previousMission,missionChoices:previousChoices||generatedMissionChoices,missionChosen:previousMissionChosen,missionFailed:false,missionCompleted:false,missionLevel:state.level,featherActive,pawprintActive,
     missionMetrics:{runs3:0,naturalSpecials:0,endRun:0,lastRemaining:built.cards.length}};
   if(game.luckyHand){
     const normal=shuffle(game.layout.filter(c=>!c.special),rng);
     if(game.luckyVariant==='wildflower'){
       for(let i=0;i<2;i++)game.stock.push(makeBonusCard(rng));normal.slice(0,2).forEach(c=>c.card={r:null,s:null,wild:true});
     }else if(game.luckyVariant==='golden'){
       for(let i=0;i<2;i++)game.stock.push(makeBonusCard(rng));if(normal[0]){normal[0].special='gold';state.collection.specials.gold=true}
     }else if(game.luckyVariant==='deepStock'){
       for(let i=0;i<5;i++)game.stock.push(makeBonusCard(rng));
     }else if(game.luckyVariant==='treasurePatch'){
       for(let i=0;i<2;i++)game.stock.push(makeBonusCard(rng));normal.slice(0,2).forEach(c=>{c.special='gold';state.collection.specials.gold=true});
     }else{
       for(let i=0;i<3;i++)game.stock.push(makeBonusCard(rng));if(normal[0])normal[0].card={r:null,s:null,wild:true};
     }
   }
   if(game.luckyHand&&game.handSeasonInfo){
     const favor=DSH.Seasons?.luckyFavor(state.level);if(favor){game.seasonLuckyFavor=`${favor.season.variant?.icon||favor.season.icon} ${favor.label}: ${favor.desc}`;
       if(favor.kind==='stock')game.stock.push(makeBonusCard(rng));
       else{const eligible=shuffle(game.layout.filter(c=>!c.special&&!c.card?.wild),rng);if(eligible[0]){eligible[0].special=favor.kind;state.collection.specials[favor.kind]=true}}
     }
   }
   if(pawprintActive){
     const pawCandidates=game.layout.filter(c=>!c.special&&!c.card?.wild);
     if(pawCandidates.length)pawCandidates[Math.floor(rng()*pawCandidates.length)].card={r:null,s:null,wild:true};
   }
   if(game.handSeasonInfo){
     const seasonKey=game.handSeasonInfo.key.split('-').slice(1).join('-');
     document.getElementById('app').dataset.playSeason=seasonKey;
   }
   if(previousChallengeHand)applyChallengeMutators();
   if(featherActive&&!sameRetry)state.rosieAdventureFinds.feather--;
   if(pawprintActive&&!sameRetry)state.rosieAdventureFinds.pawprint--;
   ...
   close('winOverlay');close('mainMenuOverlay');render();showLevelPreview(built);if(built.recovered)showRewardToast('Field Recovered',`Level ${state.level} used a safe fallback layout because the generated board failed validation.`,'🛠️');if(pawprintActive)setTimeout(()=>msg('🐾 Lucky Pawprint: Rosie hid an extra Wild card in this tableau!'),330);
   if(built.farmYearMilestone)showRewardToast(`🌾 Farm Year Milestone • Level ${state.level}`,`${seasonInfo?.icon||'🌾'} ${seasonInfo?.name||'Season'} milestone hand • Seasonal formation • +2 extra stock`,'🌾');
   else if(built.milestone)showRewardToast(`🏆 Level ${state.level} Milestone`,`${milestoneName(state.level)} • Bonus stock + boosted clear reward`,'🏆');
   else if(game.luckyHand){
     const luckyText=game.luckyVariant==='wildflower'?'+2 stock cards • 2 bonus Wilds are hiding in the field':game.luckyVariant==='golden'?'+2 stock cards • a bonus Golden Card is hiding in the field':game.luckyVariant==='deepStock'?'+5 stock cards • a deeper draw pile for this hand':game.luckyVariant==='treasurePatch'?'+2 stock cards • 2 bonus Golden Cards are hiding in the field':'+3 stock cards • a bonus Wild is hiding in the field';
     showRewardToast('✨ LUCKY HAND!',`${luckyText}${game.seasonLuckyFavor?` • ${game.seasonLuckyFavor}`:''}`,'🌟');
   }
 ...
 let returnInspectorSelection=null;
 function closeReturnInspector(){
   returnInspectorSelection=null;const panel=document.getElementById('returnRewardInspector');if(panel)panel.classList.remove('open');
 }
 function returnClaimSourceLabel(source){
   return source==='makeup'?'Recovered with a Makeup Board':source==='legacy'?'Collected before v43.2':source==='manual'?'Collected manually':source?'Collected':'Collected';
 }
 function renderReturnRewardsMenuStatus(today=localDateKey()){
   const rr=ensureReturnSets(today),missing=returnMissingDates(today),pending=returnPendingClaims(today);
   const parts=[`🔥 ${rr.dailyStreak}-day streak`];if(pending.length)parts.push(`🎁 ${pending.length} ready`);if(missing.length)parts.push(`🃏 ${missing.length} makeup`);
   if(!pending.length&&!missing.length)parts.push('all recent rewards handled');
   setTextIfPresent('returnRewardsMenuStatus',rr.clockHold?'⚠️ Calendar check required':parts.join(' • '));
 }
 function renderReturnInspector(type,dateKey){
   returnInspectorSelection={type,dateKey};const rr=returnState(),panel=document.getElementById('returnRewardInspector'),icon=document.getElementById('returnInspectorIcon'),title=document.getElementById('returnInspectorTitle'),status=document.getElementById('returnInspectorStatus'),rewardEl=document.getElementById('returnInspectorReward'),action=document.getElementById('returnInspectorAction'),closeBtn=document.getElementById('returnInspectorClose');
   if(!panel||!icon||!title||!status||!rewardEl||!action)return;
   panel.classList.add('open');if(closeBtn)closeBtn.onclick=closeReturnInspector;
   action.hidden=true;action.onclick=null;action.textContent='Collect Reward';
   if(type==='daily'){
     const entry=rr.daily?.[dateKey]||null,streak=entry?.streak||0,reward=entry?dailyRewardFor(dateKey):null;
     icon.textContent='🔥';title.textContent=`Daily Reward • ${formatReturnDate(dateKey,{weekday:true})}`;
     rewardEl.textContent=reward?returnRewardText(reward):'No Daily reward was unlocked for this date.';
     if(rr.clockHold)status.textContent='Calendar check required • claims are paused until the device date catches up.';
     else if(entry?.claimed){status.textContent=`Day ${streak} • ${returnClaimSourceLabel(entry.source)}${entry.claimedAt?` • ${new Date(entry.claimedAt).toLocaleString()}`:''}`}
     else if(entry){status.textContent=`Day ${streak} • Ready to collect`;action.hidden=false;action.textContent='🎁 Collect Daily Reward';action.onclick=()=>claimDailyReturn(dateKey)}
     else{status.textContent=dateSerial(dateKey)<dateSerial(localDateKey())?'No visit recorded. Daily streak rewards cannot be recovered by Makeup Board.':'Daily reward not available yet.'}
     return;
   }
   const st=returnSlotStatus(type,dateKey),reward=slotRewardFor(type,dateKey),label=type==='weekly'?'Weekly':'Monthly';
   icon.textContent=type==='weekly'?'📆':'🗓️';title.textContent=`${label} Reward • ${formatReturnDate(dateKey,{weekday:true})}`;rewardEl.textContent=st.eligible?returnRewardText(reward):'This date was before Return Rewards were activated.';
   if(rr.clockHold)status.textContent='Calendar check required • claims and Makeup Boards are paused until the device date catches up.';
   else if(!st.eligible)status.textContent='Exempt • no reward was required for this date.';
   else if(st.claimed)status.textContent=`${returnClaimSourceLabel(st.source)}${st.claimedAt?` • ${new Date(st.claimedAt).toLocaleString()}`:''}`;
   else if(st.ready){status.textContent='Visited ✓ • reward is still waiting on the board.';action.hidden=false;action.textContent=`🎁 Collect ${label} Reward`;action.onclick=()=>claimReturnPeriodSlot(type,dateKey,'manual')}
   else if(st.missing){status.textContent='Missed visit • recover this reward by clearing its Makeup Board.';action.hidden=false;action.textContent='🃏 Play Makeup Board';action.onclick=()=>startMakeupBoard(dateKey)}
   else if(st.future)status.textContent='Upcoming reward • tap again after visiting on this date to collect it.';
   else status.textContent='Today’s visit has not been registered yet.';
 }
 function renderReturnRewardsUI(){
   const today=localDateKey(),rr=ensureReturnSets(today),weekKey=returnPeriodKey('weekly',today),monthKey=returnPeriodKey('monthly',today),week=ensureReturnSet('weekly',weekKey),month=ensureReturnSet('monthly',monthKey),todaySerial=dateSerial(today),dailyEntry=rr.daily?.[today]||null;
   const nextGap=rr.lastVisitDate?dateSerial(today)-dateSerial(rr.lastVisitDate):999,nextStreak=dailyEntry?dailyEntry.streak:(nextGap===1?rr.dailyStreak+1:Math.max(1,rr.dailyStreak||1)),daily=dailyEntry?dailyRewardFor(today):dailyReturnReward(nextStreak,today,(state.stats.returnDaysVisited||0)+1);
   setTextIfPresent('returnDailyStreak',dailyEntry?`🔥 Return Day ${dailyEntry.streak}${dailyEntry.claimed?' ✓':' • READY'}`:`🔥 Return streak ${rr.dailyStreak}`);
   setTextIfPresent('returnDailyStatus',rr.clockHold?'Return rewards are paused because the device date moved backward.':dailyEntry?.claimed?'Today’s Daily reward is collected. Tap the card to review it.':dailyEntry?'Tap Collect Daily to take today’s reward.':'Visit today to unlock the reward.');
   setTextIfPresent('returnDailyReward',returnRewardText(daily));
   const dailyBtn=document.getElementById('returnDailyClaimBtn');if(dailyBtn){dailyBtn.hidden=!dailyEntry||dailyEntry.claimed||rr.clockHold;dailyBtn.textContent=dailyEntry?.claimed?'Collected ✓':'🎁 Collect Daily';dailyBtn.onclick=()=>claimDailyReturn(today)}
   const dailyCard=document.getElementById('returnDailyCard');if(dailyCard)dailyCard.onclick=e=>{if(e.target!==dailyBtn)renderReturnInspector('daily',today)};
   const warning=document.getElementById('returnClockWarning');if(warning){warning.classList.toggle('show',!!rr.clockHold);warning.textContent=rr.clockHold?`⚠️ Device date is earlier than your last recorded visit (${rr.lastSeenDate}). New return visits and claims are paused until the calendar catches up.`:''}
   const loyalty=loyaltyInfo(state.stats.returnDaysVisited||1),baseScale=DSH.Config.returnRewards.economyScale||45;
   setTextIfPresent('returnLoyaltyRank',`Loyalty Rank ${loyalty.rank} • ${loyalty.visits} return day${loyalty.visits===1?'':'s'}`);
   setTextIfPresent('returnLoyaltyDetail',`Permanent reward growth • next rank at ${loyalty.nextRankAt} total return days • +10% each rank plus +25% every 30 days`);
   setTextIfPresent('returnLoyaltyMultiplier',`${Math.round(baseScale*loyalty.mult)}× coin value`);

   const recent=document.getElementById('returnRecentDailyGrid');if(recent){
     const dates=Object.keys(rr.daily).sort((a,b)=>dateSerial(b)-dateSerial(a)).slice(0,7);
     recent.innerHTML=dates.length?dates.map(k=>{const e=rr.daily[k],r=dailyRewardFor(k);return `<button class="returnDailyHistory ${e.claimed?'claimed':'ready'}" data-return-inspect="daily" data-return-date="${k}" title="${returnRewardText(r)}"><b>${formatReturnDate(k)}</b><span>${e.claimed?'✓':'🎁'}</span><small>Day ${e.streak}</small></button>`}).join(''):'<div class="makeupEmpty">Daily history begins with your first v43.2 visit.</div>';
   }

   const weekDates=returnPeriodDates('weekly',weekKey),weekEligible=returnEligibleDates('weekly',weekKey),weekCount=weekEligible.filter(k=>week.claimed[k]).length;
   setTextIfPresent('returnWeekLabel',`${formatReturnDate(weekDates[0])} – ${formatReturnDate(weekDates[6])}`);setTextIfPresent('returnWeekProgress',`${weekCount}/${weekEligible.length} collected`);
   const wg=document.getElementById('returnWeekGrid');if(wg)wg.innerHTML=weekDates.map(k=>{
     const st=returnSlotStatus('weekly',k,today),status=!st.eligible?'exempt':st.claimed?'claimed':st.ready?'ready':st.missing?'missed':k===today?'today':'future',reward=slotRewardFor('weekly',k),label=new Date(dateSerial(k)*86400000).toLocaleDateString(undefined,{weekday:'short'});
     return `<button class="returnSlot ${status}" data-return-inspect="weekly" data-return-date="${k}" title="${st.eligible?returnRewardText(reward):'Before Return Rewards were activated'}"><b>${label}</b><span>${st.claimed?'✓':st.ready?'🎁':!st.eligible?'—':st.missing?'!':k===today?'•':'·'}</span><small>${st.claimed?'Collected':st.ready?'Tap to collect':st.missing?'Makeup':st.eligible?returnRewardText(reward):'Exempt'}</small></button>`;
   }).join('');

   const monthDates=returnPeriodDates('monthly',monthKey),monthEligible=returnEligibleDates('monthly',monthKey),monthCount=monthEligible.filter(k=>month.claimed[k]).length;
   const[y,m]=monthKey.split('-').map(Number);setTextIfPresent('returnMonthLabel',new Date(y,m-1,1).toLocaleDateString(undefined,{month:'long',year:'numeric'}));setTextIfPresent('returnMonthProgress',`${monthCount}/${monthEligible.length} collected`);
   const mg=document.getElementById('returnMonthGrid');if(mg){
     const blank=(new Date(Date.UTC(y,m-1,1)).getUTCDay()+6)%7,blanks=Array.from({length:blank},()=>'<div class="returnMonthBlank"></div>').join('');
     mg.innerHTML=blanks+monthDates.map(k=>{const st=returnSlotStatus('monthly',k,today),status=!st.eligible?'exempt':st.claimed?'claimed':st.ready?'ready':st.missing?'missed':k===today?'today':'future';
       return `<button class="returnMonthDay ${status}" data-return-inspect="monthly" data-return-date="${k}" title="${st.eligible?returnRewardText(slotRewardFor('monthly',k))+' — tap to inspect':'Before Return Rewards were activated'}"><b>${Number(k.slice(8,10))}</b><span>${st.claimed?'✓':st.ready?'🎁':!st.eligible?'—':st.missing?'!':''}</span></button>`}).join('');
   }

   const renderBonus=(type,key,id)=>{
     const el=document.getElementById(id);if(!el)return;const set=rr[type][key],ready=returnSetComplete(type,key),reward=completionReward(type,key),partial=returnSetPartial(type,key);el.className=`returnSetBonus${ready?' ready':''}${set.completionClaimed?' claimed':''}${partial?' partial':''}`;
     el.innerHTML=partial?`<div><b>Starter partial ${type==='weekly'?'week':'month'}</b><small>Collect the remaining slots normally. Full-set completion bonuses begin with your next complete ${type==='weekly'?'week':'month'}.</small></div>`:
       set.completionClaimed?`<b>✓ ${type==='weekly'?'Weekly':'Monthly'} completion reward collected</b>`:
       ready?`<div><b>🎁 ${type==='weekly'?'Weekly':'Monthly'} completion reward ready!</b><small>${returnRewardText(reward)}</small></div><button data-return-complete="${type}" data-return-key="${key}">Claim</button>`:
       `<div><b>Special completion reward</b><small>${returnRewardText(reward)} • manually collect every slot to unlock</small></div>`;
   };
   renderBonus('weekly',weekKey,'returnWeekBonus');renderBonus('monthly',monthKey,'returnMonthBonus');

   const pending=returnPendingClaims(today),pendingList=document.getElementById('returnPendingList');setTextIfPresent('returnPendingCount',String(pending.length));
   if(pendingList)pendingList.innerHTML=pending.length?pending.map(x=>{
     const icon=x.type==='daily'?'🔥':x.type==='weekly'?'📆':'🗓️',label=x.type==='daily'?'Daily':x.type==='weekly'?'Weekly':'Monthly',reward=x.type==='daily'?dailyRewardFor(x.dateKey):slotRewardFor(x.type,x.dateKey);
     return `<button class="returnHistoryRow ready" data-return-inspect="${x.type}" data-return-date="${x.dateKey}"><span>${icon}</span><div><b>${label} • ${formatReturnDate(x.dateKey,{weekday:true})}</b><small>${returnRewardText(reward)}</small></div><strong>COLLECT</strong></button>`;
   }).join(''):'<div class="makeupEmpty">✓ No visited rewards are waiting to be collected.</div>';

   const historyItems=[];
   Object.entries(rr.daily).forEach(([dateKey,e])=>{if(e?.claimed)historyItems.push({type:'daily',dateKey,at:e.claimedAt||0,source:e.source||''})});
   for(const type of ['weekly','monthly'])for(const set of Object.values(rr[type]))for(const dateKey of Object.keys(set.claimed||{}))if(set.claimed[dateKey])historyItems.push({type,dateKey,at:set.claimedAt?.[dateKey]||0,source:set.sources?.[dateKey]||''});
   historyItems.sort((a,b)=>(b.at||0)-(a.at||0)||dateSerial(b.dateKey)-dateSerial(a.dateKey));const historyList=document.getElementById('returnHistoryList');
   if(historyList)historyList.innerHTML=historyItems.slice(0,14).map(x=>{
     const icon=x.type==='daily'?'🔥':x.type==='weekly'?'📆':'🗓️',label=x.type==='daily'?'Daily':x.type==='weekly'?'Weekly':'Monthly',reward=x.type==='daily'?dailyRewardFor(x.dateKey):slotRewardFor(x.type,x.dateKey);
     return `<button class="returnHistoryRow" data-return-inspect="${x.type}" data-return-date="${x.dateKey}"><span>${icon}</span><div><b>${label} • ${formatReturnDate(x.dateKey,{weekday:true})}</b><small>${returnRewardText(reward)}</small></div><strong>✓</strong></button>`;
   }).join('')||'<div class="makeupEmpty">Collected reward history will appear here.</div>';

   const missing=returnMissingDates(today),actions=returnCompletionActions().filter(x=>!(x.type==='weekly'&&x.key===weekKey)&&!(x.type==='monthly'&&x.key===monthKey)),list=document.getElementById('makeupList');
   setTextIfPresent('makeupCount',String(missing.length));
   if(list){
     const makeupHtml=missing.slice(0,14).map(x=>`<div class="makeupRow"><div><b>🃏 ${formatReturnDate(x.dateKey,{weekday:true})}</b><small>Recover ${x.types.map(t=>t==='weekly'?'Weekly':'Monthly').join(' + ')} slot${x.types.length===1?'':'s'}</small></div><button data-makeup-date="${x.dateKey}">Play Makeup Board</button></div>`).join('');
     const bonusHtml=actions.map(x=>`<div class="makeupRow bonusReady"><div><b>🎁 ${x.type==='weekly'?'Weekly':'Monthly'} bonus ready</b><small>${returnRewardText(x.reward)}</small></div><button data-return-complete="${x.type}" data-return-key="${x.key}">Claim</button></div>`).join('');
     list.innerHTML=makeupHtml+bonusHtml||'<div class="makeupEmpty">✓ No missed Return Reward slots need makeup.</div>';
   }
   document.querySelectorAll('[data-return-inspect]').forEach(b=>b.onclick=()=>renderReturnInspector(b.dataset.returnInspect,b.dataset.returnDate));
   document.querySelectorAll('[data-makeup-date]').forEach(b=>b.onclick=()=>startMakeupBoard(b.dataset.makeupDate));
   document.querySelectorAll('[data-return-complete]').forEach(b=>b.onclick=()=>claimReturnCompletion(b.dataset.returnComplete,b.dataset.returnKey));
   if(returnInspectorSelection)renderReturnInspector(returnInspectorSelection.type,returnInspectorSelection.dateKey);
   renderReturnRewardsMenuStatus(today);
 }
 ...
 function startDailyChallengeUnsafe(){
   ...
   game={layout:built.cards,formation:built.name,stock,waste,streak:0,levelBestStreak:0,chainUnlocked:false,score:0,moves:0,awardedMilestones:[],
     ...
     previewChoiceOpen:false,challengeDecisionMade:true,challengeOfferOpen:false,handSeasonInfo:null,handSeasonFx:{},seasonChallengeBonus:0,seasonChallengeLabel:'',seasonLuckyFavor:'',dailyMode:true,dailyKey:desc.key,dailySeed:desc.seed,dailyChallengeLevel:desc.level,dailyRewardPower:desc.power};
 ...
 let makeupProgressBaseline=null;
 const MAKEUP_RETURN_STAT_KEYS=['returnDaysClaimed','returnBestStreak','returnCoinsEarned','returnGemsEarned','returnTreatsEarned','weeklySlotsClaimed','monthlySlotsClaimed','makeupBoardsStarted','makeupBoardsCompleted','weeklyCompletionRewards','monthlyCompletionRewards','treatsFound','windmillsFound','magicGatesFound','rosieRescuesFound'];
 function captureMakeupProgress(){
   makeupProgressBaseline={bestStreak:state.bestStreak,stats:JSON.parse(JSON.stringify(state.stats||{})),collection:JSON.parse(JSON.stringify(state.collection||{})),mastery:JSON.parse(JSON.stringify(state.mastery||{})),masteryRewardClaims:JSON.parse(JSON.stringify(state.masteryRewardClaims||{})),masterySetClaims:JSON.parse(JSON.stringify(state.masterySetClaims||{}))};
   state._makeupProgressBackup={bestStreak:makeupProgressBaseline.bestStreak,stats:JSON.parse(JSON.stringify(makeupProgressBaseline.stats))};
 }
 function restoreMakeupProgress(){
   if(!makeupProgressBaseline)return false;const liveStats=state.stats||{},preserve={};MAKEUP_RETURN_STAT_KEYS.forEach(k=>preserve[k]=liveStats[k]);
   state.bestStreak=makeupProgressBaseline.bestStreak;state.stats=JSON.parse(JSON.stringify(makeupProgressBaseline.stats));MAKEUP_RETURN_STAT_KEYS.forEach(k=>{if(preserve[k]!==undefined)state.stats[k]=preserve[k]});
   state.collection=JSON.parse(JSON.stringify(makeupProgressBaseline.collection));state.mastery=JSON.parse(JSON.stringify(makeupProgressBaseline.mastery));state.masteryRewardClaims=JSON.parse(JSON.stringify(makeupProgressBaseline.masteryRewardClaims));state.masterySetClaims=JSON.parse(JSON.stringify(makeupProgressBaseline.masterySetClaims));state._makeupProgressBackup=null;makeupProgressBaseline=null;return true;
 }
 function startMakeupBoardUnsafe(dateKey,today=localDateKey()){
   if(returnState().clockHold){showRewardToast('Calendar Check Required','Makeup Boards are paused until the device calendar catches up to the last recorded visit.','⚠️');openSection('returnRewardsOverlay');return false}
   if(!makeupNeeded(dateKey,today)){showRewardToast('Makeup Not Needed','That date no longer has a missed Weekly or Monthly reward slot.','✓');openSection('returnRewardsOverlay');return false}
   const desc=makeupDescriptor(dateKey),C=DSH.Config.returnRewards.makeup,rng=mulberry32(desc.seed);captureMakeupProgress();
   document.getElementById('app').classList.remove('menu-mode');close('mainMenuOverlay');close('returnRewardsOverlay');close('dailyOverlay');close('winOverlay');
   const built=DSH.Levels.buildSafe?DSH.Levels.buildSafe(desc.level,rng):DSH.Levels.build(desc.level,rng),d=deck(rng);
   built.cards.forEach(c=>c.card=d.pop());assignWilds(built.cards,desc.level,rng);prepareSpecialCards(built.cards);
   state.collection=state.collection||{specials:{},crops:{},regions:{0:true},powers:{},rosie:{}};
   const waste=d.pop(),desired=Math.min(43,(built.stockTarget||DSH.Levels.stockTarget(desc.level,built.cards.length))+(C.extraStock||4));let stock=d.slice(-Math.min(desired,d.length));
   if(stock.length<desired){const extra=deck(rng);while(stock.length<desired&&extra.length)stock.unshift(extra.pop())}
   game={layout:built.cards,formation:built.name,stock,waste,streak:0,levelBestStreak:0,chainUnlocked:false,score:0,moves:0,awardedMilestones:[],
     diceUsed:false,diceMode:false,diceRoll:0,seedUsed:false,seedUses:0,seedMode:false,sunUsed:false,gateMode:false,shearsMode:false,
     shearsCharges:built.obstacleCount?DSH.Config.shears.startOnObstacleLevel:0,shearsMeter:0,shearsGenerated:built.obstacleCount?DSH.Config.shears.startOnObstacleLevel:0,
     powersUsed:0,drawPacksBought:0,obstacleLevel:built.obstacleCount>0,obstacleStockBonus:built.obstacleStockBonus||0,cardsClearedThisHand:0,
     previewChoiceOpen:false,challengeDecisionMade:true,challengeOfferOpen:false,handSeasonInfo:null,handSeasonFx:{},seasonChallengeBonus:0,seasonChallengeLabel:'',seasonLuckyFavor:'',
     dailyMode:false,makeupMode:true,makeupDate:dateKey,makeupToday:today,makeupSeed:desc.seed,makeupChallengeLevel:desc.level};
   history=[];lastExposed=new Set(exposed().map(c=>c.id));locked=false;handStarted=true;handComplete=false;completionFailureCount=0;
   state.stats.makeupBoardsStarted=(state.stats.makeupBoardsStarted||0)+1;DSH.Save.save(state);render();showLevelPreview(built);
   if(built.recovered)showRewardToast('Makeup Field Recovered','The generated Makeup Board needed the safe fallback field.','🛠️');
   msg(`🃏 Makeup Board for ${desc.label}: clear the field to recover the missed Return Reward slot${returnMissingDates(today).find(x=>x.dateKey===dateKey)?.types.length===2?'s':''}.`);setTimeout(collectMechanicNotices,350);return true;
 }
 function startMakeupBoard(dateKey,today=localDateKey()){
   if(makeupProgressBaseline)restoreMakeupProgress();
   const stateBackup=JSON.stringify(state),gameBackup=JSON.stringify(game),rawBackup=DSH.Save.rawPrimary?.()??null;
   try{return startMakeupBoardUnsafe(dateKey,today)}
   catch(err){makeupProgressBaseline=null;state=JSON.parse(stateBackup);game=JSON.parse(gameBackup);DSH.Audio.bindState(state);DSH.Farm.bind(state,commit,msg,showRewardToast);try{if(rawBackup===null)localStorage.removeItem(DSH.Save.KEY);else localStorage.setItem(DSH.Save.KEY,rawBackup)}catch(e){}recoverStartFailure('Makeup Board',err);return false}
 }
 function winMakeup(){
   const target=game.makeupDate,levelStreak=game.levelBestStreak||0,perfect=(game.drawPacksBought||0)===0&&(game.powersUsed||0)===0,walletCoins=state.coins,walletGems=state.gems,recovered=[];restoreMakeupProgress();
   for(const type of ['weekly','monthly'])if(returnSlotMissing(type,target,game.makeupToday||localDateKey()))recovered.push(claimReturnPeriodSlot(type,target,'makeup'));
   if(recovered.length)state.stats.makeupBoardsCompleted=(state.stats.makeupBoardsCompleted||0)+1;
   handComplete=true;handStarted=false;game.completing=false;DSH.Audio.play.win();DSH.Save.save(state);renderAllBalances();renderReturnRewardsUI();renderStats();
   const coins=Math.max(0,state.coins-walletCoins),gems=Math.max(0,state.gems-walletGems);
   setTextIfPresent('rewardCoins',coins);setTextIfPresent('rewardGems',gems);setTextIfPresent('winStars',perfect?'★★★':'★★☆');document.getElementById('perfectClearBadge').classList.toggle('show',perfect);
   setTextIfPresent('winText',`Makeup Board • ${formatReturnDate(target,{weekday:true})} • Best streak ${levelStreak}`);
   const recoveredText=recovered.length?recovered.map(x=>`${x.type==='weekly'?'📆 Weekly':'🗓️ Monthly'}: ${returnRewardText(x.bundle)}`).join(' • '):'The missed slot had already been recovered.';
   const ready=returnCompletionActions().filter(x=>x.type==='weekly'||x.type==='monthly');setTextIfPresent('winBonusText',`${recoveredText}${ready.length?' • 🎁 A set-completion reward is ready in Return Rewards.':''}`);
   document.getElementById('nextBtn').textContent='Return Rewards';showRewardToast('🃏 Makeup Board Complete!',recoveredText,'🃏');open('winOverlay');
 }
 ...
 function restartCurrent(){close('pauseOverlay');if(game.makeupMode){restoreMakeupProgress();startMakeupBoard(game.makeupDate,game.makeupToday||localDateKey())}else if(game.dailyMode)startDailyChallenge();else startLevel(true)}
 ...
 function ensureHand(){
   ...
   if(game.dailyMode||game.makeupMode||!handStarted||handComplete||!game.layout)startLevel();else render();
 }
 ...
 function awardStreakMilestone(){
   ...
   if(phase===S.phases.coin){
     ...
     if(game.dailyMode||game.makeupMode){const scoreReward=reward*10;game.score+=scoreReward;text=`+${scoreReward} challenge score`}
   ...
   if(!game.dailyMode&&!game.makeupMode&&game.streak%DSH.Config.streak.happinessEvery===0){
 ...
 function undo(){
   ...
   ['bestStreak','stats','windmills','coins','gems','magicGates','rosieRescues','rosieTreats','rosieHappiness','lifetime','_walletSnapshot','mastery','masteryRewardClaims','masterySetClaims','collection',
 ...
 function recordPowerMastery(key){if(game.makeupMode)return;state.mastery=state.mastery||{crops:{},specials:{},powers:{},rosie:{}};state.mastery.powers=state.mastery.powers||{};state.mastery.powers[key]=(state.mastery.powers[key]||0)+1}
 function recordRosieMastery(key){if(game.makeupMode)return;state.mastery=state.mastery||{crops:{},specials:{},powers:{},rosie:{}};state.mastery.rosie=state.mastery.rosie||{};state.mastery.rosie[key]=(state.mastery.rosie[key]||0)+1}
 function useShearsOnCard(c){
   ...
   game.powersUsed=(game.powersUsed||0)+1;state.stats.powersUsed=(state.stats.powersUsed||0)+1;updateMission();state.stats.shearsUsed=(state.stats.shearsUsed||0)+1;recordPowerMastery('shears');
 ...
 function useGateOnCard(c){
   ...
   state.magicGates--;
   state.stats.magicGatesUsed=(state.stats.magicGatesUsed||0)+1;recordPowerMastery('gate');game.powersUsed=(game.powersUsed||0)+1;state.stats.powersUsed=(state.stats.powersUsed||0)+1;updateMission();
   c.removed=true;
 ...
 function useRosieRescue(){
   ...
   if(mechanicHelp){
     ...
     state.rosieRescues--;state.stats.rosieRescuesUsed=(state.stats.rosieRescuesUsed||0)+1;recordPowerMastery('rescue');recordRosieMastery('rescue');game.powersUsed=(game.powersUsed||0)+1;state.stats.powersUsed=(state.stats.powersUsed||0)+1;updateMission();
   ...
   const consume=()=>{
     state.rosieRescues--;
     state.stats.rosieRescuesUsed=(state.stats.rosieRescuesUsed||0)+1;recordPowerMastery('rescue');recordRosieMastery('rescue');
 ...
 function useDiceOnCard(c){
   ...
   snapshot();game.streak=0;breakHarvestChain();c.removed=true;applySpecialClear(c,true);game.diceUsed=true;recordPowerMastery('dice');game.powersUsed=(game.powersUsed||0)+1;state.stats.powersUsed=(state.stats.powersUsed||0)+1;updateMission();game.diceMode=false;game.diceRoll=0;
 ...
 function useSeedOnCard(c){
   ...
   game.seedUses=(game.seedUses||0)+1;recordPowerMastery('seed');const seedLimit=state.luckySeedTier2?DSH.Config.rewardShopTier2.seed.uses:1;game.seedUsed=game.seedUses>=seedLimit;
 ...
 function useSunCharm(){
   ...
   game.sunUsed=true;recordPowerMastery('sun');game.powersUsed=(game.powersUsed||0)+1;state.stats.powersUsed=(state.stats.powersUsed||0)+1;if(game.missionMetrics)game.missionMetrics.endRun=0;updateMission();DSH.Save.save(state);DSH.Audio.play.flip();render();
 ...
 function useWindmill(){
   ...
   snapshot();state.windmills--;state.stats.windmillsUsed++;recordPowerMastery('windmill');game.powersUsed=(game.powersUsed||0)+1;state.stats.powersUsed=(state.stats.powersUsed||0)+1;updateMission();locked=true;DSH.Audio.play.windmill();gust();
 ...
 function win(){
   ...
   if(game.makeupMode)return winMakeup();
   ...
   const winSeasonFx=game.handSeasonFx||DSH.Seasons?.effects(state,completedLevel)||{};let coins=Math.round((base+streak+barnBonus+starBonus+perfectBonus)*((game.handWeatherFx||DSH.Weather.effects(state)).levelCoin||1)*(winSeasonFx.levelCoin||1));
   const farmYearFinaleBonus=game.farmYearMilestone?Math.min(4000,600+Math.floor(completedLevel/40)*100):0;
   coins+=farmYearFinaleBonus;
   ...
   if(completedLevel%DSH.Config.milestones.interval===0&&!state.milestonesCleared[completedLevel]){
     const MC=DSH.Config.milestones,landmark=MC.landmarks?.[completedLevel]||null;
     const mc=landmark?.coins||(MC.coinBase+completedLevel*MC.coinPerLevel),mg=landmark?.gems||(MC.gemsBase+Math.floor(completedLevel/50));
     const rescues=landmark?.rescues||1,gates=landmark?.gates||0,windmills=landmark?.windmills||0,treats=landmark?.treats||0;
     state.milestonesCleared[completedLevel]=true;state.stats.milestonesCompleted=(state.stats.milestonesCompleted||0)+1;
     if(landmark)state.stats.landmarkMilestonesCompleted=(state.stats.landmarkMilestonesCompleted||0)+1;
     ...
     state.coins+=mc;state.gems+=mg;state.rosieRescues=(state.rosieRescues||0)+rescues;state.stats.rosieRescuesFound=(state.stats.rosieRescuesFound||0)+rescues;
     if(gates){state.magicGates=(state.magicGates||0)+gates;state.stats.magicGatesFound=(state.stats.magicGatesFound||0)+gates}
     if(windmills){state.windmills=(state.windmills||0)+windmills;state.stats.windmillsFound=(state.stats.windmillsFound||0)+windmills}
     if(treats){state.rosieTreats=(state.rosieTreats||0)+treats;state.stats.treatsFound=(state.stats.treatsFound||0)+treats}
     milestoneResult={coins:mc,gems:mg,rescues,gates,windmills,treats,name:landmark?.title||milestoneName(completedLevel),landmark,level:completedLevel};
   }
   if(game.challengeHand){state.coins+=DSH.Config.specialCards.challengeCoins+(game.seasonChallengeBonus||0);state.gems+=DSH.Config.specialCards.challengeGems;state.stats.challengeHandsWon=(state.stats.challengeHandsWon||0)+1}
   ...
   const seasonResult=DSH.Seasons?.addProgress(state,'level',1,completedLevel);
   ...
   setTextIfPresent('winStars','★'.repeat(stars)+'☆'.repeat(3-stars));
   document.getElementById('perfectClearBadge').classList.toggle('show',perfect);
   const landmarkCelebration=document.getElementById('landmarkCelebration');
   if(landmarkCelebration){
     const lm=milestoneResult?.landmark;
     landmarkCelebration.classList.toggle('show',!!lm);
     setTextIfPresent('landmarkIcon',lm?.icon||'🏆');setTextIfPresent('landmarkTitle',lm?.title||'');
     setTextIfPresent('landmarkDesc',lm?.desc||'');
   }
   document.getElementById('nextBtn').textContent=completedLevel===DSH.Config.milestones.campaignHorizon?'Continue to Endless Harvest':'Next Level';
   ...
   if(milestoneResult){
     const extras=[`${milestoneResult.rescues} Rosie Rescue${milestoneResult.rescues===1?'':'s'} 🐾`];
     if(milestoneResult.gates)extras.push(`${milestoneResult.gates} Magic Gate${milestoneResult.gates===1?'':'s'} 🚪`);
     if(milestoneResult.windmills)extras.push(`${milestoneResult.windmills} Windmill${milestoneResult.windmills===1?'':'s'} 🌬️`);
     if(milestoneResult.treats)extras.push(`${milestoneResult.treats} Treat${milestoneResult.treats===1?'':'s'} 🦴`);
     details.push(`${milestoneResult.landmark?'🌟':'🏆'} ${milestoneResult.name}: +${milestoneResult.coins} coins +${milestoneResult.gems} 💎 + ${extras.join(' + ')}`);
   }
   if(game.seasonalFormation)details.push(`${game.handSeasonInfo?.icon||'🌾'} Seasonal formation: ${game.formation}`);
   if(game.farmYearMilestone)details.push(`🌾 Farm Year Milestone bonus: +${farmYearFinaleBonus} coins`);
   if((winSeasonFx.levelCoin||1)!==1)details.push(`${game.handSeasonInfo?.icon||'🍂'} ${game.handSeasonInfo?.name||'Season'} level payout ${Math.round(((winSeasonFx.levelCoin||1)-1)*100)>=0?'+':''}${Math.round(((winSeasonFx.levelCoin||1)-1)*100)}%`);
   if(seasonResult?.added)details.push(`${seasonResult.info.icon} ${seasonResult.info.name} Basket +${seasonResult.added} (${seasonResult.points}/${seasonResult.goal})`);
   if(seasonResult?.completed)details.push(`🧺 Season Basket complete: ${DSH.Seasons.rewardText(seasonResult.reward)}${seasonResult.firstKeepsake?` • New ${seasonResult.info.keepsake} keepsake`:''}`);
   if(seasonResult?.yearResult?.complete){const y=seasonResult.yearResult;details.push(`🌎 Farm Year ${y.year} complete: ${DSH.Seasons.rewardText(y.reward)} • Full-year streak ${y.streak}${y.masteryUp?` • Stewardship Tier ${y.masteryTier} unlocked`:''}`)}
   if(game.challengeHand)details.push(`🔥 ${game.challengeVariantName||'Challenge Hand'}: +${DSH.Config.specialCards.challengeCoins+(game.seasonChallengeBonus||0)} coins +${DSH.Config.specialCards.challengeGems} 💎${game.seasonChallengeBonus?` • ${game.seasonChallengeLabel}`:''}`);if(game.luckyHand)details.push(`🌟 Lucky Hand (${game.luckyVariant||'bountiful'}) completed${game.seasonLuckyFavor?` • ${game.seasonLuckyFavor}`:''}`);
   const nextSeason=DSH.Seasons?.info(completedLevel+1);if(seasonResult?.info&&nextSeason&&nextSeason.key!==seasonResult.info.key)details.push(`➡️ Next season: ${nextSeason.icon} ${nextSeason.name} • ${nextSeason.variant?.icon||'✨'} ${nextSeason.variant?.name||''}`);
   ...
   showRewardToast(milestoneResult?.landmark?`${milestoneResult.landmark.icon} ${milestoneResult.name}!`:milestoneResult?'Milestone Cleared!':seasonResult?.yearResult?.complete?`🌎 Farm Year ${seasonResult.yearResult.year} Complete!`:seasonResult?.completed?`${seasonResult.info.icon} ${seasonResult.info.name} Complete!`:stars===3?'Three-Star Clear!':'Level Complete',
     milestoneResult?`${milestoneResult.coins} coins • ${milestoneResult.gems} 💎 • ${milestoneResult.rescues} Rosie Rescue${milestoneResult.rescues===1?'':'s'}${completedLevel===DSH.Config.milestones.campaignHorizon?' • Endless Harvest unlocked!':''}`:seasonResult?.yearResult?.complete?`${DSH.Seasons.rewardText(seasonResult.yearResult.reward)} • Full-year streak ${seasonResult.yearResult.streak}${seasonResult.yearResult.masteryUp?` • 🏵️ Tier ${seasonResult.yearResult.masteryTier}`:''}`:seasonResult?.completed?`${DSH.Seasons.rewardText(seasonResult.reward)}${seasonResult.firstKeepsake?' • new keepsake!':''}`:perfect?'Perfect Clear — no purchased draws or powers used!':`${stars} star${stars===1?'':'s'} earned`,
     milestoneResult?.landmark?milestoneResult.landmark.icon:milestoneResult?'🏆':seasonResult?.completed?seasonResult.info.icon:'⭐');
   open('winOverlay');
 }
 ...
 function drawPackPrice(){
   const bought=game.drawPacksBought||0,D=DSH.Config.draws,P=D.prices,pricingLevel=game.makeupMode?(game.makeupChallengeLevel||state.level):game.dailyMode?(game.dailyChallengeLevel||state.level):state.level;
 ...
 function pouchInventoryCount(){
   ...
   const D=DSH.Config.draws,pricingLevel=game.makeupMode?(game.makeupChallengeLevel||state.level):game.dailyMode?(game.dailyChallengeLevel||state.level):state.level,surcharge=Math.floor(Math.max(0,pricingLevel-1)/(D.levelSurchargeEvery||10))*(D.levelSurcharge||0);
 ...
 function renderAllBalances(){
   processReturnDay(localDateKey(),true);if(!game.makeupMode){backfillMasteryFromStats();checkMasteryRewards(true);checkFarmhouseAchievements()}
   ...
   document.getElementById('menuLevel').textContent=state.level;renderChapterProgress();renderCampaignHorizon();renderSeasonUI();renderWeatherBulletin();
   renderShop();
   renderPouch();
   renderDailyUI();if(document.getElementById('app').classList.contains('menu-mode'))renderReturnRewardsMenuStatus();
   renderMainRosieStatus();
   refreshCollectionKnowledge();
   const allCollection=['specials','crops','regions','powers','rosie','milestones'].flatMap(collectionEntries),collectionFound=allCollection.filter(x=>x.found).length,collectionCompletion=almanacCompletion();
   setTextIfPresent('collectionMenuStatus',`${collectionCompletion.pct}% complete • ${collectionFound}/${allCollection.length} discoveries`);
   const fh=document.getElementById('farmhouseMenuStatus');if(fh){const te=Object.values(state.farmhouseTrophies||{}).filter(Boolean).length,toys=Object.values(state.rosieToys||{}).filter(Boolean).length,pending=farmhousePendingRewardCount();fh.textContent=pending?`🎁 ${pending} reward${pending===1?'':'s'} waiting`:`${te}/${Object.keys(DSH.Config.farmhouse.trophies).length} trophies • ${toys}/${Object.keys(DSH.Config.farmhouse.toys).length} toys`}
 }


 function render(animateReveals=false){
   applyAccessibilitySettings();renderAllBalances();
   document.getElementById('levelNum').textContent=game.makeupMode?'MAKEUP':game.dailyMode?'DAILY':state.level;document.getElementById('formationName').textContent=game.formation||'Meadow';
 ...
 function showMain(){
   const restoredMakeup=game.makeupMode&&makeupProgressBaseline?restoreMakeupProgress():false;if(restoredMakeup)DSH.Save.save(state,{skipBackup:true});
 ...
 function masteryThresholds(group){return DSH.Config.mastery?.thresholds?.[group]||[10,30,75]}
 function masteryTierFor(group,value){const t=masteryThresholds(group);return value>=t[2]?3:value>=t[1]?2:value>=t[0]?1:0}
 function backfillMasteryFromStats(){
   if((state.masteryBackfillVersion||0)>=1)return false;
   state.mastery=state.mastery||{crops:{},specials:{},powers:{},rosie:{}};['crops','specials','powers','rosie'].forEach(k=>state.mastery[k]=state.mastery[k]||{});
   const st=state.stats||{},apply=(group,key,value)=>{state.mastery[group][key]=Math.max(state.mastery[group][key]||0,Math.max(0,Number(value)||0))};
   apply('crops','apple',st.applesHarvested||0);
   const sm={flower:'flowersCleared',gold:'goldenCardsCleared',key:'keysCleared',barnlock:'barnLocksCleared',watering:'wateringCansCleared',bee:'beeCardsCleared',harvestchain:'harvestChainsCompleted',heavy:'heavyCardsCleared',sleeping:'sleepingCardsCleared',sunflower:'sunflowerCardsCleared',rainbow:'rainbowCardsCleared'};
   Object.entries(sm).forEach(([k,v])=>apply('specials',k,st[v]||0));
   const pm={windmill:'windmillsUsed',gate:'magicGatesUsed',rescue:'rosieRescuesUsed',shears:'shearsUsed'};Object.entries(pm).forEach(([k,v])=>apply('powers',k,st[v]||0));
   const rm={adventure:'rosieAdventuresCompleted',pet:'petRosieCount',treat:'treatsFed',rescue:'rosieRescuesUsed'};Object.entries(rm).forEach(([k,v])=>apply('rosie',k,st[v]||0));
   state.masteryBackfillVersion=1;DSH.Save.save(state,{skipBackup:true});return true;
 }
 function masteryValue(group,key){return Math.max(0,Number(state.mastery?.[group]?.[key])||0)}
 function masteryEntries(){
   const out=[];
   Object.entries(DSH.Config.crops).forEach(([key,d])=>out.push({group:'crops',key,icon:d.icon,name:d.name,value:masteryValue('crops',key)}));
   Object.entries(DSH.Config.almanac.specials).forEach(([key,d])=>out.push({group:'specials',key,icon:d.icon,name:d.name,value:masteryValue('specials',key)}));
   [['windmill','🌬️','Windmill'],['gate','🚪','Magic Gate'],['rescue','🐾','Rosie Rescue'],['dice','🎲','Magic Dice'],['seed','🌱','Lucky Seed'],['sun','☀️','Sun Charm'],['shears','✂️','Garden Shears']].forEach(([key,icon,name])=>out.push({group:'powers',key,icon,name,value:masteryValue('powers',key)}));
   [['adventure','🗺️','Rosie Adventures'],['pet','💗','Pet Rosie'],['treat','🦴','Treat Rosie'],['rescue','🐕','Rosie Rescues']].forEach(([key,icon,name])=>out.push({group:'rosie',key,icon,name,value:masteryValue('rosie',key)}));
   return out.map(e=>({...e,tier:masteryTierFor(e.group,e.value),thresholds:masteryThresholds(e.group)}));
 }
 function masteryTotals(){const entries=masteryEntries(),earned=entries.reduce((n,e)=>n+e.tier,0),possible=entries.length*3;return{entries,earned,possible,gold:entries.filter(e=>e.tier===3).length}}
 function almanacCompletion(){
   const tabs=['specials','crops','regions','powers','rosie','milestones'],all=tabs.flatMap(collectionEntries),found=all.filter(x=>x.found).length,m=masteryTotals();
   return{found,discoveries:all.length,mastery:m,earned:found+m.earned,possible:all.length+m.possible,pct:Math.round((found+m.earned)/Math.max(1,all.length+m.possible)*100)};
 }
 function masterySetStatus(){
   const entries=masteryEntries(),defs=DSH.Config.mastery?.sets||{};
   return Object.entries(defs).map(([key,d])=>{const group=entries.filter(e=>e.group===d.group),complete=group.length>0&&group.every(e=>e.tier>=3);return{key,...d,complete,claimed:!!state.masterySetClaims?.[key],gold:group.filter(e=>e.tier>=3).length,total:group.length}});
 }
 function checkMasteryRewards(announce=true){
   state.masteryRewardClaims=state.masteryRewardClaims||{};state.masterySetClaims=state.masterySetClaims||{};
   const tierKeys=['bronze','silver','gold'],aggregate={coins:0,gems:0,treats:0,windmills:0,gates:0,rescues:0};let medals=0,sets=0;
   for(const e of masteryEntries())for(let tier=1;tier<=e.tier;tier++){
     const claim=`${e.group}:${e.key}:${tier}`,tierKey=tierKeys[tier-1];if(state.masteryRewardClaims[claim])continue;
     state.masteryRewardClaims[claim]=true;const reward=DSH.Config.mastery.rewards?.[tierKey]||{},got=grantRewardBundle(reward,'mastery');Object.keys(aggregate).forEach(k=>aggregate[k]+=got[k]||0);medals++;state.stats.masteryRewardsClaimed=(state.stats.masteryRewardsClaimed||0)+1;
   }
   for(const set of masterySetStatus()){
     if(!set.complete||state.masterySetClaims[set.key])continue;state.masterySetClaims[set.key]=true;const got=grantRewardBundle(set.reward||{},'mastery');Object.keys(aggregate).forEach(k=>aggregate[k]+=got[k]||0);sets++;state.stats.masterySetsCompleted=(state.stats.masterySetsCompleted||0)+1;
   }
   if(medals||sets){DSH.Save.save(state);if(announce)setTimeout(()=>showRewardToast('⭐ Mastery Reward!',`${medals?`${medals} medal reward${medals===1?'':'s'}`:''}${medals&&sets?' • ':''}${sets?`${sets} collection set${sets===1?'':'s'}`:''} • ${returnRewardText(aggregate)}`,'⭐'),180)}
   return{medals,sets,reward:aggregate};
 }
 function collectionEntries(tab){
   ...
   const milestoneLevels=[...Array.from({length:10},(_,i)=>(i+1)*10),250,500,750,1000];
   const milestones=milestoneLevels.map(level=>{
     const ch=chapterForLevel(level),found=!!state.milestonesCleared[level],landmark=DSH.Config.milestones.landmarks?.[level];
     return{key:String(level),icon:found?(landmark?.icon||'🏆'):'🔒',name:found?`${level} — ${milestoneName(level)}`:landmark?`Level ${level} — ${landmark.title}`:`Level ${level} Milestone`,
       desc:found?`${ch.icon} ${ch.name} completed. First-clear ${landmark?'landmark':'milestone'} reward claimed.`:`Reach and clear Level ${level} to reveal this ${landmark?'campaign landmark':'milestone'}.`,found};
   });
   const rosie=[
     ...
     ...Object.entries(DSH.Config.rosieAdventures.seasonal||{}).map(([key,d])=>({key:'season-'+key,icon:d.memento?.icon||d.icon,name:d.memento?.name||`${d.name} Memento`,desc:d.memento?.desc||'A seasonal Adventure memento.',found:!!state.rosieSeasonalFinds?.[key]}))
   ];
 ...
 function renderCollection(tab){
   ...
   const comp=almanacCompletion();
   setTextIfPresent('collectionProgress',`${comp.found} / ${comp.discoveries} discoveries`);
   setTextIfPresent('collectionMenuStatus',`${comp.pct}% complete • ${comp.found}/${comp.discoveries} discoveries`);
   setTextIfPresent('masterySummaryLine',`Mastery: ${comp.mastery.earned}/${comp.mastery.possible} medals • ${comp.mastery.gold}/${comp.mastery.entries.length} Gold • Overall completion ${comp.pct}%`);
   const bar=document.getElementById('collectionProgressBar');if(bar)bar.style.width=comp.pct+'%';
   const grid=document.getElementById('collectionGrid');if(!grid)return;
   if(tab==='mastery'){
     const tierIcons=['▫️','🥉','🥈','🥇'],sets=masterySetStatus(),setHtml=`<div class="masterySetHeader"><b>⭐ Gold Collection Sets</b><small>Reach Gold with every entry in a set for an extra one-time reward.</small></div>`+sets.map(x=>`<div class="masterySetCard ${x.complete?'complete':''}"><span>${x.icon}</span><div><b>${x.name}${x.claimed?' ✓':''}</b><small>${x.gold}/${x.total} Gold • ${x.claimed?'Set reward collected':returnRewardText(x.reward)}</small></div></div>`).join('');
     const entriesHtml=comp.mastery.entries.map(e=>{const next=e.tier>=3?e.thresholds[2]:e.thresholds[e.tier],pct=e.tier>=3?100:Math.min(100,e.value/Math.max(1,next)*100),nextKey=['bronze','silver','gold'][e.tier],nextReward=e.tier>=3?'All medal rewards collected':`${tierIcons[e.tier+1]} reward: ${returnRewardText(DSH.Config.mastery.rewards?.[nextKey]||{})}`;
       return `<div class="collectionEntry masteryEntry tier${e.tier}"><div class="collectionIcon">${e.icon}</div><div><b>${e.name} ${tierIcons[e.tier]}</b><small>${e.value} actions${e.tier>=3?' • Gold mastered':` • next medal at ${next}`}</small><small class="masteryRewardHint">${nextReward}</small><div class="masteryTrack"><div style="width:${pct}%"></div></div></div><span>${e.tier}/3</span></div>`}).join('');
     grid.innerHTML=setHtml+entriesHtml;return;
   }
 ...
 function checkFarmhouseAchievements(context={}){
   ...
   const tests={
     ...
     quarterHarvest:!!state.milestonesCleared?.[250],halfwayHome:!!state.milestonesCleared?.[500],longRoad:!!state.milestonesCleared?.[750],
     ultraHarvest:!!state.milestonesCleared?.[1000],rosieVeteran:(state.stats.rosieAdventuresCompleted||0)>=100,
     fourSeasons:['spring','summer','autumn','winter'].every(k=>!!state.seasonKeepsakes?.[k]),
     seasonalSniffer:['spring','summer','autumn','winter'].every(k=>!!state.rosieSeasonalFinds?.[k]),
     fullCircle:(state.farmYearsCompleted||0)>=1,steadySeasons:(state.bestFarmYearStreak||0)>=3,masterSteward:(state.farmYearMasteryTier||0)>=3,
     firstMastery:masteryTotals().earned>=1,cropMaster:masteryEntries().filter(e=>e.group==='crops').every(e=>e.tier>=3),completionist:almanacCompletion().pct>=100
   };
   let first=null;
   ...
   [250,500,750,1000].forEach(level=>{if(state.milestonesCleared?.[level])state.chapterMemorabilia[level]=true});
 ...
 function renderFarmhouse(tab){
   ...
   if(shelf){
     // The room shelf has limited visual space. Always surface unclaimed trophies
     // first so a newly earned late-game reward can never be hidden behind old ones.
     const shelfEntries=[...earned].sort((a,b)=>Number(a.claimed)-Number(b.claimed));
     shelf.innerHTML=shelfEntries.slice(0,8).map(t=>`<span class="${!t.claimed?'rewardWaiting':''}" title="${t.name}${!t.claimed?' • reward waiting':''}">${t.icon}${!t.claimed?'<i>!</i>':''}</span>`).join('')||'<small>No trophies yet</small>';
   }
   const wall=document.getElementById('chapterWall');
   if(wall){
     const early=Array.from({length:10},(_,i)=>(i+1)*10),landmarks=[250,500,750,1000];
     wall.innerHTML=[...early,...landmarks].map(level=>{const got=!!state.chapterMemorabilia[level],ch=chapterForLevel(level),landmark=DSH.Config.milestones.landmarks?.[level];
       return `<span class="${got?'earned':''}${landmark?' landmarkKeepsake':''}" title="${got?(landmark?.title||ch.name):landmark?`Locked • Level ${level}`:'Locked chapter keepsake'}">${got?(landmark?.icon||ch.icon):'▧'}</span>`;
     }).join('');
   }
   ...
   if(tab==='trophies'){
     ...
     const earlyKeeps=['🌱 Pressed Clover','🌻 Sunflower Painting','🐾 Muddy Pawprint','🍎 Golden Apple','⛰️ Mountain Plaque','🍂 Autumn Wreath','🌙 Moon Lantern','❄️ Crystal Snowflake','🌷 Spring Bouquet','👑 Centennial Crown'];
     const entries=[
       ...Array.from({length:10},(_,i)=>({level:(i+1)*10,label:earlyKeeps[i]})),
       {level:250,label:'🌾 Quarter Harvest Crown'},{level:500,label:'🏅 Golden Seasons Plaque'},
       {level:750,label:"🐾 Rosie's Long Road Star"},{level:1000,label:'🏆 Ultra Harvest Crown'}
     ];
     box.innerHTML=`<div class="chapterKeepsakes">${entries.map(({level,label})=>{const ch=chapterForLevel(level),got=!!state.chapterMemorabilia[level],icon=label.split(' ')[0],name=label.substring(label.indexOf(' ')+1);return `<div class="${got?'earned':'locked'}${level>=250?' landmarkKeepsake':''}"><span>${got?icon:'🔒'}</span><b>${got?name:(DSH.Config.milestones.landmarks?.[level]?.title||ch.name)}</b><small>${got?`Earned by clearing Level ${level}.`:`Clear Level ${level} to place this keepsake.`}</small></div>`}).join('')}</div>`;
   }else if(tab==='mastery'){
     const m=masteryTotals(),comp=almanacCompletion(),sets=masterySetStatus(),gold=m.entries.filter(e=>e.tier>=3),notGold=m.entries.filter(e=>e.tier<3).sort((a,b)=>(b.value/b.thresholds[2])-(a.value/a.thresholds[2])).slice(0,6);
     const trophiesTotal=Object.keys(DSH.Config.farmhouse.trophies).length,trophiesEarned=Object.values(state.farmhouseTrophies||{}).filter(Boolean).length,toysTotal=Object.keys(DSH.Config.farmhouse.toys).length,toysFound=Object.values(state.rosieToys||{}).filter(Boolean).length,seasonKeeps=Object.values(state.seasonKeepsakes||{}).filter(Boolean).length;
     const setCabinet=sets.map(x=>`<div class="masteryHouseKeepsake ${x.claimed?'earned':'locked'}"><span>${x.claimed?(x.keepsake?.icon||x.icon):'🔒'}</span><b>${x.claimed?(x.keepsake?.name||x.name):x.name}</b><small>${x.claimed?'Gold collection set completed.':`${x.gold}/${x.total} Gold masteries`}</small></div>`).join('');
     const goldWall=gold.length?gold.map(e=>`<div class="masteryGoldPlaque"><span>${e.icon}</span><b>${e.name}</b><small>🥇 Gold • ${e.value} actions</small></div>`).join(''):'<div class="masteryHouseEmpty">No Gold Masteries yet. Your first Gold plaque will appear here.</div>';
     const nearest=notGold.length?notGold.map(e=>`<div class="masteryNearRow"><span>${e.icon}</span><div><b>${e.name}</b><small>${e.value}/${e.thresholds[2]} toward Gold</small></div><strong>${Math.round(Math.min(1,e.value/e.thresholds[2])*100)}%</strong></div>`).join(''):'<div class="masteryHouseEmpty">Every tracked Mastery is Gold. 🥇</div>';
     box.innerHTML=`<div class="masteryHouseHero"><div><span>⭐</span><b>${comp.pct}% Almanac Completion</b><small>${m.earned}/${m.possible} Mastery medals • ${m.gold}/${m.entries.length} Gold</small></div><div class="masteryHouseMeter"><div style="width:${comp.pct}%"></div></div></div>
       <h3>🏡 Mastery Display Cabinet</h3><div class="masteryHouseCabinet">${setCabinet}</div>
       <h3>🥇 Gold Mastery Wall</h3><div class="masteryGoldWall">${goldWall}</div>
       <h3>⭐ Closest to Gold</h3><div class="masteryNearList">${nearest}</div>
       <h3>📊 Completion Breakdown</h3><div class="completionBreakdown">
         <div><span>📖</span><b>${comp.found}/${comp.discoveries}</b><small>Discoveries</small></div>
         <div><span>🎖️</span><b>${m.earned}/${m.possible}</b><small>Mastery medals</small></div>
         <div><span>🏆</span><b>${trophiesEarned}/${trophiesTotal}</b><small>Farmhouse trophies</small></div>
         <div><span>🧸</span><b>${toysFound}/${toysTotal}</b><small>Rosie toys</small></div>
         <div><span>🍂</span><b>${seasonKeeps}/4</b><small>Season keepsakes</small></div>
         <div><span>🌎</span><b>${state.farmYearsCompleted||0}</b><small>Farm Years completed</small></div>
       </div>`;
   }else if(tab==='seasons'){
     const defs=DSH.Config.seasons.definitions,keeps=state.seasonKeepsakes||{},ss=state.seasonState||{},inf=DSH.Seasons.info(state.level),history=Object.entries(state.seasonHistory||{}).sort((a,b)=>(b[1]?.endLevel||0)-(a[1]?.endLevel||0)).slice(0,6),fy=DSH.Seasons.farmYearStatus(state,inf.year),yearHistory=Object.values(state.farmYearHistory||{}).sort((a,b)=>(b.year||0)-(a.year||0)).slice(0,8),masteryPct=(fy.masteryTier||0)*2;
     box.innerHTML=`<div class="seasonHouseSummary"><b>${inf.icon} ${inf.name} • Year ${inf.year}</b><span>${inf.variant?.icon||'✨'} ${inf.variant?.name||''} • ${ss.claimed?'Current basket complete ✓':`Current basket ${ss.points||0}/${inf.goal}`}</span><small>${DSH.Seasons.effectText(inf)}</small></div>
     <div class="farmYearSummary"><div><span>🌎</span><b>Farm Year ${inf.year}</b><small>${fy.record?.complete?'Complete ✓':`${fy.count}/4 Baskets complete`}</small></div><div><span>🔥</span><b>${fy.streak}</b><small>Current full-year streak • Best ${fy.bestStreak}</small></div><div><span>🏵️</span><b>Tier ${fy.masteryTier}</b><small>${masteryPct?`+${masteryPct}% crop & Order coins`:'Complete 3 full Farm Years for Tier I'}</small></div></div>
     <div class="seasonKeepsakes">${defs.map(d=>`<div class="${keeps[d.key]?'earned':'locked'}"><span>${keeps[d.key]?d.icon:'🔒'}</span><b>${keeps[d.key]?d.keepsake:d.name}</b><small>${keeps[d.key]?'First '+d.name+' Basket completed.':'Complete a '+d.name+' Basket to earn this keepsake.'}</small></div>`).join('')}</div>
     ${yearHistory.length?`<div class="farmYearHistory"><b>Farm Year History</b>${yearHistory.map(x=>`<span>${x.complete?'✅':'▫️'} Year ${x.year} • ${x.baskets}/4 Baskets${x.complete?` • Streak ${x.streak} • ${DSH.Seasons.rewardText(x.reward||{})}`:' • Finale missed'}</span>`).join('')}</div>`:''}
     ${history.length?`<div class="seasonHistory"><b>Recent Seasons</b>${history.map(([k,x])=>{const d=defs.find(v=>v.key===x.season),past=DSH.Seasons.byKey(k);return `<span>${x.claimed?'✅':'▫️'} ${d?.icon||'🍂'} Year ${x.year} ${d?.name||x.season}${past?.variant?` • ${past.variant.icon} ${past.variant.name}`:''} • ${x.points||0}/${DSH.Config.seasons.pointGoal}</span>`}).join('')}</div>`:''}`;
   }else if(tab==='rosie'){
     ...
     const favSeason=DSH.Farm.favoriteAdventureSeason?.(),seasonDefs=DSH.Config.seasons.definitions,seasonProfiles=DSH.Config.rosieAdventures.seasonal||{},mementos=state.rosieSeasonalFinds||{};
     const seasonRows=seasonDefs.map(d=>{const x=state.rosieSeasonStats?.[d.key]||{},trips=x.trips||0,avg=trips?Math.round((x.score||0)/trips):0,profile=seasonProfiles[d.key];return `<div class="${trips?'activeSeasonJournal':'emptySeasonJournal'}"><span>${d.icon}</span><b>${d.name}</b><small>${trips?`${trips} trip${trips===1?'':'s'} • Adventure score ${avg}/trip`:'No trips yet'}</small><em>${mementos[d.key]?`${profile?.memento?.icon||'✨'} ${profile?.memento?.name||'Memento found'}`:'🔒 Seasonal memento undiscovered'}</em></div>`}).join('');
     box.innerHTML=`<div class="rosieJournalStats"><div>🐾 <b>${state.stats.rosieAdventuresCompleted||0}</b><small>Adventures</small></div><div>🪙 <b>${(state.stats.rosieAdventureCoins||0).toLocaleString()}</b><small>Coins Found</small></div><div>💎 <b>${state.stats.rosieAdventureGems||0}</b><small>Gems Found</small></div><div>🎁 <b>${state.stats.rosieAdventureRareFinds||0}</b><small>Rare Finds</small></div></div><p class="houseJournal">Favorite destination: <b>${fav}</b> • Favorite season: <b>${favSeason?`${favSeason.def?.icon||'🍂'} ${favSeason.def?.name||favSeason.key}`:'None yet'}</b></p><h3>Seasonal Adventure Journal</h3><div class="rosieSeasonJournal">${seasonRows}</div><h3>Rosie's Toy Box</h3><div class="toyJournal">${Object.entries(toyDefs).map(([k,d])=>`<div class="${state.rosieToys?.[k]?'found':'locked'}${d.season?' seasonalToy':''}"><span>${state.rosieToys?.[k]?d.icon:'❔'}</span><b>${state.rosieToys?.[k]?d.name:d.season?`${seasonDefs.find(s=>s.key===d.season)?.icon||'🍂'} Seasonal Toy`:'Undiscovered Toy'}</b></div>`).join('')}</div>`;
   }else{
   ...
   setTextIfPresent('farmhouseMenuStatus',pending?`🎁 ${pending} reward${pending===1?'':'s'} waiting`:`${earned.length}/${trophies.length} trophies • ${toysFound}/${Object.keys(DSH.Config.farmhouse.toys).length} toys`);
 }
 ...
 function openSection(id){
   ...
   if(id==='returnRewardsOverlay'){closeReturnInspector();renderReturnRewardsUI()}
 ...
 function renderStats(){
   ...
   const horizon=DSH.Config.milestones.campaignHorizon||1000;
   setTextIfPresent('statCampaign',state.level>horizon?'COMPLETE':`${Math.max(0,Math.min(horizon,state.level-1))}/${horizon}`);
   setTextIfPresent('statHighestLevel',state.highestLevelReached||state.level||1);
   setTextIfPresent('statToyTotal',Object.keys(DSH.Config.farmhouse.toys).length);
   ...
   setTextIfPresent('statGoldenGems',state.stats.goldenGems||0);
   setTextIfPresent('statFlowers',state.stats.flowersCleared||0);setTextIfPresent('statRainbows',state.stats.rainbowCardsCleared||0);
   setTextIfPresent('statKeysLocks',`${state.stats.keysCleared||0} / ${state.stats.barnLocksCleared||0}`);
   setTextIfPresent('statWatering',state.stats.wateringCansCleared||0);setTextIfPresent('statBees',`${state.stats.beeCardsCleared||0} / ${state.stats.beeBuzzes||0}`);
   setTextIfPresent('statHarvestChains',state.stats.harvestChainsCompleted||0);setTextIfPresent('statHeavy',state.stats.heavyCardsCleared||0);
   setTextIfPresent('statSleeping',state.stats.sleepingCardsCleared||0);setTextIfPresent('statSunflowers',state.stats.sunflowerCardsCleared||0);
   setTextIfPresent('statShearsUsed',state.stats.shearsUsed||0);setTextIfPresent('statObstacleLevels',state.stats.obstacleLevelsCleared||0);setTextIfPresent('statRegionsUnlocked',Math.min((state.region||0)+1,DSH.Farm.regions.length));setTextIfPresent('statRegionDecorations',state.stats.regionDecorationsOwned||0);setTextIfPresent('statApples',state.stats.applesHarvested||0);
   ...
   setTextIfPresent('statDailyPerfect',state.stats.dailyPerfectClears||0);setTextIfPresent('statDailyStreak',state.stats.dailyBestWinStreak||0);
   setTextIfPresent('statReturnDays',`${state.stats.returnDaysVisited||0} / ${state.stats.returnDaysClaimed||0}`);setTextIfPresent('statReturnStreak',`${state.returnRewards?.dailyStreak||0} / ${state.returnRewards?.bestDailyStreak||0}`);setTextIfPresent('statReturnSlots',`${state.stats.weeklySlotsClaimed||0} / ${state.stats.monthlySlotsClaimed||0}`);setTextIfPresent('statMakeups',state.stats.makeupBoardsCompleted||0);setTextIfPresent('statReturnCompletions',`${state.stats.weeklyCompletionRewards||0} / ${state.stats.monthlyCompletionRewards||0}`);setTextIfPresent('statReturnWallet',`${(state.stats.returnCoinsEarned||0).toLocaleString()} / ${state.stats.returnGemsEarned||0}`);setTextIfPresent('statMasteryRewards',`${state.stats.masteryRewardsClaimed||0} / ${state.stats.masterySetsCompleted||0}`);
   setTextIfPresent('statTier2',state.stats.tier2UpgradesOwned||0);
   ...
   setTextIfPresent('statMilestones',state.stats.milestonesCompleted||0);setTextIfPresent('statMilestoneCoins',(state.stats.milestoneCoinsEarned||0).toLocaleString());setTextIfPresent('statMilestoneGems',state.stats.milestoneGemsEarned||0);setTextIfPresent('statSeasonBaskets',state.stats.seasonBasketsCompleted||0);setTextIfPresent('statSeasonPoints',state.stats.seasonPointsEarned||0);setTextIfPresent('statSeasonKeepsakes',Object.values(state.seasonKeepsakes||{}).filter(Boolean).length);
   setTextIfPresent('statFarmYears',state.farmYearsCompleted||0);setTextIfPresent('statFarmYearStreak',`${state.farmYearStreak||0} / ${state.bestFarmYearStreak||0}`);setTextIfPresent('statFarmYearMastery',`Tier ${state.farmYearMasteryTier||0}${state.farmYearMasteryTier?` (+${state.farmYearMasteryTier*2}% Farm coins)`:''}`);setTextIfPresent('statFarmYearCoins',(state.stats.farmYearFinaleCoins||0).toLocaleString());setTextIfPresent('statFarmYearGems',state.stats.farmYearFinaleGems||0);
   {const m=masteryTotals(),c=almanacCompletion();setTextIfPresent('statMasteryMedals',`${m.earned} / ${m.possible}`);setTextIfPresent('statGoldMasteries',m.gold);setTextIfPresent('statCompletionPct',`${c.pct}%`)}
   ...
   setTextIfPresent('statSeasonAdventureEvents',state.stats.rosieSeasonalEvents||0);setTextIfPresent('statSeasonMementos',Object.values(state.rosieSeasonalFinds||{}).filter(Boolean).length);setTextIfPresent('statSeasonToys',state.stats.rosieSeasonalToys||0);
   const favSeasonStat=DSH.Farm.favoriteAdventureSeason?.();setTextIfPresent('statRosieFavoriteSeason',favSeasonStat?`${favSeasonStat.def?.icon||'🍂'} ${favSeasonStat.def?.name||favSeasonStat.key}`:'—');
   setTextIfPresent('statRosieToys',Object.values(state.rosieToys||{}).filter(Boolean).length);setTextIfPresent('statTrophies',Object.values(state.farmhouseTrophies||{}).filter(Boolean).length);setTextIfPresent('statReserveUses',state.stats.reserveUses||0);setTextIfPresent('statChallengeWins',state.stats.challengeHandsWon||0);setTextIfPresent('statLuckyWins',state.stats.luckyHandsWon||0);
 ...
 function runDevAction(action){
   ...
   if(action==='levelAudit'){setDevReport(DSH.Diagnostics.textReport(state,{from:1,to:1000,samples:6}));showRewardToast('Diagnostics Complete','6,000 procedural boards audited.','🧪');return}
   ...
   if(action==='sampleLevels'){
     const out=[1,50,100,250,500,750,1000].map(level=>{const b=DSH.Levels.build(level);return{level,name:b.name,cards:b.cards.length,stock:b.stockTarget,compensation:b.obstacleStockBonus,specials:b.specialCounts}});setDevReport(JSON.stringify(out,null,2));return
   }
 ...
 document.getElementById('challengeAcceptBtn').onclick=()=>{
   ...
   document.getElementById('challengeOfferOverlay').classList.remove('show');render(true);showRewardToast(`🔥 ${game.challengeVariantName||'Challenge'} Accepted`,`Win for +${DSH.Config.specialCards.challengeCoins+(game.seasonChallengeBonus||0)} coins +${DSH.Config.specialCards.challengeGems} 💎${game.seasonChallengeLabel?` • ${game.seasonChallengeLabel}`:''}`,'🔥')
 };
 ...
 document.getElementById('mainReturnBtn').onclick=()=>openSection('returnRewardsOverlay');
 ...
 document.getElementById('nextBtn').onclick=()=>{if(game.makeupMode){showMain();openSection('returnRewardsOverlay')}else if(game.dailyMode)showMain();else startLevel()};
 ...
 document.getElementById('harvestAllBtn').onclick=()=>DSH.Farm.harvestAll();
 document.getElementById('replantBtn').onclick=()=>DSH.Farm.replantLastHarvest();
