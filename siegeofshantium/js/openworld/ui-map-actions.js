...
function regionTerrainHTML(region){
 const raw=region==='bluestone'?SOSText("openworld_ui_map_actions.regionTerrainHTML.001"):region==='redstone'?SOSText("openworld_ui_map_actions.regionTerrainHTML.002"):SOSText("openworld_ui_map_actions.regionTerrainHTML.003");
...
function openWorldMapHTML(){
 ...
 for(const loc of locationsInRegion(region)){if(!state.world.discovered.includes(loc.id)||((loc.hidden&&hiddenSiteRetired(loc.id))&&loc.id!==here))continue;const days=worldTravelDays(here,loc.id),current=loc.id===here&&!eq,contract=tq&&(tq.target===loc.id||tq.origin===loc.id),road=!current?roadConditionProfile(here,loc.id):null,cross=regionConnectionAt(loc.id);html+=`<button class="world-location ${current?'current':''} ${contract?'contract-location':''} ${cross?'regional-crossing-location':''} ${road?`route-${road.status}`:''} world-type-${loc.type} terrain-${loc.terrain||'normal'}" data-worldloc="${loc.id}" style="left:${loc.x}%;top:${loc.y}%" title="${esc(loc.desc)}${cross?` Regional crossing via ${esc(cross.name)}.`:''}"><i class="world-loc-icon">${worldLocationIcon(loc)}</i><b>${esc(loc.name)}</b>${current?'<span class="map-you-here">YOU ARE HERE</span>':`<span>${days}d • ${road.status}</span>`}${cross?`<em class="regional-crossing-tag">REGION GATE</em>`:''}${contract?'<em class="contract-pin">!</em>':''}${politicalMapTag(loc.id)}</button>`}
 ...
 for(const p of state.world.parties){if(worldPartyDisplayRegion(p)!==region)continue;const base=worldPartyPosition(p),d=worldPartyDisposition(p);if(!p.contractProtected&&p.id!==state.world.trackedPartyId&&((d==='hostile'&&!state.world.mapFilters.hostile)||(d==='friendly'&&!state.world.mapFilters.friendly)||(!['hostile','friendly'].includes(d)&&!state.world.mapFilters.neutral)))continue;const pos=arrangeTransientMapPosition(region,'party_'+p.id,base,used);html+=`<button class="world-party ${d} party-${p.kind} ${state.world.trackedPartyId===p.id?'tracked-party':''} ${p.contractProtected?'escort-caravan':''}" data-worldparty="${p.id}" style="left:${pos.x}%;top:${pos.y}%" title="${esc(p.name)} — ${esc(p.faction)} — toward ${esc(worldLocation(p.destination).name)}"><span>${worldPartySymbol(p,d)}</span></button>`}
 if(tp&&locationRegion(tp.location)===region){const pp=worldPartyPosition(tp),dest=worldLocation(tp.destination);html+=`<svg class="tracking-route" viewBox="0 0 920 700" preserveAspectRatio="none"><line x1="${pp.x*9.2}" y1="${pp.y*7}" x2="${dest.x*9.2}" y2="${dest.y*7}"/></svg>`}
...
function openWorldPlayerActionsHTML(){
 const atHall=state.world?.location==='shantium',loc=worldLocation(state.world.location),inSettlement=!!state.world.settlements?.[loc.id],held=partyPrisonerCount(),fieldProc=homeFieldProcurementsHere(),innBase=loc.id==='southroad'?14:loc.id==='redoubt'?28:20,innCost=inSettlement&&!atHall?settlementServicePrice(loc.id,innBase):0;
...
function confirmSettlementTavernRest(){
 const loc=worldLocation(state.world.location),inSettlement=!!state.world.settlements?.[loc.id];if(!inSettlement||loc.id==='shantium')return renderOpenWorld();
...
function showOpenWorldCampingMenu(){modalRouteEnter(SOSText("openworld_ui_map_actions.showOpenWorldCampingMenu.001"),Array.from(arguments));
 const loc=worldLocation(state.world.location);if(state.world.settlements?.[loc.id])return actionResult(SOSText("openworld_ui_map_actions.showOpenWorldCampingMenu.002"),SOSText("openworld_ui_map_actions.showOpenWorldCampingMenu.003",loc.name),'info',renderOpenWorld);
 overlay(SOSText("openworld_ui_map_actions.showOpenWorldCampingMenu.004",esc(loc.name)),true);
 $('#campMake').onclick=makeRoadCamp;$('#campLife').onclick=showRoadLife;wireClose()
}
...
function openWorldTownActionsPanelHTML(loc,escort=false){
 if(escort||!isTownLikeLocation(loc))return '';
 return SOSText("openworld_ui_map_actions.openWorldTownActionsPanelHTML.001",esc(loc.name),loc.id==='shantium'?'<button id="worldCouncilHall"><b>Council Hall</b><small>Civic administration, law & public affairs</small></button>':'')
}
function openWorldNearbyPartiesPanelHTML(loc,escort=false){
 ...
 if(escort||!isTownLikeLocation(loc))return '';const list=nearbyWorldParties();if(!list.length)return '';
...
function renderOpenWorld(){SOSServices.navigation.reset();
 if(combat&&!combat.over)return renderCombat();normalize();ensureWorldState();sanitizeLoadedCampaignState();if(!validWorldLocationId(state.world.location)){state.world.location='shantium';state.world.region='shantium'}
 safeLoadRepair(SOSText("openworld_ui_map_actions.renderOpenWorld.001"),()=>repairEscortContracts(),[]);
 safeLoadRepair(SOSText("openworld_ui_map_actions.renderOpenWorld.002"),()=>repairActiveContractParties(),[]);
 safeLoadRepair(SOSText("openworld_ui_map_actions.renderOpenWorld.003"),()=>maintainWorldParties(),[]);
