...
function worldLocationIcon(loc){const tier=settlementTier(loc);return tier==='metropolis'?'♛':tier==='city'?'♜':tier==='town'?'▣':tier==='village'?'⌂':tier==='outpost'?'▲':loc.type==='fort'?'♜':loc.type==='ruins'?'✦':loc.type==='hidden'?(loc.minor?'·':'✧'):'◆'}
...
const REGION_MAP_ROAD_SEGMENTS={
 ...
 spawn:[
  // Main grid avenues: straight east-west / north-south district links.
  ['spawn_northwest','spawn_northgate'],['spawn_northgate','spawn_northcentral'],['spawn_northcentral','spawn_market'],
  ['spawn_west','spawn_works'],['spawn_works','spawn_civic'],['spawn_civic','spawn_merchant'],
  ['spawn_working','spawn_oldcity'],['spawn_oldcity','spawn_centralres'],['spawn_centralres','spawn_warehouses'],
  ['spawn_southwest','spawn_spirits'],['spawn_spirits','spawn_stockyards'],['spawn_stockyards','spawn_bazaar'],
  ['spawn_northwest','spawn_west'],['spawn_west','spawn_working'],['spawn_working','spawn_southwest'],
  ['spawn_northgate','spawn_works'],['spawn_works','spawn_oldcity'],['spawn_oldcity','spawn_spirits'],
  ['spawn_northcentral','spawn_civic'],['spawn_civic','spawn_centralres'],['spawn_centralres','spawn_stockyards'],
  ['spawn_market','spawn_merchant'],['spawn_merchant','spawn_warehouses'],['spawn_warehouses','spawn_bazaar'],
  // Secondary cross-streets: direct diagonal links between immediately adjacent city districts.
  ['spawn_northwest','spawn_works'],['spawn_northgate','spawn_west'],['spawn_northgate','spawn_civic'],['spawn_northcentral','spawn_works'],['spawn_northcentral','spawn_merchant'],['spawn_market','spawn_civic'],
  ['spawn_west','spawn_oldcity'],['spawn_works','spawn_working'],['spawn_works','spawn_centralres'],['spawn_civic','spawn_oldcity'],['spawn_civic','spawn_warehouses'],['spawn_merchant','spawn_centralres'],
  ['spawn_working','spawn_spirits'],['spawn_oldcity','spawn_southwest'],['spawn_oldcity','spawn_stockyards'],['spawn_centralres','spawn_spirits'],['spawn_centralres','spawn_bazaar'],['spawn_warehouses','spawn_stockyards']
 ]
};
...
function spawnRoadNetworkHTML(){
 const pairs=REGION_MAP_ROAD_SEGMENTS.spawn||[];
 const groups=pairs.map(([aId,bId])=>{
  const a=worldLocation(aId),b=worldLocation(bId);if(!a||!b)return'';
  const diagonal=Math.abs(a.x-b.x)>8&&Math.abs(a.y-b.y)>8;
  const cls=diagonal?'minor diagonal':'major';
  const d=`M ${a.x} ${a.y} L ${b.x} ${b.y}`;
  return `<g class="${cls}"><path class="road-shadow" d="${d}"/><path class="road-line" d="${d}"/><path class="road-highlight" d="${d}"/></g>`
 }).join('');
 return `<svg class="spawn-road-network" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">${groups}</svg>`
}
function regionTerrainHTML(region){
 ...
 const spawn=`<div class="spawn-city-grid"></div>${spawnRoadNetworkHTML()}<div class="map-region-name" style="left:38%;top:3%">NORTH CITY</div><div class="map-region-name" style="left:39%;bottom:3%">SOUTH CITY</div><div class="spawn-desert-edge">ENDLESS DESERT →</div><div class="map-compass"><b>N</b><span>✥</span></div>`;
 const raw=region==='farnorth'?farNorth:region==='spawn'?spawn:region==='bluestone'?SOSText("openworld_ui_map_actions.regionTerrainHTML.001"):region==='redstone'?SOSText("openworld_ui_map_actions.regionTerrainHTML.002"):SOSText("openworld_ui_map_actions.regionTerrainHTML.003");
 return applyRegionRoadStatusClasses(raw,region)
}
function arrangeTransientMapPosition(region,key,base,used,fixed=null){
 let x=clamp(base.x,2,98),y=clamp(base.y,2,98);fixed=fixed||locationsInRegion(region).filter(l=>state.world.discovered.includes(l.id));const collides=()=>fixed.some(l=>Math.hypot(x-l.x,y-l.y)<5.5)||used.some(u=>Math.hypot(x-u.x,y-u.y)<3.2);
...
function openWorldMapHTML(){
 ensureWorldState();const region=currentWorldRegion(),rd=regionDef(region),here=state.world.location,eq=activeEscortQuest(),tp=trackedWorldParty(),tq=state.world.quests.find(q=>q.id===state.world.trackedQuestId&&['active','ready'].includes(q.status)),used=[],fixedMapLocations=locationsInRegion(region).filter(l=>state.world.discovered.includes(l.id));
 ...
 for(const c of activeLiveRegionalConflicts()){if(c.region!==region)continue;const base=liveConflictPosition(c),a=state.world.parties.find(p=>p.id===c.aId),b=state.world.parties.find(p=>p.id===c.bId);if(!a||!b)continue;const pos=arrangeTransientMapPosition(region,'battle_'+c.id,base,used,fixedMapLocations);html+=`<button class="live-conflict-marker" data-liveconflict="${c.id}" style="left:${pos.x}%;top:${pos.y}%" title="Battle underway near ${esc(worldLocation(c.locId).name)}: ${esc(a.name)} vs ${esc(b.name)}">⚔</button>`}
 if(typeof warIntelMapRows==='function')for(const report of warIntelMapRows(region)){const base=warIntelMapPosition(report);if(!base)continue;const pos=arrangeTransientMapPosition(region,'army_'+report.formationId,base,used,fixedMapLocations),destId=warIntelProbableDestination(report),dest=destId?worldLocation(destId):null,age=report.mapAge||0,label=`${report.name||'Army'} — ${report.confidence||'reported'} intelligence — ${report.estimatedMin||'?'}–${report.estimatedMax||'?'} troops — ${age===0?'reported today':`reported ${age}d ago`}${dest?` — probable direction: ${dest.name}`:''}`;
  if(dest&&report.mapQuality>=45){html+=`<svg class="army-intel-route ${report.friendly?'friendly':'unknown'}" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><defs><marker id="armyArrow_${uiClassToken(report.formationId)}" markerWidth="5" markerHeight="5" refX="4" refY="2.5" orient="auto"><path d="M0,0 L5,2.5 L0,5 z"/></marker></defs><line x1="${pos.x}" y1="${pos.y}" x2="${dest.x}" y2="${dest.y}" marker-end="url(#armyArrow_${uiClassToken(report.formationId)})"/></svg>`}
  const identity=typeof warIntelKnownIdentity==='function'&&report.formation?warIntelKnownIdentity(report.formation,report):{name:report.name||'Army'};html+=`<button class="army-intel-marker map-positioned-marker ${report.friendly?'friendly':'unknown'} quality-${report.mapQuality>=75?'strong':report.mapQuality>=50?'credible':'uncertain'}" data-armyintel="${report.formationId}" style="left:${pos.x}%;top:${pos.y}%" title="${esc(identity.name)} — ${report.confidence||'reported'} intelligence — ${report.estimatedMin||'?'}–${report.estimatedMax||'?'} troops${dest?` — probable direction: ${dest.name}`:''}"><span>⚑</span><small>${report.estimatedMin||'?'}–${report.estimatedMax||'?'}</small></button>`}
 for(const p of state.world.parties){repairWorldPartyMapRoute(p);if(worldPartyDisplayRegion(p)!==region)continue;const base=worldPartyPosition(p),d=worldPartyDisposition(p);if(!p.contractProtected&&p.id!==state.world.trackedPartyId&&((d==='hostile'&&!state.world.mapFilters.hostile)||(d==='friendly'&&!state.world.mapFilters.friendly)||(!['hostile','friendly'].includes(d)&&!state.world.mapFilters.neutral)))continue;const pos=arrangeTransientMapPosition(region,'party_'+p.id,base,used,fixedMapLocations),dest=worldLocation(worldPartySafeLocationId(p.destination,p.location));html+=`<button class="world-party ${d} party-${p.kind} ${p.northernArchetype?'northern-warband':''} ${state.world.trackedPartyId===p.id?'tracked-party':''} ${p.contractProtected?'escort-caravan':''}" data-worldparty="${p.id}" style="left:${pos.x}%;top:${pos.y}%" title="${esc(p.name)} — ${esc(p.faction||'Unaffiliated')} — toward ${esc(dest.name)}"><span>${worldPartySymbol(p,d)}</span></button>`}
 const playerPos=playerPartyMapPosition(),playerField=playerPartyFieldState();if(playerPartyInField()&&playerField.region===region){if(playerField.previousRegion===region&&Number.isFinite(playerField.previousX)&&Number.isFinite(playerField.previousY)&&playerField.lastMoveDay>=state.world.day-1){html+=`<svg class="guardian-field-trail" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><path d="M ${playerField.previousX} ${playerField.previousY} Q ${(playerField.previousX+playerPos.x)/2+1.2} ${(playerField.previousY+playerPos.y)/2-1.1} ${playerPos.x} ${playerPos.y}"/></svg>`}html+=`<div class="player-party-marker" style="left:${playerPos.x}%;top:${playerPos.y}%" title="Guardian party — in the field"><span>G</span></div>`}
...
function showOpenWorldWorldLifeMenu(){modalRouteEnter(SOSText("openworld_ui_map_actions.showOpenWorldWorldLifeMenu.001"),Array.from(arguments));
 ...
 $('#worldLifeContacts').onclick=showRoadContactLedger;$('#worldLifeSocial').onclick=showSocialLifeJournal;$('#worldLifePopulation').onclick=showPopulationMovementJournal;$('#worldLifeWork').onclick=showRelationshipContractJournal;$('#worldLifeFactionPeople').onclick=()=>showFactionSocialNetwork(state.world.location,'world-life');wireClose()
}
function worldPoliticsControlName(f){return OPEN_WORLD_FACTIONS[f]?majorFaction(f).short:f||'Independent'}
function worldPoliticsSnapshot(){
 ensurePoliticalState();
 const regions=Object.keys(WORLD_REGIONS),worldCounts={},regionWins={};
 const rows=regions.map(region=>{
  const locs=regionalSettlements(region),counts={};
  for(const loc of locs){const control=settlementControl(loc.id)||'Independent';counts[control]=(counts[control]||0)+1;worldCounts[control]=(worldCounts[control]||0)+1}
  const ranked=Object.entries(counts).sort((a,b)=>b[1]-a[1]||worldPoliticsControlName(a[0]).localeCompare(worldPoliticsControlName(b[0]))),top=ranked[0]?.[1]||0,leaders=ranked.filter(x=>x[1]===top).map(x=>x[0]);
  const leader=leaders.length===1?leaders[0]:null;if(leader)regionWins[leader]=(regionWins[leader]||0)+1;
  return {region,locs,counts,ranked,leader,leaders,total:locs.length}
 });
 const worldRanked=Object.entries(worldCounts).sort((a,b)=>b[1]-a[1]||worldPoliticsControlName(a[0]).localeCompare(worldPoliticsControlName(b[0]))),regionRanked=Object.entries(regionWins).sort((a,b)=>b[1]-a[1]||worldPoliticsControlName(a[0]).localeCompare(worldPoliticsControlName(b[0])));
 return {rows,worldCounts,worldRanked,regionWins,regionRanked,totalSettlements:rows.reduce((n,r)=>n+r.total,0)}
}
function showWorldPolitics(){modalRouteEnter('showWorldPolitics',Array.from(arguments));
 const snap=worldPoliticsSnapshot(),topRegions=snap.regionRanked[0]?.[1]||0,regionLeaders=snap.regionRanked.filter(x=>x[1]===topRegions),topSettlements=snap.worldRanked[0]?.[1]||0,settlementLeaders=snap.worldRanked.filter(x=>x[1]===topSettlements);
 const worldLead=regionLeaders.length===1?`${esc(worldPoliticsControlName(regionLeaders[0][0]))} controls the plurality in <b>${regionLeaders[0][1]}</b> region${regionLeaders[0][1]===1?'':'s'}.`:regionLeaders.length?`Regional control is tied: ${regionLeaders.map(x=>`${esc(worldPoliticsControlName(x[0]))} (${x[1]})`).join(' • ')}`:'No faction currently holds a regional plurality.';
 const settlementLead=settlementLeaders.length===1?`${esc(worldPoliticsControlName(settlementLeaders[0][0]))} controls the most settlements worldwide: <b>${settlementLeaders[0][1]}</b>.`:settlementLeaders.length?`Worldwide settlement control is tied: ${settlementLeaders.map(x=>`${esc(worldPoliticsControlName(x[0]))} (${x[1]})`).join(' • ')}`:'No settlement control data is available.';
 const totals=snap.worldRanked.map(([f,n])=>`<div class="stat-row"><span>${esc(worldPoliticsControlName(f))}</span><b>${n} settlement${n===1?'':'s'} • ${snap.regionWins[f]||0} regional pluralit${(snap.regionWins[f]||0)===1?'y':'ies'}</b></div>`).join('')||'<p class="muted">No political control data available.</p>';
 const regions=snap.rows.map(r=>{const lead=r.leader?`${esc(worldPoliticsControlName(r.leader))} leads`:r.leaders.length?`Tied: ${r.leaders.map(f=>esc(worldPoliticsControlName(f))).join(' / ')}`:'No settled control';const counts=r.ranked.map(([f,n])=>`${esc(worldPoliticsControlName(f))} ${n}`).join(' • ');const settlements=r.locs.map(l=>`${esc(l.name)} — <b>${esc(worldPoliticsControlName(settlementControl(l.id)))}</b>`).join('<br>');return `<div class="card"><div class="stat-row"><span><b>${esc(regionDef(r.region).name)}</b></span><b>${lead}</b></div><small>${r.total} political settlement${r.total===1?'':'s'}${counts?` • ${counts}`:''}</small><details><summary>Settlement control</summary><div class="compact">${settlements||'<span class="muted">No settlements tracked.</span>'}</div></details></div>`}).join('');
 overlay(`<h2>World Politics</h2><p class="muted">A world-scale summary of formal political control. A regional leader is the faction controlling more settlements in that region than any rival; ties remain contested.</p><div class="notice compact">${worldLead}<br>${settlementLead}<br><small>${snap.totalSettlements} political settlements tracked across ${snap.rows.length} regions.</small></div><h3>Worldwide Balance</h3><div class="card">${totals}</div><h3>Regional Control</h3>${regions}<div class="dialog-footer"><button id="worldPoliticsBack">Back</button></div>`,true);
 $('#worldPoliticsBack').onclick=()=>{resetModalNavigation();closeOverlay();renderOpenWorld()}
}
...
function regionalPartyCardHTML(p){const d=worldPartyDisposition(p),type=worldPartyTypeLabel(p),faction=p.faction||'Unaffiliated',est=worldPartyTravelEstimate(p),loc=worldLocation(p.location),inReach=canEngageWorldParty(p),behavior=typeof worldPartyBehavior==='function'?worldPartyBehavior(p):{label:p.behaviorLabel||p.purpose||type};return `<button class="regional-party-card disposition-${uiClassToken(d)} faction-${uiClassToken(faction)}" data-regionalparty="${p.id}"><span><b>${esc(p.name)}</b><small>${esc(type)} • ${esc(faction)} • ${esc(behavior.label)}</small></span><span class="regional-party-meta">${inReach?'<b>IN RANGE</b>':`<b>${esc(est.label)}</b>`}<small>${esc(loc?.name||'On the road')} • ${esc(d)}${esc(regionalPartyMeetingLabel(p))}</small></span></button>`}
...
function openWorldActionsHTML(){
 const eq=activeEscortQuest(),loc=worldLocation(state.world.location),tq=state.world.quests.find(q=>q.id===state.world.trackedQuestId&&['active','ready'].includes(q.status)),tp=trackedWorldParty(),P=typeof pursuitState==='function'?pursuitState():null,pursuitTarget=P?.targetId&&!P.caught?state.world.parties.find(x=>x.id===P.targetId):null,regional=regionalTravelSummary();
 if(eq){
 ...
 return SOSText("openworld_ui_map_actions.openWorldActionsHTML.002",openWorldPlayerActionsHTML(),liveBattleAlertHTML(),homeHomecomingSummaryHTML(),pursuitTarget?`<div class="warning notice compact"><b>Active pursuit: ${esc(pursuitTarget.name)}</b><br><small>Closing progress ${Math.floor(P.progress||0)}/${pursuitClosingNeed(pursuitTarget)}.</small><br><button id="resumePursuit" class="mini-action">Resume Pursuit</button></div>`:(tp?`<div class="notice compact"><b>Tracked party: ${esc(tp.name)}</b><br><small>${esc(worldPartyTravelEstimate(tp).label)}</small><br><button id="openTrackedParty" class="mini-action">Open Party Report</button></div>`:''),tq?`<div class="notice compact"><b>Tracked contract: ${esc(tq.name)}</b><br><small>${esc(contractObjective(tq))}</small><br><button id="openTrackedContract" class="mini-action">Open Contract</button></div>`:'',regional?`<div class="region-crossing-hint"><b>Regional travel:</b> ${esc(regional)}</div>`:'',state.world.day,esc(loc.name))
}
...
function isTownLikeLocation(loc){return !!loc&&['town','settlement','camp','fort','district'].includes(loc.type)}
function openWorldTownActionsPanelHTML(loc,escort=false){
 if(escort||playerPartyInField()||!isTownLikeLocation(loc))return '';
 const spawnDistrict=locationRegion(loc.id)==='spawn';
 return SOSText("openworld_ui_map_actions.openWorldTownActionsPanelHTML.001",esc(loc.name),loc.id==='shantium'?'<button id="worldCouncilHall"><b>Council Hall</b><small>Civic administration, law & public affairs</small></button>':'',`<button id="worldLeaveSettlement" class="leave-settlement-action"><b>${spawnDistrict?'Leave District':`Leave ${esc(loc.name)}`}</b><small>${spawnDistrict?'Move the Player Party away from this district while remaining within the Spawn.':`Exit the settlement and move the Player Party into the nearby field.`}</small></button>`)
}
...
function scrollToCurrentTownSection(){
 ...
 const section=document.getElementById(SOSText("openworld_ui_map_actions.scrollToCurrentTownSection.001"));if(!section)return renderOpenWorld();
...
function renderOpenWorld(){SOSServices.navigation.reset();
 if(combat&&!combat.over)return renderCombat();if(typeof recoverPendingRegionalJourney==='function'&&recoverPendingRegionalJourney())return;if(typeof recoverStaleDirectTravelPlan==='function'&&recoverStaleDirectTravelPlan())return;if(typeof recoverInterruptedPursuit==='function')recoverInterruptedPursuit();sosFastOpenWorldStateReady();scheduleOpenWorldMaintenance();
