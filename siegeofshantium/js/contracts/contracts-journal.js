...
function contractIssuerName(q){return worldLocation(q.origin)?.name||SOSText("contracts_contracts_journal.contractIssuerName.001")}
...
function generateContract(origin,forcedType=null,opts={}){
 ...
 assignContractPaymentTerms(q,{spot:false});
 return q
}
...
function refreshContracts(){
 ...
 for(const loc of availableQuestLocations()){
   ...
   state.world.contracts[loc.id]=arr.filter(q=>q.status==='offered'&&(!q.regional||regionalContractValid(q))).sort((a,b)=>((b.politicalCampaign?4:0)+(b.referral?1:0))-((a.politicalCampaign?4:0)+(a.referral?1:0))).slice(0,2);
   ensureRegionalContractOffer(loc.id);
...
function acceptQuest(id,loc){
 const q=(state.world.contracts[loc]||[]).find(x=>x.id===id);if(!q||state.world.quests.filter(x=>['active','ready'].includes(x.status)).length>=4)return;
 q.status='active';q.acceptedDay=state.world.day;if(q.workOfferId)updateWorldWorkOffer(q.workOfferId,{status:'open',acceptedDay:state.world.day});
 if(q.type==='escort'){q.dueDay=state.world.day+Math.max(contractTemplate(q.type)?.days||9,q.escortTotalDays+4);const p=createEscortCaravan(q);q.escortStage='rendezvous';p.contractExpiresDay=q.dueDay;state.world.trackedPartyId=p.id}
 else{q.dueDay=state.world.day+Math.max(contractTemplate(q.type)?.days||7,q.crossRegion?tradeRouteDistanceDays(q.origin,q.target)+4:0);if(['hunt','recovery'].includes(q.type)){const p=createContractTargetParty(q);if(p){p.contractExpiresDay=q.dueDay;state.world.trackedPartyId=p.id}}}
...
function maybeCreateFollowupContract(q){
 ...
 if(level<2||q.followup||level%2!==0)return null;
...
function finalizeQuestPayment(q,opts={}){
 const bonus=contractPerformanceBonus(q),payout=q.reward+bonus;q.status='complete';q.completedDay=state.world.day;q.payout=payout;gainGold(payout);state.reputation++;state.world.contractStats.completed++;if(q.earlyBonus)state.world.contractStats.early++;
...
function failEscortContract(q,reason,abandoned=false){
 ...
 state.world.contractStats[abandoned?'abandoned':'failed']++;state.world.factionStanding[q.faction]=(state.world.factionStanding[q.faction]||0)-(abandoned?2:1);if(abandoned)state.reputation=Math.max(0,(state.reputation||0)-1);applyPoliticalCampaignFailure(q,reason,abandoned);releaseEscortCaravan(q,false);save()
}
 ...
 for(const q of state.world.quests.filter(x=>x.status==='active')){
   ...
   if(left<0){
     ...
     if(q.type==='escort'){failEscortContract(q,reason,false);queueContractFailureNotice(q,reason,1)}
       ...
       state.world.factionStanding[q.faction]=(state.world.factionStanding[q.faction]||0)-1;applyPoliticalCampaignFailure(q,reason,false);if(q.politicalCampaign)q.politicalOutcome={before:wholePoliticalBefore,after:politicalOutcomeSnapshot(q.politicalLocId||q.origin,[q.politicalFaction||q.faction,q.politicalRival]),notes:[reason]};queueContractFailureNotice(q,reason,1)
     }
   }
 }
}
...
function showDiplomacyDecision(q){modalRouteEnter(SOSText("contracts_contracts_journal.showDiplomacyDecision.001"),Array.from(arguments));
 ...
 $('#termsBack').onclick=()=>showContractDetails(q.id)
}
...
function showContractDetails(id){modalRouteEnter(SOSText("contracts_contracts_journal.showContractDetails.001"),Array.from(arguments));
 ...
 overlay(SOSText("contracts_contracts_journal.showContractDetails.002",esc(q.name),esc(contractStatusLabel(q)),esc(contractObjective(q)),politicalCampaignSourceHTML(q),q.regional?`<div class="regional-contract-source"><b>Why this contract exists</b><br>${esc(regionalContractReason(q))}${regionalContractImpactText(q)?`<br><small>Expected regional effect: ${esc(regionalContractImpactText(q))}</small>`:''}${q.matterId?worldMatterChannelLabel(q.matterId,'contract'):''}</div>`:'',relationshipContractSourceHTML(q),interregionalContractRouteHTML(q),esc(contractVariantLabel(q)),esc(contractIssuerName(q)),esc(contractTargetName(q)),esc(contractPaymentSummary(q)),esc(contractProgress(q)),q.reward,q.status==='complete'?`<div class="stat-row"><span>Performance</span><b>${esc(contractPerformanceText(q))}</b></div><div class="stat-row"><span>Paid</span><b>${q.payout||q.reward}g</b></div>`:'',q.acceptedDay?`Day ${q.acceptedDay}`:'Not accepted',q.dueDay?`Day ${q.dueDay}${days!=null&&q.status==='active'?` • ${Math.max(0,days)} day${days===1?'':'s'} left`:''}`:'—',chain?`<div class="stat-row"><span>Issuer history</span><b>${chain} completed job${chain===1?'':'s'} in this line</b></div>`:'',primary,['active','ready'].includes(q.status)?`<button id="trackContract">${state.world.trackedQuestId===q.id?'Stop Tracking':'Track Contract'}</button>`:'',['active','ready'].includes(q.status)&&!(q.type==='escort'&&q.escortStage==='escorting')?'<button id="abandonContract">Abandon Contract…</button>':''));
