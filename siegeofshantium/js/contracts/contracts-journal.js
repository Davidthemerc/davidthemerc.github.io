...
function endContractRefreshReadCache(){SOSContractRefreshReadCache=null}
function contractRefreshReadCache(){return SOSContractRefreshReadCache&&SOSContractRefreshReadCache.state===state&&SOSContractRefreshReadCache.day===state.world.day?SOSContractRefreshReadCache:null}
function contractWorldLinkedValid(q){
 if(!q?.worldLinked)return true;
 if(q.worldPartyId){const c=contractRefreshReadCache(),p=c?.partiesById?.get(q.worldPartyId)||state.world.parties.find(x=>x.id===q.worldPartyId);if(!p)return false;if(q.status==='offered'&&q.origin&&p.location!==q.origin)return false}
...
function refreshContracts(){
 ...
 try{for(const loc of locs){
   ...
   t=now();ensureWorldLinkedContractOffers(loc.id);let dt=now()-t;sub.worldLinked+=dt;sub.linked+=dt;t=now();ensureRegionalContractOffer(loc.id);dt=now()-t;sub.regionalLinked+=dt;sub.linked+=dt;
   t=now();if(crossRegionTradeUnlocked()&&!state.world.contracts[loc.id].some(q=>q.crossRegion)&&chance((state.world.unlockedRegions||[]).length>=3?.42:.22)){const cq=chance(.6)?crossRegionSupplyContract(loc.id):generateContract(loc.id,pick(['delivery','escort','diplomacy','visit']),{crossRegion:true});if(cq)state.world.contracts[loc.id].push(cq)}sub.cross+=now()-t;
   t=now();const ss=settlementState(loc.id),desired=ss.prosperity>=70&&ss.security>=55?3:ss.prosperity<35||ss.security<30?2:3;
...
function acceptQuest(id,loc,termsConfirmed=false){
 ...
 state.world.quests.push(q);state.world.contracts[loc]=state.world.contracts[loc].filter(x=>x.id!==id);state.world.trackedQuestId=q.id;sfxWorld('questAccept');log(SOSText("contracts_contracts_journal.acceptQuest.001",q.name),'good');save();
...
function turnInQuest(q,after=renderOpenWorld){
 ...
 sfxWorld('questComplete');actionResult(SOSText("contracts_contracts_journal.turnInQuest.005"),SOSText("contracts_contracts_journal.turnInQuest.006",q.name,q.paymentPayer||'the authorized payer',worldLocation(contractPaymentLocation(q)).name,payout,bonus?` Bonus: ${bonus}g.`:'',politicalImpact?`\n\nPolitical result: ${politicalImpact}.`:'',`${regionalImpact?`\n\nRegional impact: ${regionalImpact}.`:''}${worldImpact?`\n\nWorld follow-through: ${worldImpact}.`:''}`,follow?` New work is available in ${contractIssuerName(q)}.`:''),'good',after)
}
...
function showWorldJournal(){if(typeof refreshRuntimeJavascriptErrorsFromStorage==='function')refreshRuntimeJavascriptErrorsFromStorage();modalRouteEnter(SOSText("contracts_contracts_journal.showWorldJournal.001"),Array.from(arguments));
