...
function homePrisonerLaborGroups(){return (state.prisoners||[]).filter(p=>p.world&&p.hallCustody&&!homePrisonerIsSpecial?.(p))}
// v1.6.65.28: crew state is canonical. Reads must not trigger a full restaff.
function homePrisonerLaborAssigned(){if(typeof homePrisonerWorkState==='function')return homePrisonerWorkState().crews.reduce((n,c)=>n+(c.staffed||0),0);return homePrisonerLaborGroups().reduce((n,p)=>n+(p.count||0),0)}
function homePrisonerLaborCapacity(){ensureHomeBase();const lvl=Math.max(0,Math.min(3,homeUpgradeLevel('prisonerWorks')||0));return [10,24,44,64][lvl]||10}
...
function homeFinanceDailyTick(){if(!isOpenWorld())return;ensureHomeBase();const perf=(name,fn)=>typeof sosPerfRun==='function'?sosPerfRun(name,fn):fn();perf('Finance — Prisoner Labor',()=>homePrisonerLaborDailyIncome());perf('Finance — Facility Services',()=>homeFacilityServiceIncomeTick());perf('Finance — Commercial Operations',()=>homeCommercialDailyTick());perf('Finance — Business Operations',()=>homeBusinessDailyTick())}
...
function homeHallEconomyOutlook(){
 ensureHomeBase();const h=state.world.homeBase,pros=Math.max(0,Math.min(100,settlementState('shantium')?.prosperity||50)),B=h.business||{},A=h.artisan||{},T=h.staff||{},S=h.security||{},L=h.logistics||{};
 const staff14=typeof homeStaffPayroll==='function'?homeStaffPayroll():0,artisan14=(A.master?.salary||0)+(A.workmen||0)*32,leaders14=(B.adviser?.salary||0)+(L.master?.salary||0)+(S.headGuard?.salary||0);
 const payrollDay=Math.round(((staff14+artisan14+leaders14)/14)*10)/10;
 const prisoners=homePrisonerLaborAssigned(),band=homePrisonerLaborProsperityBand(pros),prisonerDay=(typeof homePrisonerWorkState==='function'&&typeof homePrisonerCrewDailyValue==='function')?homePrisonerWorkState().crews.reduce((n,c)=>n+homePrisonerCrewDailyValue(c),0):Math.min(prisoners,homePrisonerLaborCapacity())*band.rate;
 const facilityScore=homeBusinessFacilityScore(),spare=homeBusinessSpareCapacity(),adv=B.adviser?.competence||0,premium=pros>=85?8:pros>=70?4:0;
 const routineGross=B.adviser?Math.max(4,Math.round(adv*5+facilityScore*1.55+spare*1.15+pros/15+premium+.5))/2:0;
 const strat=HALL_BUSINESS_STRATEGIES[B.strategy]||HALL_BUSINESS_STRATEGIES.balanced,routineHall=Math.round(routineGross*strat.hallShare*10)/10;
 const serviceBand=pros>=85?'strong':pros>=70?'healthy':pros>=55?'steady':pros>=35?'limited':'weak';
 return {pros,payrollDay,prisoners,prisonerDay,routineHall,serviceBand,hallShare:Math.round(strat.hallShare*100),
  note:pros>=85?'Exceptional prosperity supports a mature Hall, but commercial work is intentionally paced to keep surplus moderate.':pros>=70?'Prosperous Shantium should let a developed Hall cover routine operations when its revenue systems are actually used.':'At lower prosperity, the Hall may need reserves, prisoner-work income, or selective commercial work to cover a large establishment.'}
}
...
function showHomeBudget(){modalRouteEnter(SOSText("hall_business_finance_logistics.showHomeBudget.001"),Array.from(arguments));guardianHallRouteEnter('showHomeBudget',[]);ensureHomeBase();const h=state.world.homeBase,L=h.logistics,F=h.finance,w7=homeFinanceWindow(7),w30=homeFinanceWindow(30),assigned=homePrisonerLaborAssigned(),cap=homePrisonerLaborCapacity(),max=Math.max(0,state.gold),suggest=Math.min(max,Math.max(0,Math.min(500,max))),fmt=n=>`${n>=0?'+':''}${n}g`,recent=F.ledger.slice(-36).reverse();
 ...
 const incomeRows=homeFinanceCategoryBreakdown(w30.rows,'income'),expenseRows=homeFinanceCategoryBreakdown(w30.rows,'expense'),categoryHTML=(rows,empty)=>rows.length?rows.slice(0,6).map(([name,amount])=>`<div class="budget-category-row"><span>${esc(name)}</span><b>${fmtGold(amount)}g</b></div>`).join(''):`<p class="muted compact">${esc(empty)}</p>`;
 const health=homeHallOperatingHealth(30),coveragePct=Math.round(health.coverage*100),outlook=homeHallEconomyOutlook();
 const sourceHTML=`<div class="card compact"><h4>Operating Self-Sufficiency</h4><div class="stat-row"><span>30-day status</span><b class="${health.tone}">${esc(health.label)}</b></div><div class="stat-row"><span>Income coverage</span><b>${coveragePct}% of operating expenses</b></div><div class="stat-row"><span>Operating result / day</span><b class="${health.netPerDay>=0?'good-text':'danger-text'}">${health.netPerDay>=0?'+':''}${fmtGold(health.netPerDay)}g</b></div><div class="stat-row"><span>Shantium prosperity</span><b>${fmtStat(health.prosperity)}/100</b></div><small>${esc(health.target)}</small></div><div class="card compact"><h4>Current Operating Outlook</h4><div class="stat-row"><span>Recurring payroll / day</span><b>${fmtGold(outlook.payrollDay)}g</b></div><div class="stat-row"><span>Prisoner-work income / day</span><b>${fmtGold(outlook.prisonerDay)}g</b></div><div class="stat-row"><span>Routine Adviser share / day</span><b>~${fmtGold(outlook.routineHall)}g</b></div><div class="stat-row"><span>Hall share policy</span><b>${outlook.hallShare}% retained</b></div><div class="stat-row"><span>Local service market</span><b>${esc(outlook.serviceBand)}</b></div><small>${esc(outlook.note)}</small></div><div class="stat-row"><span>Prisoner work crews</span><b>${assigned?`${assigned} assigned / ${cap} capacity`:'None assigned'}</b></div><div class="stat-row"><span>Workshop services</span><b>${h.upgrades.workshop?'Active':'Not established'}</b></div><div class="stat-row"><span>Training fees</span><b>${h.upgrades.trainingYard?'Active':'Not established'}</b></div><div class="stat-row"><span>Archive & scribe services</span><b>${h.upgrades.archives?'Active':'Not established'}</b></div><div class="stat-row"><span>Hosted events</span><b>${h.upgrades.banquetFacilities?'Active':'No major facilities'}</b></div><div class="stat-row"><span>Economic Adviser</span><b>${h.business?.adviser?`${esc(h.business.adviser.name)} • active`:'Position vacant'}</b></div>`;
 ...
 const ledgerHTML=recent.map(x=>`<div class="budget-ledger-row ${esc(x.type)}"><span><b>Day ${x.day}</b><small>${esc(x.category||'Hall account')} • ${esc(x.text||'')}</small></span><b>${x.type==='income'?'+':x.type==='expense'?'-':x.type==='capital'?'◆ ':x.type==='capital_return'?'↩ ':'↔ '}${fmtGold(x.amount)}g</b></div>`).join('')||'<p class="muted">No Hall account activity has been recorded yet.</p>';
...
function homeGuardSupplyDailyUse(){ensureHomeBase();return (state.world.homeBase.security?.guards||0)>0?2:0}
function homeRoutineSupplyOrderCost(){ensureHomeBase();const p=homeLogisticsPolicy(),guards=Math.max(0,state.world.homeBase.security?.guards||0);return p.cost+Math.ceil(Math.max(0,guards-12)/12)*3}
...
function completeHomeLogisticsShipment(p){if(!p?.logisticsShipment||!state.world?.homeBase)return;ensureHomeBase();const L=state.world.homeBase.logistics;if(L.shipmentId!==p.id)return;const delivered=homeApplySupplyDelivery(28+homeUpgradeLevel(SOSText("hall_business_finance_logistics.completeHomeLogisticsShipment.001"))*6,'physical_caravan');L.shipmentId=null;L.disruption=null;p.logisticsShipment=false;homeLogisticsReport(`${SOSText("hall_business_finance_logistics.completeHomeLogisticsShipment.002",p.name)}${delivered?.stableAdded?` Stable supplies +${Math.round(delivered.stableAdded)}.`:''}`,'good');recordWorldHistory(SOSText("hall_business_finance_logistics.completeHomeLogisticsShipment.003"),'good','home')}
...
function showHomeTradeProcurement(tab='active'){
 ...
 if(tab==='goods'){
   ...
   const allPlans=homeTradeProcurementPlans().slice().reverse(),activePlans=allPlans.filter(p=>!['complete','completed','failed','cancelled'].includes(p.status)),historyPlans=allPlans.filter(p=>['complete','completed','failed','cancelled'].includes(p.status)).slice(0,12),plans=activePlans.map(p=>{const lines=p.lines.map(l=>`${worldGood(l.goodId)?.name||l.goodId} ${l.acquired||0}/${l.qty}`).join(' • '),remaining=homePlanRemainingLines(p).reduce((n,l)=>n+l.remaining,0);return `<div class="proc-plan-card"><div><b>Mixed Procurement Plan</b><span>${String(p.status||'queued').replaceAll('_',' ').toUpperCase()}</span></div><small>${esc(lines)}<br>${HOME_TRADE_PROCUREMENT_PRIORITIES[homeNormalizeProcPriority(p.priority)].name} priority • ${HOME_TRADE_PROCUREMENT_SECURITY[p.security||'auto'].name}${remaining?` • ${remaining} units still awaiting dispatch/acquisition`:''}</small></div>`}).join('');
   const allOrders=homeTradeProcurementOrders().slice().reverse(),activeIds=new Set(homeTradeProcurementActive().map(o=>o.id)),activeOrders=allOrders.filter(o=>activeIds.has(o.id)),historyOrders=allOrders.filter(o=>!activeIds.has(o.id)).slice(0,24);const card=o=>{const p=o.partyId?state.world.parties.find(x=>x.id===o.partyId):null,lines=homeProcurementLines(o),manifest=lines.map(l=>`${worldGood(l.goodId)?.name||l.goodId} ${l.acquired||0}/${l.qty}`).join(' • ');return `<div class="proc-caravan-card"><div><b>${esc(homeProcurementOrderLabel(o))}</b><span>${homeTradeOrderStatusLabel(o)}</span></div><small>${esc(manifest)}<br>${HOME_TRADE_PROCUREMENT_PRIORITIES[homeNormalizeProcPriority(o.priority)].name} priority • ${HOME_TRADE_PROCUREMENT_SECURITY[o.security||'standard']?.name||'Standard'} • contract ${o.cost}g${p?`<br>${esc(worldLocation(p.location).name)} → ${esc(worldLocation(p.destination).name)} • ~${p.travelLeft}d`:''}</small>${p?`<button data-trackproc="${p.id}">Track Caravan</button>`:''}</div>`};const activeHTML=plans+activeOrders.map(card).join('')||'<p class="muted">No procurement caravans are currently active.</p>',historyPlanHTML=historyPlans.map(p=>{const lines=p.lines.map(l=>`${worldGood(l.goodId)?.name||l.goodId} ${l.acquired||0}/${l.qty}`).join(' • ');return `<div class="proc-plan-card"><div><b>Mixed Procurement Plan</b><span>${String(p.status||'complete').replaceAll('_',' ').toUpperCase()}</span></div><small>${esc(lines)}</small></div>`}).join(''),historyHTML=historyPlanHTML+historyOrders.map(card).join('')||'<p class="muted">No completed procurement history yet.</p>';body=`<h3>Active Procurements</h3>${activeHTML}<details class="proc-history-collapsible"><summary><b>Procurement History</b> <small>${historyOrders.length} recent record${historyOrders.length===1?'':'s'}</small></summary><div class="proc-history-body">${historyHTML}</div></details>`
 }
...
// v1.6.61 — Economic Adviser owns Crystal Ward financial reporting.
function crystalFinancialWindow(days=30){if(typeof crystalCommercialState!=='function')return null;const X=crystalCommercialState(),from=Math.max(0,state.world.day-days+1),rows=(X.ledger||[]).filter(x=>x.day>=from),gross=rows.filter(x=>x.amount>0).reduce((n,x)=>n+x.amount,0),costs=Math.abs(rows.filter(x=>x.amount<0).reduce((n,x)=>n+x.amount,0)),stats=(X.dailyStats||[]).filter(x=>x.day>=from),util=stats.length?Math.round(stats.reduce((n,x)=>n+(x.capacity?x.active/x.capacity:0),0)/stats.length*100):0,unmet=(X.demandQueue||[]).filter(d=>d.status==='unmet'&&d.expiresDay>=from).length;return {days,gross,costs,net:gross-costs,util,unmet,active:X.bookings.filter(b=>b.type==='rental'&&['active','late','in_transit'].includes(b.status)).length,capacity:X.crystals.length}}
function showCrystalEconomicReport(){const h=state.world.homeBase,B=h.business;if(!B?.adviser)return actionResult('Economic Adviser Required','Hire an Economic Adviser before reviewing Crystal Ward financial performance.','info',showHomeBusiness);const X=crystalCommercialState(),w7=crystalFinancialWindow(7),w30=crystalFinancialWindow(30),factions={};for(const b of X.bookings||[]){if(!b.faction||!b.fee)continue;factions[b.faction]=(factions[b.faction]||0)+b.fee}const concentration=Object.entries(factions).sort((a,b)=>b[1]-a[1])[0],waiting=X.demandQueue.filter(d=>d.status==='waiting').length,avail=crystalAvailableUnits().length,recommend=waiting>avail?`Current unmet demand supports considering ${Math.min(3,Math.max(1,waiting-avail))} additional portable crystal${waiting-avail===1?'':'s'}.`:(w30.util>=80?'Utilization is high enough to consider one additional portable crystal.':'Current capacity appears adequate; further commissioning is not presently necessary.');overlay(`<h2>Economic Adviser — Crystal Ward Commercial Report</h2><p>${esc(B.adviser.name)} reviews the financial performance of Guardian Hall's portable Sending Crystal business.</p><div class="two-col"><div class="card"><h3>Last 7 Days</h3><div class="stat-row"><span>Gross revenue</span><b>${fmt(w7.gross)}g</b></div><div class="stat-row"><span>Recorded costs</span><b>${fmt(w7.costs)}g</b></div><div class="stat-row"><span>Net</span><b>${fmt(w7.net)}g</b></div><div class="stat-row"><span>Average utilization</span><b>${w7.util}%</b></div></div><div class="card"><h3>Last 30 Days</h3><div class="stat-row"><span>Gross revenue</span><b>${fmt(w30.gross)}g</b></div><div class="stat-row"><span>Recorded costs</span><b>${fmt(w30.costs)}g</b></div><div class="stat-row"><span>Net</span><b>${fmt(w30.net)}g</b></div><div class="stat-row"><span>Average utilization</span><b>${w30.util}%</b></div><div class="stat-row"><span>Unmet requests</span><b>${w30.unmet}</b></div></div></div><div class="card"><h3>Adviser Assessment</h3><p>${esc(recommend)}</p>${concentration?`<p><b>Client concentration:</b> ${esc(concentration[0])}-affiliated contracts account for ${fmt(concentration[1])}g of recorded contract value.</p>`:''}<p><b>Current operating position:</b> ${w30.active} committed / ${w30.capacity} commissioned; ${waiting} waiting request${waiting===1?'':'s'}.</p></div><div class="dialog-footer"><button id="crystalEconomicBack">Back to Hall Business</button></div>`,true);$('#crystalEconomicBack').onclick=showHomeBusiness}
showHomeBusiness=(function(base){return function(){const r=base.apply(this,arguments),B=state.world.homeBase.business;if(B?.adviser&&typeof crystalCommercialState==='function'&&crystalCommercialState().enabled){const dlg=document.querySelector('.dialog'),anchor=document.getElementById('businessOpportunities');if(dlg&&anchor&&!document.getElementById('crystalEconomicReport')){const b=document.createElement('button');b.id='crystalEconomicReport';b.classList.add('hall-action-card');b.innerHTML='<b>Crystal Ward Commercial Report</b><small>Revenue, costs, utilization, unmet demand, concentration risk and capacity recommendation.</small>';b.onclick=showCrystalEconomicReport;anchor.parentNode.insertBefore(b,anchor.nextSibling)}}return r}})(showHomeBusiness);

// v1.6.65 — Economic Adviser: financial consequences of Crystal Ward scarcity remain here, not in Ward operations.
if(typeof showCrystalEconomicReport==='function'){
 const __crystalScarcityEconomicReport=showCrystalEconomicReport;
 showCrystalEconomicReport=function(){const r=__crystalScarcityEconomicReport.apply(this,arguments),dlg=document.querySelector('.dialog');if(!dlg||typeof crystalCommercialState!=='function')return r;const X=crystalCommercialState(),S=typeof crystalScarcityState==='function'?crystalScarcityState():{deniedValue:0},waiting=(X.demandQueue||[]).filter(d=>d.status==='waiting'&&d.expiresDay>=state.world.day),queueValue=waiting.reduce((n,d)=>n+(X.dailyRate||250)*(d.days||1)*(d.units||1),0),reserve=Math.max(0,Number(X.policy?.reserve)||0),util=crystalFinancialWindow(30)?.util||0,estIdle=Math.round((X.dailyRate||250)*reserve*Math.max(.15,util/100)*7),footer=dlg.querySelector('.dialog-footer');if(!document.getElementById('crystalScarcityFinance')){const box=document.createElement('div');box.id='crystalScarcityFinance';box.className='card';box.innerHTML=`<h3>Scarcity & Capacity Economics</h3><div class="stat-row"><span>Current qualified queue value</span><b>${fmt(queueValue)}g</b></div><div class="stat-row"><span>Recorded forgone contract value</span><b>${fmt(Number(S.deniedValue)||0)}g</b></div><div class="stat-row"><span>Emergency reserve</span><b>${reserve} crystal${reserve===1?'':'s'}</b></div><div class="stat-row"><span>Estimated 7-day opportunity cost of reserve</span><b>${fmt(estIdle)}g</b></div><p class="muted">Opportunity cost is an adviser estimate based on recent utilization; it is not an expense charged to the Hall.</p>`;dlg.insertBefore(box,footer||null)}return r}
}


// v1.6.66.9.23 — make Crystal Ward participation in the Hall business-profit policy explicit.
if(typeof showCrystalEconomicReport==='function'){
 const __crystalDistributionEconomicReport=showCrystalEconomicReport;
 showCrystalEconomicReport=function(){
  const r=__crystalDistributionEconomicReport.apply(this,arguments),dlg=document.querySelector('.dialog');
  if(!dlg||typeof crystalCommercialDistributionSummary!=='function')return r;
  const q=crystalCommercialDistributionSummary(),footer=dlg.querySelector('.dialog-footer');
  if(!document.getElementById('crystalProfitDistribution')){
   const box=document.createElement('div');box.id='crystalProfitDistribution';box.className='card';
   box.innerHTML=`<h3>Profit Distribution</h3><div class="stat-row"><span>Current policy</span><b>${esc(q.strategy)}</b></div><div class="stat-row"><span>Hall / Guardian share</span><b>${q.hallPct}% / ${q.guardianPct}%</b></div><div class="stat-row"><span>Lifetime net commercial profit</span><b>${fmt(q.net)}g</b></div><div class="stat-row"><span>Guardian share paid</span><b>${fmt(q.guardianPaid)}g</b></div>${q.guardianOwed?`<div class="stat-row"><span>Guardian share awaiting settlement</span><b>${fmt(q.guardianOwed)}g</b></div>`:''}<p class="muted">Crystal Ward commercial profit follows the Economic Adviser’s Hall business distribution policy. Operating revenue and costs are settled through Hall accounts first; the Guardian’s share is then paid from net profit.</p>`;
