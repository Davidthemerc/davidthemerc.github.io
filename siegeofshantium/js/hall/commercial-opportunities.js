...
const HOME_COMMERCIAL_OPPORTUNITY_DEFS={
 ...
 training_contract:{
  ...
  available:h=>!!(homeCommercialUpgradeLevel(h,'trainingYard')&&h.security?.headGuard&&(h.security.guards||0)>=4),
  capability:h=>Math.max(1,(h.security.headGuard?.competence||1)+homeCommercialUpgradeLevel(h,'trainingYard')*1.5+(h.security.quality||1)),
 ...
 archive_commission:{
  ...
  available:h=>!!(homeCommercialUpgradeLevel(h,'archives')&&(h.staff?.steward||(h.staff?.messengers||0)>=2)),
  capability:h=>Math.max(1,homeCommercialUpgradeLevel(h,'archives')*2+(h.staff?.steward?.competence||0)+(h.staff?.messengers||0)*.5),
 ...
 storage_lease:{
  ...
  available:h=>homeCommercialUpgradeLevel(h,'storeroom')>0&&homeCommercialAvailabilityStorageFree(h)>=4,
 ...
 wealthy_patron_commission:{
  ...
  available:h=>!!(homeCommercialAvailabilityPrestige()>=25&&homeCommercialAvailabilityProsperity()>=65&&h.artisan?.master&&(h.artisan.master.competence||0)>=9&&(h.artisan.workmen||0)>=4&&!h.artisan.currentProject),
 ...
 prestige_event:{
  ...
  available:h=>!!(homeCommercialAvailabilityPrestige()>=35&&homeCommercialAvailabilityProsperity()>=75&&h.upgrades?.banquetFacilities&&(h.staff?.cooks||0)>=3&&(h.staff?.attendants||0)>=4),
 ...
 infirmary_service:{
  ...
  available:h=>!!(homeCommercialUpgradeLevel(h,'infirmary')&&(h.staff?.medical||0)>=1&&(h.logistics?.supplies?.medical||0)>=30),
  capability:h=>Math.max(1,homeCommercialUpgradeLevel(h,'infirmary')*1.8+(h.staff?.medical||0)*1.5),
...
function homeCommercialUpgradeLevel(h,id){const v=h?.upgrades?.[id];return v===true?1:Math.max(0,Math.round(Number(v)||0))}
let homeCommercialAvailabilitySnapshot=null;
function homeCommercialAvailabilityPrestige(){return homeCommercialAvailabilitySnapshot?.prestige??(typeof homeHallPrestige==='function'?homeHallPrestige():0)}
function homeCommercialAvailabilityProsperity(){return homeCommercialAvailabilitySnapshot?.prosperity??(settlementState('shantium')?.prosperity||50)}
function homeCommercialAvailabilityStorageFree(h){return homeCommercialAvailabilitySnapshot?.storageFree??Math.max(0,homeStorageCapacity()-(h?.storage||[]).reduce((n,x)=>n+(x.qty||1),0))}
...
function homeCommercialOfferValue(def){
 ...
 // v1.6.44.1: keep prosperity, skill and prestige meaningful without letting their multipliers compound into runaway endgame margins.
 const market=.70+pros/180,adviser=1+adv*.035,skill=.88+Math.min(18,cap)*.018,variance=rnd(94,108)/100;
 const prestige=(typeof homeHallPrestige==='function'?homeHallPrestige():0),prestigeFactor=def.prestigeSensitive?1+Math.min(.12,prestige*.0015):1;
 return Math.max(24,Math.round(def.base*market*adviser*skill*variance*prestigeFactor))
}
function homeCommercialGenerateOffer(force=false){
 ...
 if(C.offers.length>=4)return null;if(!force&&state.world.day-C.lastOfferDay<3)return null;
 const now=()=>typeof performance!=='undefined'&&performance.now?performance.now():Date.now();let t=now();
 // Eligibility is a hot path on mature Halls. The Hall was already normalized above, so snapshot
 // active resources and offered types once rather than re-entering home state and rescanning arrays per definition.
 const usedResources=new Set(C.active.filter(x=>x?.status==='active').map(x=>x.resource)),offeredTypes=new Set(C.offers.map(o=>o.type));let availabilityMs=0;
 // Shared values used by several availability rules are calculated once for this offer-generation pass.
 homeCommercialAvailabilitySnapshot={prestige:(typeof homeHallPrestige==='function'?homeHallPrestige():0),prosperity:(settlementState('shantium')?.prosperity||50),storageFree:Math.max(0,homeStorageCapacity()-h.storage.reduce((n,x)=>n+(x.qty||1),0))};
 const pool=[];for(const [id,d] of Object.entries(HOME_COMMERCIAL_OPPORTUNITY_DEFS)){if(usedResources.has(d.resource)||offeredTypes.has(id))continue;const ta=now();let available=false;try{available=!!d.available(h)}catch(e){available=false}availabilityMs+=now()-ta;if(available)pool.push([id,d])}
 homeCommercialAvailabilitySnapshot=null;
 if(typeof sosPerfRecordDuration==='function'){sosPerfRecordDuration('Commercial Offers — Eligibility',now()-t);sosPerfRecordDuration('Commercial Eligibility — Availability Rules',availabilityMs)};
 if(!pool.length){C.lastOfferDay=state.world.day;return null}
 t=now();const [type,def]=pick(pool),duration=rnd(def.days[0],def.days[1]),pros=settlementState('shantium')?.prosperity||50,payout=homeCommercialOfferValue(def),offer={id:'bizopp_'+uid(),type,name:def.name,resource:def.resource,summary:def.summary,detail:def.detail(h),createdDay:state.world.day,expiresDay:state.world.day+rnd(4,7),duration,payout,prosperity:pros,status:'offered',supplies:def.supplies?{...def.supplies}:null};
 if(typeof sosPerfRecordDuration==='function')sosPerfRecordDuration('Commercial Offers — Build & Value',now()-t);
