...
function sengiaNpcPersonalProblem(npc,locId,r){
 ...
 if(role.includes('petition')){
  adjustJurisdictionRep(locId,1,SOSText("settlements_people_townlife.sengiaNpcPersonalProblem.009"));npcMemoryAdd(npc.id,SOSText("settlements_people_townlife.sengiaNpcPersonalProblem.010"),2);return SOSText("settlements_people_townlife.sengiaNpcPersonalProblem.012",npc.name)
 }
 ...
 if(role.includes('herder')||role.includes('cistern')||role.includes('well')){
  settlementState(locId).prosperity=Math.min(100,settlementState(locId).prosperity+1);npcMemoryAdd(npc.id,SOSText("settlements_people_townlife.sengiaNpcPersonalProblem.016"),1);return SOSText("settlements_people_townlife.sengiaNpcPersonalProblem.017",npc.name)
 }
 return null
}
...
function showNpcPoliticalConversation(locId,npcId){
 modalRouteEnter('showNpcPoliticalConversation',Array.from(arguments));
 const npc=settlementNpc(locId,npcId),present=npc&&currentNpcLocation(npc.id,npcHomeLocation(npc.id))===locId;if(!npc||!present)return showSettlementPeople(locId);
 const r=npcRelationshipState(npcId),a=npcFactionAlignment(npcId),loc=worldLocation(locId);if(!a)return showSettlementNPCConversation(locId,npcId);
 const faction=majorFaction(a.faction),profile=OPEN_WORLD_FACTIONS[a.faction]?localPoliticalProfile(locId,a.faction):null,control=settlementControl(locId),tone=(r.opinion||0)>=5?'speaks openly':(r.opinion||0)<=-3?'chooses their words carefully':'answers after a moment';
 const position=a.support<0?`I don't put much faith in ${faction?.short||a.faction}.`:`I tend to favor ${faction?.short||a.faction}.`;
 const local=profile?`${faction.short} is ${profile.overall>=70?'strongly placed':profile.overall>=50?'well established':profile.overall>=30?'visible but contested':'on uncertain ground'} in ${loc.name}.`:'';
 overlay(`<h2>${esc(npc.name)} — Local Politics</h2><p>${esc(npc.name)} ${tone}. “${position}”</p><p>${esc(a.reason)}</p>${local?`<div class="notice compact">${esc(local)} ${control===a.faction?'They currently hold local authority.':`Local authority currently rests with ${esc(majorFaction(control)?.short||control)}.`}</div>`:''}<div class="choice-list"><button id="npcPoliticsWhy">Ask what has shaped their view</button></div><div class="dialog-footer"><button id="npcPoliticsBack">Back to Conversation</button></div>`,true);
 $('#npcPoliticsWhy').onclick=()=>{if(r.lastPoliticalTalkDay!==state.world.day){r.lastPoliticalTalkDay=state.world.day;r.familiarity=Math.min(10,(r.familiarity||0)+1);npcMemoryAdd(npcId,`${npc.name} and the Guardian spoke about politics in ${loc.name}.`,1)}save();actionResult(`${npc.name} on Local Politics`,`${npc.name} explains their view in more detail. ${a.reason}`,'info',()=>showNpcPoliticalConversation(locId,npcId))};
 $('#npcPoliticsBack').onclick=()=>SOSServices.navigation.back(()=>showSettlementNPCConversation(locId,npcId))
}
...
function townLifeSocialize(locId){
 const people=settlementNpcsPresent(locId),loc=worldLocation(locId),ss=settlementState(locId);
 if(ss.lastSocializeDay===state.world.day)return actionResult('Already Socialized Today',`You have already spent time meeting people around ${loc.name} today.`,'info',()=>showTownLife(locId));
 ss.lastSocializeDay=state.world.day;
 if(people.length){const npc=pick(people),r=npcRelationshipState(npc.id);r.familiarity=Math.min(10,(r.familiarity||0)+1);r.opinion=Math.min(10,(r.opinion||0)+1);npcMemoryAdd(npc.id,`The Guardian spent time talking with people around ${loc.name}.`,1);save();return actionResult('A Local Conversation',`While spending time around ${loc.name}, you fall into conversation with ${npc.name}, ${npc.role}. The visit leaves you on slightly more familiar terms.`,'good',()=>showSettlementNPCConversation(locId,npc.id))}
 changeLocalReputation(locId,1,'Spent time among local residents');save();return actionResult('Local Socializing',`You spend time among the people of ${loc.name}. No particular introduction stands out, but the Guardian becomes a little more familiar around town.`,'info',()=>showTownLife(locId))
}
function townLifeMigratedSettlementContentHTML(locId){
 const regionalHere=regionalThreadAt(locId),opportunitiesHere=opportunityAt(locId),storyHere=activeCompanionStories().filter(x=>state.party.active.includes(x.id)&&((x.s.stage===1&&x.d.loc1===locId)||(x.s.stage===2&&x.d.loc2===locId)||x.s.status==='decision')),maps=treasureMapsAtLocation(locId),ledgerReady=storyLedgerResolutionReady(),site=adventureSiteForLocation(locId);
 const blocks=[];
 const problem=settlementProblemHTML(locId),event=settlementEventHTML(locId);
 if(problem||event)blocks.push(`<h3>Current Local Events</h3>${problem}${event}`);
 if(opportunitiesHere.length)blocks.push(`<h3>Local Opportunities</h3>${opportunitiesHere.map(opportunityMarkerHTML).join('')}`);
 if(regionalHere.length)blocks.push(`<div class="regional-thread-card"><b>Regional consequence:</b><br>${regionalHere.map(t=>`${esc(t.title)} — ${esc(t.notes[t.notes.length-1]||t.text)}`).join('<br>')}<br><button id="townRegionalSimulation">Region Overview</button></div>`);
 if(storyHere.length)blocks.push(`<div class="success notice"><b>Companion story:</b><br>${storyHere.map(x=>`${esc(state.party.members[x.id].name)} — ${esc(x.d.title)}`).join('<br>')}<br>${storyHere.map(x=>`<button data-townstoryhere="${x.id}">Continue ${esc(x.d.title)}</button>`).join('')}</div>`);
 if(maps.length)blocks.push(`<div class="success notice"><b>Treasure lead:</b> ${maps.map(m=>esc(m.name)).join(', ')}<br>${maps.map(m=>`<button data-towndigmap="${m.id}">Follow ${esc(m.name)}</button>`).join('')}</div>`);
 if(ledgerReady)blocks.push('<div class="warning notice"><b>The Watchfort Ledger:</b> This location can receive the recovered evidence.<br><button id="townLedgerDecision">Decide Recipient</button></div>');
 if(site)blocks.push(`<div class="success notice"><b>Adventure Site:</b> ${esc(site.name)} • ${adventureProgressText(site)}<br><button id="townExploreSite">Explore Interior</button></div>`);
 return blocks.join('')
}
function wireTownLifeMigratedSettlementContent(locId){
...
function resolveSettlementEvent(locId,choice){
 const e=settlementEvent(locId);if(!e||e.resolved)return showTownLife(locId);const ss=settlementState(locId);let text='',tone='info';
 if(e.type==='refugees'&&choice==='hall_lodging'){
   ...
   if(!admit.ok&&admit.reason==='beds')return actionResult(SOSText("hall_life_visitors_diplomacy.homeAudienceResult.017"),SOSText("hall_life_visitors_diplomacy.homeAudienceResult.018",need,need===1?'':'s',admit.available,admit.total),'info',modalCurrentReturn(()=>showTownLife(locId)));
   if(!admit.ok)return actionResult(SOSText("hall_life_visitors_diplomacy.homeAudienceResult.019"),SOSText("hall_life_visitors_diplomacy.homeAudienceResult.020"),'bad',modalCurrentReturn(()=>showTownLife(locId)));
 ...
 e.resolved=true;e.resolvedDay=state.world.day;recordWorldHistory(`${worldLocation(locId).name}: ${e.title} — ${text}`,tone,SOSText("settlements_people_townlife.resolveSettlementEvent.015"));save();actionResult(e.title,text,tone,()=>showTownLife(locId))
}
