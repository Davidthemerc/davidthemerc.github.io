...
function populationAtSettlementHTML(locId){
 ...
 return SOSText("settlements_problems_stories.populationAtSettlementHTML.001",house.map(h=>`<div class="card compact"><b>${esc(h.name)}</b><br><small>${h.members} people • ${esc(h.status)} since Day ${h.sinceDay}</small></div>`).join(''),moves.map(x=>`<div class="card compact"><b>Day ${x.day}</b> — ${esc(x.text)}</div>`).join(''))
}
...
function showShantiumCommunity(){modalRouteEnter(SOSText("settlements_problems_stories.showShantiumCommunity.001"),Array.from(arguments));
 ...
 overlay(SOSText("settlements_problems_stories.showShantiumCommunity.002",esc(shantiumCommunityText()),esc(h.publicMood),ss.security,ss.prosperity,esc(localRepTier(rep)),recent.map(x=>`<div class="card compact"><b>Day ${x.day}</b> — ${esc(x.text)}</div>`).join('')||'<p class="muted">The Guardian’s place in Shantium is still being defined.</p>',localReputationBenefits('shantium').map(x=>`<li>${esc(x)}</li>`).join(''),settlementProblemHTML('shantium')),true);if($('#helpLocalProblem'))$('#helpLocalProblem').onclick=()=>helpSettlementProblem('shantium');$('#communityBack').onclick=()=>SOSServices.navigation.back(showHomeBase)
}
