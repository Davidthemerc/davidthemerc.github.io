...
function comparisonHTML(it,ownerId='guardian'){const o=ownerFor(ownerId),cur=item(o?.equipment?.[it.slot]),cls=ownerClassName(ownerId);if(!cur)return SOSText("items_equipment.comparisonHTML.001",esc(it.slot),itemStatsLine(it));const fields=[[SOSText("items_equipment.comparisonHTML.002"),it.damage||0,cur.damage||0],[SOSText("items_equipment.comparisonHTML.003"),it.accuracy||0,cur.accuracy||0],[SOSText("items_equipment.comparisonHTML.004"),it.defense||0,cur.defense||0],[SOSText("items_equipment.comparisonHTML.005"),it.initiative||0,cur.initiative||0]],uNew=gearUtility(it,cls,'balanced',ownerId),uCur=gearUtility(cur,cls,'balanced',ownerId),delta=uNew-uCur,fit=delta>4?'stronger overall fit':delta<-4?'weaker overall fit':'sidegrade / specialized fit';return `<div class="gear-compare"><b>vs ${esc(cur.name)}</b><br>${fields.map(([n,a,b])=>`${n} ${a-b>0?'+':''}${a-b}`).join(' • ')}<br><small><b>${fit}</b>. ${gearAffinity(cls,it)>gearAffinity(cls,cur)?'Better class affinity. ':''}${traitScore(it,cls)>traitScore(cur,cls)+.5?'Traits provide a class-fit edge. ':''}High stacked gear bonuses have diminishing combat returns.</small></div>`}
