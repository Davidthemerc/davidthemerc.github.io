...
function equip(id,ownerId='guardian'){const it=item(id),owner=ownerFor(ownerId);if(!it||!it.slot||!owner||!invRemove(id))return;const old=owner.equipment[it.slot];if(old)invAdd(old);owner.equipment[it.slot]=id;if(it.id.startsWith('named_'))unlock('legend');sfx('coin');sfxWorld('equip');save();showInventory(ownerId)}
function unequip(ownerId,slot){const owner=ownerFor(ownerId);if(!owner||!owner.equipment?.[slot])return;invAdd(owner.equipment[slot]);owner.equipment[slot]=null;sfxWorld('equip');save();showInventory(ownerId)}
