...
function searchWildernessSite(id){
 ...
 if(reward[2])gainGold(reward[2]);S.status='surveyed';S.notes.push(reward[1]);sfxWorld('discovery');recordWorldHistory(`${site.name}: ${reward[1]}`,'good','exploration');save();actionResult(reward[0],reward[1],'good',()=>showWildernessSite(id))
}
...
function enterHiddenInterior(id){
 const d=interiorDef(id);if(!d)return showWildernessSite(id);const I=interiorState(id);if(!I.room)I.room=interiorStartRoom(id);if(!I.visited.includes(I.room))I.visited.push(I.room);sfxWorld('door');showHiddenInterior(id)
}
...
function showWorldArea(){ensureWorldState();const loc=worldLocation(state.world.location);/* v1.6.54.5: settlement use of this legacy area overlay is deprecated; Town Life is the canonical settlement hub. */if(isTownLikeLocation(loc))return showTownLife(loc.id);modalRouteEnter(SOSText("exploration_wilderness_artifacts.showWorldArea.001"),Array.from(arguments));
 if(loc.hidden){if(hiddenSiteRetired(loc.id))return actionResult('Exploration Retired',`${loc.name} has been removed from the active map. Restore it from the Exploration Journal if you want to investigate it again.`,'info',renderOpenWorld);return showWildernessSite(loc.id)};
 ...
 const spawnDistrict=locationRegion(loc.id)==='spawn'&&loc.type==='district';
 const townActions=town?SOSText("exploration_wilderness_artifacts.showWorldArea.006",`${loc.id==='shantium'?'<button id="areaCouncilHall"><b>Council Hall</b><small>Civic administration, law & public affairs</small></button>':''}${spawnDistrict?'<button id="areaSpawnDistrict"><b>Explore District</b><small>Avenues, landmarks & local places</small></button>':''}`):'';
...
function askAroundWorld(){
 ...
 const opps=opportunityAt(locId);if(opps.length&&roll<=2){const o=pick(opps);return actionResult(SOSText("exploration_wilderness_artifacts.askAroundWorld.002"),`${o.title}: ${o.summary}`,'info',()=>showRegionalOpportunity(o.id))}const threads=regionalThreadAt(locId);if(threads.length&&roll<=2){const t=pick(threads);return actionResult(SOSText("exploration_wilderness_artifacts.askAroundWorld.003"),`${t.title}: ${t.notes[t.notes.length-1]||t.text}`,'info',()=>isTownLikeLocation(loc)?showTownLife(locId):showWorldArea())}if(event&&!event.resolved&&roll<=2)return actionResult(SOSText("exploration_wilderness_artifacts.askAroundWorld.004"),`${event.title}: ${event.text}`,'info',()=>isTownLikeLocation(loc)?showTownLife(locId):showWorldArea());
