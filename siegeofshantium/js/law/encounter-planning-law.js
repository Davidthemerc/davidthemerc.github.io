...
function bribeLocalAuthority(id){
 const L=lawState(),cost=Math.max(20,Math.round(localBounty(id)*.55));if(state.gold<cost)return showLocalLaw(id);const roll=rnd(1,20)+stat(state,'cha')-lawProfile(id).strict*2;state.gold-=cost;sfxWorld('coinsSmall');L.bribes=(L.bribes||0)+1;
...
function showLocalLaw(id=state.world.location){if(isOpenWorld()&&typeof localServiceContextAvailable==='function'&&!localServiceContextAvailable(id))return actionResult('Not Present',`The Guardian must be in ${esc(worldLocation(id).name)} to deal directly with its local authority.`,'info',renderOpenWorld);modalRouteEnter(SOSText("law_encounter_planning_law.showLocalLaw.001"),Array.from(arguments));
