...
function submitToArrest(id){sfxWorld('custody');return beginLawDetention(id,'voluntary surrender')}
...
function attemptLawCustodyEscape(){const d=activeLawDetention();if(!d)return;const security=state.world.settlements[d.id]?.security??50,pct=clamp(Math.round(12+stat(state,'dex')*3+(state.scouting||0)*2+(guardianClass()==='Rogue'?18:guardianClass()==='Ranger'?10:0)-security*.35-d.strict*4-d.severity*3),4,72);const ok=rnd(1,100)<=pct;lawIIIState().escapes++;
 if(ok){lawIIIState().activeDetention=null;recordCrimeDetailed('escape_custody',d.id,d.faction,{witnessed:true,severity:Math.max(3,d.severity),desc:`escaped ${d.authority} custody`});const w=normalizeLawWarrantIII(d.id);if(w){w.severity=Math.max(w.severity,d.severity+1);w.status='escaped'}lawCustodyHistory(`${state.name} escaped ${d.authority} custody in ${d.location}.`,'bad',{type:'escape'});placePlayerPartyOutsideSettlement(d.id);sfxWorld('escape');save();return actionResult('Escape Successful',`The Player Party slips out of custody and reaches the outskirts of ${esc(d.location)}. The warrant remains active and now includes escape from custody.`,'bad',renderOpenWorld)}
 d.aggravation+=2;d.severity=Math.min(7,d.severity+1);recordCrimeDetailed('escape_custody',d.id,d.faction,{witnessed:true,severity:3,desc:`attempted escape from ${d.authority} custody`});lawCustodyHistory(`${state.name} failed an escape attempt from ${d.authority} custody.`,'bad',{type:'failed_escape'});sfxWorld('warning');save();actionResult('Escape Failed',`The attempt is stopped. Security tightens, and escape from custody is added to the case.`,'bad',showLawDetention)
}
...
function showLocalLaw(id=state.world.location){if(isOpenWorld()&&typeof localServiceContextAvailable==='function'&&!localServiceContextAvailable(id))return actionResult('Not Present',`The Guardian must be in ${esc(worldLocation(id).name)} to deal directly with its local authority.`,'info',renderOpenWorld);modalRouteEnter('Local Law',Array.from(arguments));const C=lawIIIState(),L=lawState(),p=jurisdictionRule(id),b=localBounty(id),h=lawHeat(id),rest=restitutionDue(id),crimes=L.crimes.filter(c=>c.locId===id).slice(-8).reverse(),jrep=jurisdictionRep(id),w=normalizeLawWarrantIII(id),history=C.history.filter(x=>x.id===id).slice(-5).reverse();
