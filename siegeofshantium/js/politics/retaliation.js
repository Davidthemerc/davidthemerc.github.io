...
function politicalRetaliationVictoryHTML(gr){
 ...
 if(identified){const f=politicalRetaliationIdentify(gr,'defeated attackers'),locId=gr.politicalRetaliationLocId,c=localPoliticalFactionState(locId,f),severity=clamp(Math.round((gr.members?.length||2)/2),1,4),supportHit=.3+severity*.12,orgHit=.45+severity*.18,legHit=.55+severity*.15;c.support=clamp(c.support-supportHit,-6,12);c.organization=clamp(c.organization-orgHit,-6,12);c.legitimacy=clamp(c.legitimacy-legHit,-6,12);const ps=politicalSettlement(locId);ps.pressure[f]=Math.max(0,(ps.pressure[f]||0)-(.2+severity*.12));markLocalPoliticsDirty(locId);recordLocalPoliticalAction(locId,f,'retaliation_defeated',`${majorFaction(f).short}-aligned attackers were defeated after striking at the Guardian, and their affiliation was established from the surviving evidence.`);recordWorldHistory(`Evidence recovered after a defeated attack on the Guardian ties the assailants to ${majorFaction(f).short} in ${worldLocation(locId).name}. The failed attack damages the faction's local political standing.`,'bad','politics');return `<div class="warning notice"><b>Attackers Identified — ${esc(majorFaction(f).short)}</b><br>Insignia, testimony, documents, or recognizable ties among the defeated attackers establish their local political affiliation. Their defeat and exposure damage ${esc(majorFaction(f).short)} political standing here. This does not necessarily prove which leader ordered the attack.</div>`}
