...
function showSengiaAuthorityCase(locId){modalRouteEnter(SOSText("regions_sengia_redstone.showSengiaAuthorityCase.001"),Array.from(arguments));
 ...
 document.querySelectorAll('[data-sengiaorder]').forEach(b=>b.onclick=()=>resolveSengiaAuthorityCase(locId,b.dataset.sengiaorder));$('#sengiaOrderBack').onclick=()=>SOSServices.navigation.back(()=>showSengiaAuthority(locId))
}
...
function showRedstoneCivicDecision(locId){modalRouteEnter(SOSText("regions_sengia_redstone.showRedstoneCivicDecision.001"),Array.from(arguments));
 ...
 document.querySelectorAll('[data-redcivic]').forEach(b=>b.onclick=()=>resolveRedstoneCivicIssue(locId,b.dataset.redcivic));$('#redCivicBack').onclick=()=>SOSServices.navigation.back(()=>showRedstoneCivicLife(locId))
}
...
function defaultLocalReputation(){return Object.fromEntries(WORLD_LOCATIONS.filter(x=>['town','settlement','camp','fort','district'].includes(x.type)).map(x=>[x.id,x.id==='shantium'?4:0]))}
